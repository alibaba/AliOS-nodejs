...
// TODO(wanglin) :move to atrace class
#include "atrace.h"
#undef YTRACE_TAG
#define YTRACE_TAG YTRACE_TAG_DALVIK
#endif
// <YUNOS> changed: end
...
void CompilationJob::RecordOptimizedCompilationStats() const {
  ...
  if (!function->IsOptimized()) {
    ...
    int opt_count = function->feedback_vector()->opt_count();
    function->feedback_vector()->set_opt_count(opt_count + 1);
  }
...
void EnsureFeedbackMetadata(CompilationInfo* info) {
  ...
  if (info->shared_info()->feedback_metadata()->length() == 0 ||
    ...
    SharedCodegenScope scs(info->isolate()->vmthread(),
                           info->shared_info()->is_shared_object());
...
bool ShouldUseFullCodegen(FunctionLiteral* literal, Handle<Script> script = Handle<Script>::null()) {
  ...
  if (!script.is_null() && script->is_shared_object()) return false;
...
CompilationJob* GetUnoptimizedCompilationJob(CompilationInfo* info) {
  ...
  if (ShouldUseFullCodegen(info->literal(), info->script())) {
...
void EnsureSharedFunctionInfosArrayOnScript(CompilationInfo* info) {
  ...
  PretenureFlag pretenure = info->script()->is_shared_object() ?
                            SHARED_TENURED : TENURED;
  Handle<FixedArray> infos(info->isolate()->factory()->NewFixedArray(
      info->parse_info()->max_function_literal_id() + 1, pretenure));
...
CompilationJob::Status FinalizeUnoptimizedCompilationJob(CompilationJob* job) {
  ...
  bool need_shared = info->has_shared_info() &&
                     info->shared_info()->is_shared_object();
  SharedCodegenScope scs(isolate->vmthread(), need_shared);
...
bool GenerateUnoptimizedCode(CompilationInfo* info) {
  if (UseAsmWasm(info->scope(), info->shared_info(), info->is_debug())) {
    ...
    {
      UnSharedCodegenScope ucs(info->isolate()->vmthread());
      wasm_data = AsmJs::CompileAsmViaWasm(info);
    }
    if (!wasm_data.is_null()) {
      SetSharedFunctionFlagsFromLiteral(info->literal(), info->shared_info());
      info->shared_info()->SetAsmWasmData(info->isolate(), wasm_data.ToHandleChecked());
...
bool CompileUnoptimizedCode(CompilationInfo* info,
  ...
  if (info->parse_info()->is_toplevel() &&
      (ShouldUseFullCodegen(info->literal(), info->script()) ||
...
bool CompileUnoptimizedCodeSafe(CompilationInfo* info,
                                ConcurrencyMode inner_function_mode) {
  if (info->has_shared_info()) {
    Isolate* isolate = info->isolate();
    Handle<SharedFunctionInfo> shared = info->shared_info();
    sync::MonitorLockScope<SharedFunctionInfo> mls(isolate->vmthread(), shared);

    if (shared->is_compiled() &&
        !shared->NeedReCompileWasmData(isolate)) {
      // NOTE: set BytecodeArray in InstallSharedCompilationResult after compilation.
      info->SetCode(Handle<Code>(shared->code(), isolate)); // interpreter trampoline
      if (shared->HasBytecodeArray()) {
        info->SetBytecodeArray(Handle<BytecodeArray>(shared->bytecode_array(), isolate));
      }
      return true;
    }
    return CompileUnoptimizedCode(info, inner_function_mode);
  } else {
    return CompileUnoptimizedCode(info, inner_function_mode);
  }
}
...
MUST_USE_RESULT MaybeHandle<Code> GetUnoptimizedCode(
  ...
  if (!CompileUnoptimizedCodeSafe(info, inner_function_mode)) {
    return MaybeHandle<Code>();
  }
...
MaybeHandle<Code> GetOptimizedCode(Handle<JSFunction> function,
  ...
  if (function->feedback_vector()->deopt_count() > kMaxDeoptCount) {
...
MaybeHandle<Code> GetLazyCode(Handle<JSFunction> function) {
  ...
  DCHECK(!function->is_compiled() ||
          function->shared()->NeedReCompileWasmData(isolate));
  ...
  if (function->shared()->is_compiled() &&
    ...
    UnSharedCodegenScope ucs(isolate->vmthread());
...
bool Compiler::Analyze(ParseInfo* info, Isolate* isolate,
  ...
  bool need_shared = !info->shared_info().is_null() &&
                     info->shared_info()->is_shared_object();
  SharedCodegenScope scs(isolate->vmthread(), need_shared);
...
bool Compiler::Compile(Handle<JSFunction> function, ClearExceptionFlag flag) {
  ...
  if (function->is_compiled() &&
      !function->shared()->NeedReCompileWasmData(isolate)) return true;
  DCHECK(AllowCompilation::IsAllowed(isolate));

  CompilerDispatcher* dispatcher = isolate->compiler_dispatcher();
  Handle<SharedFunctionInfo> shared(function->shared(), isolate);
  SharedCodegenScope scs(isolate->vmthread(), shared->is_shared_object());
  Handle<Code> code;
...
bool Compiler::CodeGenerationFromStringsAllowed(Isolate* isolate,
  ...
  if (callback == NULL) {
    ...
    Local<v8::Context> local_context = v8::Utils::ToLocal(context);
    Local<v8::String> local_source = v8::Utils::ToLocal(source);
    VMState<EXTERNAL> state(isolate);
    return callback(local_context, local_source);
  }
}
...
Handle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForScript(
  ...
  Isolate* isolate = source->GetIsolate();
  bool need_shared = (!FLAG_always_opt && !FLAG_stress_fullcodegen
                     && natives == NOT_NATIVES_CODE) ||
                     isolate->vmthread()->in_shared_codegen();
  // <YUNOS> changed: begin
  if (FLAG_flatten_source_code_for_seed) {
    source = String::Flatten(source);
  }
  // <YUNOS> changed: end
  SharedCodegenScope scs(isolate->vmthread(), need_shared);
  if (need_shared) {
    if (!source->is_shared_object()) {
      source = isolate->factory()->InternalizeString(source);
    }
    if (!script_name.is_null() &&
        !Handle<String>::cast(script_name)->is_shared_object()) {
      script_name = isolate->factory()->InternalizeString(
          Handle<String>::cast(script_name));
    }
  }
  ...
  Handle<SharedFunctionInfo> result;
  Handle<Cell> vector = Handle<Cell>::null();
  if (extension == NULL) {
    // First check per-isolate compilation cache.
    InfoVectorPair pair;
    if (need_shared) {
      pair = compilation_cache->LookupSharedScript(
          source, script_name, line_offset, column_offset, resource_options,
          context, language_mode);
    } else {
      pair = compilation_cache->LookupScript(
          source, script_name, line_offset, column_offset, resource_options,
          context, language_mode);
    }
...
#endif
        if (need_shared) {
          compilation_cache->PutSharedScript(source, script_name, line_offset,
  ...
  if (result.is_null() ||
    ...
    if (!source_map_url.is_null()) {
      script->set_source_mapping_url(*isolate->factory()->
          InternalizeString(
            Handle<String>(String::cast(*source_map_url), isolate)));
    }
    ...
    if (extension == NULL && !result.is_null()) {
      ...
      if (need_shared) {
        compilation_cache->PutSharedScript(source, script_name, line_offset,
                           column_offset, resource_options,
                           context, language_mode, result, vector);
      } else {
...
Handle<SharedFunctionInfo> Compiler::GetSharedFunctionInfo(
  ...
  sync::MonitorLockScope<Script> mls(isolate->vmthread(), script);

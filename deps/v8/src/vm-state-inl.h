...
VMState<Tag>::VMState(Isolate* isolate)
  ...
  if (Tag == EXTERNAL) {
    ...
    old_state_ = self->state();
    if (self->IsPreThreadFlagSet(VMThread::FAST_API_CALL)) {
      self->ClearPreThreadFlag(VMThread::FAST_API_CALL);
    } else {
      if (old_state_ != kNative) {
        if (old_state_ == kRunnable)
          self->TransitionFromRunnableToSuspended(kNative);
        else
          self->set_state(kNative);
      }
    }
  }
  ...
  isolate_->vmthread()->AssertCurrentThread();
...
VMState<Tag>::~VMState() {
  if (Tag == EXTERNAL) {
    ...
    if (self->state() != old_state_) {
      if (old_state_ == kRunnable)
        self->TransitionFromSuspendedToRunnable();
      else
        self->set_state(old_state_);
    }
  }
  if (FLAG_log_timer_events && previous_tag_ != EXTERNAL && Tag == EXTERNAL) {

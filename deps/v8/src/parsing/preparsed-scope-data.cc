...
MaybeHandle<PreParsedScopeData> ProducedPreParsedScopeData::Serialize(
  ...
  Handle<PodArray<uint32_t>> data_array =
      PodArray<uint32_t>::New(isolate, static_cast<int>(length + 1), pretenure);
  ...
  Handle<PreParsedScopeData> data =
      isolate->factory()->NewPreParsedScopeData(pretenure);
  ...
  if (child_data_length == 0) {
    ...
    Handle<FixedArray> child_array =
        isolate->factory()->NewFixedArray(child_data_length, pretenure);
    int i = 0;

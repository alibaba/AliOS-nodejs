...
#undef DEF_ENUM
  ...
  bool FindCodeInSharedCacheUnlocked(Code** code_out);
...
class CodeHashMapShape : public BaseShape<Smi*> {
 public:
  static inline bool IsMatch(Smi* key, Object* value) {
    CHECK(value->IsSmi());
    return key->value() == Smi::cast(value)->value();
  }

  static inline uint32_t Hash(Smi* key) {
    return ComputeIntegerHash(key->value(), 0);
  }
...
class CodeHashMap : public GcFreeHashMap<CodeHashMapShape, Smi*> {
 public:
  const int32_t kDefaultSize = 128;
  explicit CodeHashMap(Isolate* isolate)
                    : GcFreeHashMap<CodeHashMapShape, Smi*>(isolate, 0),
                      code_array_(NULL) {}

  ~CodeHashMap() {
     if (code_array_ != NULL) {
      Malloced::Delete(code_array_);
    }
    code_array_ = NULL;
  }

  int FindEntry(uint32_t key) {
    return GcFreeHashMap<CodeHashMapShape, Smi*>::FindEntry(Smi::FromInt(key));
  }

  void Put(uint32_t key, Handle<Object> object) {
    Put(key, *object);
  }

  void Put(uint32_t key, Object* object) {
    int entry_index = FindEntry(key);
    if (entry_index != kNotFound) {
      // have the key, update the value
      SetKeyAndValue(entry_index, Smi::FromInt(key), object);
    } else {
      // we need to insert a <key, value>
      InsertEntry(Smi::FromInt(key), object);
    }
  }

  Object* ValueAt(int index) {
    DCHECK(index < static_cast<int>(Capacity()));
    return code_array_[index];
  }

  void DeleteKey(uint32_t key) {
    int index = FindEntry(key);
    if (index == kNotFound) return;
    SetKeyAndValue(index, Smi::FromInt(0), Smi::FromInt(0));
    number_of_elements_ = Smi::FromInt(number_of_elements_->value() - 1);
  }

  virtual void EnsureCapacity(Smi* key, uint32_t n);
  virtual void Iterate(RootVisitor* v, VisitMode mode);
  static const int kNotFound = -1;

 private:
  void InsertEntry(Smi* key, Object* value);
  void SetKeyAndValue(uint32_t index, Smi* key, Object* value) {
    SetEntry(index, key);
    code_array_[index] = value;
  }
  void PrintThis();

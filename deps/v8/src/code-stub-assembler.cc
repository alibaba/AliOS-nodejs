...
Node* CodeStubAssembler::AllocateRaw(Node* size_in_bytes, AllocationFlags flags,
                                     Heap::RootListIndex top_address,
                                     Node* top, Node* limit) {
...
Node* CodeStubAssembler::AllocateRawUnaligned(Node* size_in_bytes,
  ...
  return AllocateRaw(size_in_bytes, flags, top_address, top, limit);
}
...
Node* CodeStubAssembler::Allocate(Node* size_in_bytes, AllocationFlags flags) {
  ...
  Heap::RootListIndex top_address = new_space ?
                      Heap::kNewSpaceAllocationTopRootIndex :
                      Heap::kOldSpaceAllocationTopRootIndex;
  Heap::RootListIndex limit_address = new_space ?
                      Heap::kNewSpaceAllocationLimitRootIndex :
                      Heap::kOldSpaceAllocationLimitRootIndex;

  Node* top = LoadRoot(top_address);
  Node* limit = LoadRoot(limit_address);
...
Node* CodeStubAssembler::LoadAndUntagToWord32Root(
  ...
  int index = root_index * kPointerSize + Isolate::kHeapRootsDelta;
  if (Is64()) {
#if V8_TARGET_LITTLE_ENDIAN
    index += kPointerSize / 2;
#endif
    return LoadRoot(compiler::RootType::kInt32RootType, index);
  } else {
    return SmiToWord32(LoadRoot(compiler::RootType::kAnyTaggedRootType, index));
  }
}
...
void CodeStubAssembler::TrapAllocationMemento(Node* object,
  ...
  Node* new_space_top = LoadRoot(Heap::kNewSpaceAllocationTopRootIndex);
...
Node* CodeStubAssembler::CreateAllocationSiteInFeedbackVector(
  ...
  Node* next_site = LoadRoot(Heap::kAllocationSitesListRootIndex);
  ...
  StoreRoot(compiler::RootType::kTaggedPointerRootType,
            Heap::kAllocationSitesListRootIndex, site);
...
Node* CodeStubAssembler::IsDebugActive() {
  Node* is_debug_active_address = LoadTL(ThreadLocalTop::kDebugIsActiveAddress);
  Node* is_debug_active = Load(
      MachineType::Uint8(), is_debug_active_address);
  return Word32NotEqual(is_debug_active, Int32Constant(0));
}

Node* CodeStubAssembler::IsPromiseHookEnabledOrDebugIsActive() {
  Node* promise_hook_or_debug_is_active = LoadTL(compiler::RootType::kUint8RootType,
        ThreadLocalTop::kPromiseHookOrDebugIsActiveAddress);
...
void CodeStubAssembler::Print(const char* prefix, Node* tagged_value) {
  ...
  if (prefix != nullptr) {
    ...
    Handle<String> string = isolate()->factory()->NewStringFromAsciiChecked(
        formatted.c_str(), SHARED_TENURED);
    CallRuntime(Runtime::kGlobalPrint, NoContextConstant(),
                HeapConstant(string));
  }

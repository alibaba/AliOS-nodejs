...
class FeedbackVectorSpec : public FeedbackVectorSpecBase<FeedbackVectorSpec> {
 public:
  ...
  // this number changes.
  static const int kTypeProfileSlotIndex = 8; //FeedbackVector::kReservedIndexCount
 private:
...
class FeedbackVector : public FixedArray {
 public:
  ...
  static const int kInterruptBudgetIndex = 4;
  static const int kOSRNestingLevelIndex = 5;
  static const int kOptCountIndex = 6;
  static const int kCountersIndex = 7;
  static const int kReservedIndexCount = 8;
  ...
  inline void set_interrupt_budget(int budget);

  inline int osr_loop_nesting_level() const;
  inline void set_osr_loop_nesting_level(int level);

  inline int opt_count() const;
  inline void set_opt_count(int count);

  inline int counters() const;
  inline void set_counters(int count);

  inline int deopt_count() const;
  inline void set_deopt_count(int count);
  inline void increment_deopt_count();

  inline void TryReenableOptimization();
  inline int opt_reenable_tries() const;
  inline void set_opt_reenable_tries(int tries);

  inline bool optimization_disabled() const;
  inline void set_optimization_disabled(bool disabled);
...
// Bit fields in |counter_slot|.
#define COUNTERS_BIT_FIELDS(V, _)     \
  V(SmiPlaceHolderBit, int, 1, _)     \

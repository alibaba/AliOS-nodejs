...
void RuntimeProfiler::AttemptOnStackReplacement(JavaScriptFrame* frame,
  ...
  if (function->has_feedback_vector()
          && function->feedback_vector()->optimization_disabled()) return;
  ...
  if (frame->type() == StackFrame::JAVA_SCRIPT) {
    ...
    int level = function->feedback_vector()->osr_loop_nesting_level();
    function->feedback_vector()->set_osr_loop_nesting_level(
...
void RuntimeProfiler::MaybeOptimizeFullCodegen(JSFunction* function,
  ...
  if (!shared->optimization_disabled() &&
          function->feedback_vector()->optimization_disabled()) {
    if (function->feedback_vector()->deopt_count() >= FLAG_max_deopt_count) {
      // If optimization was disabled due to many deoptimizations,
      // then check if the function is hot and try to reenable optimization.
      int ticks = function->feedback_vector()->profiler_ticks();
      if (ticks >= kProfilerTicksBeforeReenablingOptimization) {
        function->feedback_vector()->set_profiler_ticks(0);
        function->feedback_vector()->TryReenableOptimization();
      }
    }
    return;
  }
...
void RuntimeProfiler::MaybeOptimizeIgnition(JSFunction* function,
  ...
  if (function->feedback_vector()->optimization_disabled()) {
    if (function->feedback_vector()->deopt_count() >= FLAG_max_deopt_count) {
      // If optimization was disabled due to many deoptimizations,
      // then check if the function is hot and try to reenable optimization.
      if (ticks >= kProfilerTicksBeforeReenablingOptimization) {
...
void RuntimeProfiler::MarkCandidatesForOptimization() {
  ...
  if (FLAG_delay_optimized_compiler &&
      isolate_->flags().during_delay_jit() &&
      !isolate_->delay_jit_ticks().IsNull()) {
    base::TimeDelta delta = base::TimeTicks::Now()
      - isolate_->delay_jit_ticks();
    if (delta.InMilliseconds() >
        FLAG_max_delay_compiler_times) {
      PrintF("Exit Key scene because of time out! use %lld ms",
          static_cast<long long>(delta.InMilliseconds()));
      isolate_->flags().set_during_delay_jit(false);
      isolate_->set_delay_jit_ticks(base::TimeTicks());
    }
  }

  if (isolate_->flags().during_delay_jit()) return;

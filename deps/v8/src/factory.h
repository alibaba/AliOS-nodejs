...
class V8_EXPORT_PRIVATE Factory final {
 public:
  ...
  Handle<String> NewStringConstantFromAsciiChecked(const char* str);
  ...
  // the old generation).
  Handle<Struct> NewStruct(InstanceType type, PretenureFlag pretenure = TENURED);
  ...
  Handle<Object> NewHeapConstantNumber(double value);
  ...
  // <YUNOS> START
  Handle<JSArrayBuffer> NewJSArrayBufferex(
      PretenureFlag pretenure = NOT_TENURED);
  // <YUNOS> END
  ...
  Handle<PreParsedScopeData> NewPreParsedScopeData(PretenureFlag pretenure = TENURED);
...
#undef SYMBOL_ACCESSOR
 ...
 private:
  ...
  inline Handle<Code> NewCodeRaw(int object_size, bool immovable, bool is_shared);

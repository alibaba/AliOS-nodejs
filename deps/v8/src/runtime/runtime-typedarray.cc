...
RUNTIME_FUNCTION(Runtime_ArrayBufferNeuter) {
  ...
  array_buffer->Neuter();
  // <YUNOS> START:
  // get the array-buffer-allocator according to allocator_id
  const int allocator_id = array_buffer->allocator_id();
  v8::ArrayBuffer::Allocator* allocator =
      isolate->array_buffer_allocator(allocator_id);
  if (array_buffer->is_shared_process()) {
    ArrayBuffer::Allocator::FreeShared(backing_store, byte_length);
  } else {
    allocator->Free(backing_store, byte_length);
  }
  // <YUNOS> END
  return isolate->heap()->undefined_value();
}

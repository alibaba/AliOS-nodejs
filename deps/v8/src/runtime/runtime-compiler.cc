...
RUNTIME_FUNCTION(Runtime_InstantiateAsmJs) {
  ...
  if (function->shared()->HasAsmWasmData()) {
    FixedArray* wasm_data = function->shared()->GetAsmWasmData(isolate);
    if (wasm_data == isolate->heap()->empty_fixed_array()) {
      StackLimitCheck check(isolate);
      if (check.JsHasOverflowed(kStackSpaceRequiredForCompilation * KB)) {
        return isolate->StackOverflow();
      }
      /*Because the function may have been compiled, so should reset the code*/
      function->set_code(isolate->builtins()
          ->builtin(Builtins::kCompileLazy));
      if (!Compiler::Compile(function, Compiler::KEEP_EXCEPTION)) {
        return isolate->heap()->exception();
      }
      DCHECK(function->is_compiled());
    }
    wasm_data = function->shared()->GetAsmWasmData(isolate);
    DCHECK(wasm_data != isolate->heap()->empty_fixed_array());
    Handle<FixedArray> data(wasm_data, isolate);
  ...
  if (function->shared()->HasAsmWasmData()) {
    function->shared()->ClearAsmWasmData(isolate);
  }
...
static bool IsSuitableForOnStackReplacement(Isolate* isolate,
  ...
  // deopt to many times
  if (function->feedback_vector()->optimization_disabled()) return false;
...
BailoutId DetermineEntryAndDisarmOSRForInterpreter(JavaScriptFrame* frame) {
  ...
  iframe->function()->feedback_vector()->set_osr_loop_nesting_level(0);

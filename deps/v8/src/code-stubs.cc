...
bool CodeStub::FindCodeInSharedCacheUnlocked(Code** code_out) {
  CodeHashMap* code_stubs_cache = isolate()->heap()->shared_code_stubs_cache();
  int index = code_stubs_cache->FindEntry(GetKey());
  if (index != CodeHashMap::kNotFound) {
    *code_out = Code::cast(code_stubs_cache->ValueAt(index));
    return true;
  }
  return false;
}
...
bool CodeStub::FindCodeInCache(Code** code_out) {
  ...
  VMThread* self = isolate_->vmthread();
  sync::MutexLock mu(self,
           WorkerRuntime::GetLock(worker_runtime, WorkerRuntime::kCodeStubs));
  return FindCodeInSharedCacheUnlocked(code_out);
}
...
void CodeStub::DeleteStubFromCacheForTesting() {
  ...
  sync::MutexLock mu(self,
           WorkerRuntime::GetLock(worker_runtime, WorkerRuntime::kCodeStubs));
  CodeHashMap* code_stubs_cache = heap->shared_code_stubs_cache();
  code_stubs_cache->DeleteKey(GetKey());
}
...
Handle<Code> CodeStub::GetCode() {
...

  {
    ...
    SharedCodegenScope scs(isolate()->vmthread());
    ...
    if (UseSpecialCache()) {
      ...
      sync::MutexLock mu(self,
          WorkerRuntime::GetLock(worker_runtime, WorkerRuntime::kCodeStubs));
      if (FindCodeInSharedCacheUnlocked(&code)) {
        DCHECK(GetCodeKind() == code->kind());
      } else {

        // Update the dictionary and the root in Heap.
        CodeHashMap* code_stubs_cache = heap->shared_code_stubs_cache();
        code_stubs_cache->Put(GetKey(), new_object);
        DCHECK(new_object->is_shared_object());
      }
    }
    if (code == nullptr) {
      code = *new_object;
...
void JSEntryStub::FinishCode(Handle<Code> code) {
  Handle<FixedArray> handler_table =
      code->GetIsolate()->factory()->NewFixedArray(1, SHARED_TENURED);
...
void CodeHashMap::InsertEntry(Smi* key, Object* value) {
  EnsureCapacity(key, 1);
  uint32_t entry = FindInsertEntry(Hash(key));
  SetKeyAndValue(entry, key, value);
  ElementAdded();
}

void CodeHashMap::EnsureCapacity(Smi* key, uint32_t n) {
  uint32_t expected_numbers = number_of_elements_->value() + n;
  if ((expected_numbers) < (Capacity() / 2)) return;

  /*
  PrintF("@Before EnsureCapacity: print this");
  PrintThis();
  */
  ...
  Object** new_key_array = reinterpret_cast<Object**>(Malloced::New(new_size * sizeof(Object*)));
  memset(new_key_array, kSmiTag, new_size * sizeof(Object*));

  Object** new_code_array = reinterpret_cast<Object**>(Malloced::New(new_size * sizeof(Object*)));
  memset(new_code_array, kSmiTag, new_size * sizeof(Object*));

  Object** old_key_array_ = key_array_;
  Object** old_code_array_ = code_array_;

  key_array_ = new_key_array;
  code_array_ = new_code_array;

  uint32_t size = Capacity();
  DCHECK(Smi::IsValid(new_size));
  size_ = Smi::FromInt(new_size);
  for (uint32_t i = 0; i < size; i++) {
    Object* key_value = old_key_array_[i];
    Object* obj = old_code_array_[i];
    if (IsKey(key_value)) {
      CHECK(key_value->IsSmi());
      int new_entry_index = FindInsertEntry(Hash(Smi::cast(key_value)));
      SetKeyAndValue(new_entry_index, Smi::cast(key_value), obj);
    }
  }

  if (old_code_array_ != NULL) {
    Malloced::Delete(old_key_array_);
    Malloced::Delete(old_code_array_);
  }

  /*
  PrintF("@After EnsureCapacity: print this\n");
  PrintThis();
  */
}

void CodeHashMap::Iterate(RootVisitor* v, VisitMode mode) {
  if (mode == VISIT_ONLY_STRONG_FOR_SERIALIZATION) {
    v->VisitRootPointer(Root::kCodeStubs, bit_cast<Object**>(&size_));
    v->VisitRootPointer(Root::kCodeStubs, bit_cast<Object**>(&number_of_elements_));
    v->VisitRootPointers(Root::kCodeStubs, bit_cast<Object**>(&key_array_[0]),
                     bit_cast<Object**>(&key_array_[size_->value()]));
    v->VisitRootPointers(Root::kCodeStubs, &code_array_[0],
                     &code_array_[size_->value()]);
  } else if (mode == VISIT_ONLY_STRONG_FOR_DESERIALIZATION) {
    v->VisitRootPointer(Root::kCodeStubs, bit_cast<Object**>(&size_));
    uint32_t new_size = size_->value();
    size_ = Smi::FromInt(0);
    EnsureCapacity(Smi::FromInt(0), new_size);
    v->VisitRootPointer(Root::kCodeStubs, bit_cast<Object**>(&number_of_elements_));
    v->VisitRootPointers(Root::kCodeStubs, bit_cast<Object**>(&key_array_[0]),
                     bit_cast<Object**>(&key_array_[size_->value()]));
    v->VisitRootPointers(Root::kCodeStubs, &code_array_[0],
    ...
    // JSMT(TODO): add back this check
    // CHECK(main_isolate_->GetCurrentThread()->IsThreadSafe());
    USE(main_isolate_);
    v->VisitRootPointers(Root::kCodeStubs, &code_array_[0],
                         &code_array_[size_->value()]);
  }
...
void CodeHashMap::PrintThis() {
  for(uint32_t i = 0; i < Capacity(); i++) {
    if (IsKey(key_array_[i])) {
      base::OS::Print("@@index:%d, key:0x%x, obj:%p\n", i, Smi::cast(key_array_[i])->value(), (void*)code_array_[i]);
    }
}
}

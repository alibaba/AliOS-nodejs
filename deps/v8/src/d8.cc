...
// See: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=79839
  ...
  void* VirtualMemoryAllocate(size_t length) {
    ...
    // <YUNOS> changed: begin
    if (data && !base::VirtualMemory::CommitRegion(data, length, false, "ShellArrayBuffer")) {
    // <YUNOS> changed: end
      base::VirtualMemory::ReleaseRegion(data, length);
      return nullptr;
    }
...
base::RecursiveMutex Shell::workers_mutex_;
bool Shell::allow_new_workers_ = true;
i::List<Worker*> Shell::workers_;
std::map<Isolate*, i::List<Worker*>*> isolate_related_workers_;
base::LazyMutex Shell::isolate_status_lock_;
std::map<v8::Isolate*, bool> Shell::isolate_status_;
WorkerSerialization* worker_serialization_;
...
void Shell::IsWorker(const v8::FunctionCallbackInfo<v8::Value>& args) {
  i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(args.GetIsolate());
  args.GetReturnValue().Set(!i_isolate->is_main_isolate());
}

void Shell::ThreadWorkerNew(const v8::FunctionCallbackInfo<v8::Value>& args) {
  Isolate* isolate = args.GetIsolate();
  HandleScope handle_scope(isolate);
  ...
  {
    base::LockGuard<base::RecursiveMutex> lock_guard(&workers_mutex_);
    ...
    if (isolate_related_workers_.count(isolate) == 0) {
      isolate_related_workers_.insert(std::make_pair(isolate, new i::List<Worker*>()));
    }
    isolate_related_workers_[isolate]->Add(worker);
    worker->SetOwner(isolate, args.Holder());

    workers_.Add(worker);

    for (int i = 0; i < args.Length(); i++) {
      if (!args[i]->IsString()) {
        Throw(args.GetIsolate(), "arguments must be string");
        return;
      }

      String::Utf8Value file(args[i]);
      worker->AddWorkerSource(i::StrDup(*file));
    }

    worker->StartExecuteInThread(isolate, nullptr);
  }
}
...
void Shell::Quit(const v8::FunctionCallbackInfo<v8::Value>& args) {
  if (reinterpret_cast<i::Isolate*>(args.GetIsolate())
      ->is_main_isolate()) {
...
Local<ObjectTemplate> Shell::CreateGlobalTemplate(Isolate* isolate) {
  ...
  global_template->Set(
      String::NewFromUtf8(isolate, "isworker", NewStringType::kNormal)
          .ToLocalChecked(),
      FunctionTemplate::New(isolate, IsWorker));
  ...
  Local<FunctionTemplate> thread_worker_fun_template =
      FunctionTemplate::New(isolate, ThreadWorkerNew);
  Local<Signature> thread_worker_signature =
      Signature::New(isolate, thread_worker_fun_template);
  thread_worker_fun_template->SetClassName(
      String::NewFromUtf8(isolate, "ThreadWorker", NewStringType::kNormal)
          .ToLocalChecked());
  thread_worker_fun_template->ReadOnlyPrototype();
  thread_worker_fun_template->PrototypeTemplate()->Set(
      String::NewFromUtf8(isolate, "terminate", NewStringType::kNormal)
          .ToLocalChecked(),
      FunctionTemplate::New(isolate, WorkerTerminate, Local<Value>(),
                            thread_worker_signature));
  thread_worker_fun_template->PrototypeTemplate()->Set(
  ...
  thread_worker_fun_template->PrototypeTemplate()->Set(
      ...
      FunctionTemplate::New(isolate, WorkerGetMessage, Local<Value>(),
                            thread_worker_signature));
  thread_worker_fun_template->InstanceTemplate()->SetInternalFieldCount(1);
  global_template->Set(
      String::NewFromUtf8(isolate, "ThreadWorker", NewStringType::kNormal)
          .ToLocalChecked(),
      thread_worker_fun_template);
...
void SerializationDataQueue::Clear() {
  base::LockGuard<base::Mutex> lock_guard(&mutex_);
  for (auto val : data_) {
    worker_serialization_->CleanupSerializationData(val);
  }
  data_.clear();
}


const char* Worker::PopWorkerSource() {
  if (worker_source_.empty()) return NULL;
  const char* src = worker_source_.front();
  worker_source_.pop();
  return src;
}


Worker::Worker(bool is_thread_worker)
    : in_semaphore_(0),
      out_semaphore_(0),
      script_(NULL),
      running_(false),
      is_stopped_(false),
      is_thread_worker_(is_thread_worker) {}
...
void Worker::WorkerCallback(Isolate* isolate, void* data) {
  Worker* worker = reinterpret_cast<Worker*>(data);
  worker->ExecuteInThread(isolate);
}
...
void Worker::StartExecuteInThread(Isolate* isolate, const char* script) {
  ...
  Isolate::CreateParams create_params;
  create_params.array_buffer_allocator = Shell::array_buffer_allocator;
  isolate->CreateNewThreadWorker(create_params,
           WorkerStartCallback(WorkerCallback, this));
}
...
SerializationData* Worker::GetMessageOnce() {
  SerializationData* result = nullptr;
...
void Worker::ExecuteInThread(Isolate* isolate) {
  isolate->SetHostImportModuleDynamicallyCallback(
      Shell::HostImportModuleDynamically);
  D8Console console(isolate);
  debug::SetConsoleDelegate(isolate, &console);
      ...
      Local<Context> context = Shell::CreateEvaluationContext(isolate);
      {
        ...
        if (!is_thread_worker_) {
          ...
          Local<String> source =
            ...
            .ToLocalChecked();
          success = Shell::ExecuteString(isolate, source, file_name, false, true);
        } else {
          const char* file;
          while ((file = PopWorkerSource()) != NULL) {
            int size = 0;
            char* src = ReadChars(file, &size);
            if (src == NULL) {
              Throw(isolate, "Can't get worker script");
              break;;
            }
            Local<String> file_name =
                String::NewFromUtf8(isolate, file, NewStringType::kNormal)
                .ToLocalChecked();
            delete []file;
            Local<String> source =
                String::NewFromUtf8(isolate, src, NewStringType::kNormal)
                .ToLocalChecked();
            success = Shell::ExecuteString(isolate,
                                           source, file_name, false, true);
          }
        }
        if (success) {
          // Get the message handler
          Local<Value> onmessage;
          // We allow that worker without function named "onmessage"
          bool has_onmessage = global->Get(context, String::NewFromUtf8(isolate, "onmessage",
                                                   NewStringType::kNormal)
                                       .ToLocalChecked()).ToLocal(&onmessage);
          if (has_onmessage && onmessage->IsFunction()) {
            Local<Function> onmessage_fun = Local<Function>::Cast(onmessage);
            Local<String> data_key = String::NewFromUtf8(isolate,
                        "data", NewStringType::kNormal).ToLocalChecked();
            // Now wait for messages
            while (true) {
              in_semaphore_.Wait();
              SerializationData* data = nullptr;
              ...
              v8::TryCatch try_catch(isolate);
              Local<Value> value;
              if (Shell::DeserializeValue(isolate, data)
                      .ToLocal(&value)) {
                ...
                if (!is_thread_worker_) {
                  ...
                  obj->Set(context, data_key, value).FromJust();
                  argv[0] = {obj};
                }
...
SerializationData* Shell::SerializeValue(
    Isolate* isolate, Local<Value> value, Local<Value> transfer) {
  if (transfer->IsUndefined()) {
    return worker_serialization_->SerializeValue(isolate, value);
  }
  Local<Array> transfer_array = Local<Array>::Cast(transfer);
  return worker_serialization_->SerializeValue(isolate, value, transfer_array);
}

MaybeLocal<Value> Shell::DeserializeValue(
    Isolate* isolate, SerializationData* data) {
  return worker_serialization_->DeserializeValue(isolate, data);
}

void Shell::HandleWorkers(Isolate* isolate, Local<Context> context) {
  if (workers_.length() == 0) return;

  Local<Object> global = context->Global();
  Local<String> onmessage_key = String::NewFromUtf8(isolate,
              "onmessage", NewStringType::kNormal).ToLocalChecked();
  Local<String> data_key = String::NewFromUtf8(isolate,
              "data", NewStringType::kNormal).ToLocalChecked();
  i::List<Worker*>* workers_available = NULL;
  std::vector<Worker*> dead_workers;
  bool done = false;
  while(!done) {
    done = true;
    int length = 0;
    {
      base::LockGuard<base::RecursiveMutex> lock_guard(&workers_mutex_);
      workers_available = isolate_related_workers_[isolate];

      if (workers_available == NULL)
          break;

      length = workers_available->length();
      for (int i = 0; i <length; i++) {
        Worker* worker = workers_available->at(i);
        if (worker->IsStopped()) {
          dead_workers.push_back(worker);
          continue;
        }
        done = false;
        SerializationData* data = worker->GetMessageOnce();
        if (data) {
          HandleScope scope(isolate);
          Local<Value> value;
          if (Shell::DeserializeValue(isolate, data).ToLocal(&value)) {
//          1. get onmessage of the worker
//          (onmessage maybe swapped during execution of onmessage)
//          2. set args
//          3. call onmessage of the worker
            Local<Object> owner_obj = worker->GetOwner(isolate);
            DCHECK(!owner_obj.IsEmpty());
            Local<Value> onmessage = owner_obj->Get(
                         context, onmessage_key).ToLocalChecked();
            if (onmessage->IsFunction()) {
              Local<Function> onmessage_fun = Local<Function>::Cast(onmessage);
              v8::TryCatch try_catch(isolate);

              Local<Value> argv[1] = {};
              if (!worker->is_thread_worker()) {
                argv[0] = value;
              } else {
                Local<Object> obj = v8::Object::New(isolate);
                obj->Set(context, data_key, value).FromJust();
      ...
      for (size_t j = 0; j < dead_workers.size(); j++) {
        Worker* worker = dead_workers[j];
        workers_available->RemoveElement(worker);
        worker->ClearOwner();
      }
      dead_workers.clear();
    }
...
void Shell::CleanupWorkers(Isolate* isolate) {
  ...
  worker_serialization_->CleanupExternalizedSharedContents();
}
...
int Shell::Main(int argc, char* argv[]) {
  ...
  create_params.array_buffer_allocator = Shell::array_buffer_allocator;
  worker_serialization_ = new WorkerSerialization(Shell::array_buffer_allocator);
#ifdef ENABLE_VTUNE_JIT_INTERFACE

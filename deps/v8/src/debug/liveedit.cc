...
void LiveEdit::FixupScript(Handle<Script> script, int max_function_literal_id) {
  ...
  PretenureFlag pretenure = script->is_shared_object() ?
                            SHARED_TENURED : TENURED;
  Handle<FixedArray> new_infos(
      isolate->factory()->NewFixedArray(max_function_literal_id + 1, pretenure));
  ...
  while (SharedFunctionInfo* shared = iterator.Next()) {
    ...
    if (info->is_shared_object()) {
      ...
      Handle<Object> new_noscript_list = WeakFixedArray::Add(
          isolate->factory()->shared_noscript_shared_function_infos(), info);
      isolate->heap()->SetRootSharedNoScriptSharedFunctionInfos(*new_noscript_list);
    } else {
      Handle<Object> new_noscript_list = WeakFixedArray::Add(
...
static Handle<Script> CreateScriptCopy(Handle<Script> original) {
  ...
  PretenureFlag pretenure = original->is_shared_object() ?
                            SHARED_TENURED : TENURED;
  Handle<FixedArray> infos(isolate->factory()->NewFixedArray(
      original->shared_function_infos()->length(), pretenure));
...
Handle<Object> LiveEdit::ChangeScriptSource(Handle<Script> original_script,
  ...
  original_script->SetSource(isolate, new_source);

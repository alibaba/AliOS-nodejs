...
void Debug::ApplyBreakPoints(Handle<DebugInfo> debug_info) {
  DisallowHeapAllocation no_gc;
  if (debug_info->BreakPoints()->IsUndefined(isolate_)) return;
...
Handle<Object> Debug::GetSourceBreakLocations(
  ...
  for (int i = 0; i < break_points->length(); ++i) {
    if (!break_points->get(i)->IsUndefined(isolate)) {
      ...
      int break_points_count = break_point_info->GetBreakPointCount();
      if (break_points_count == 0) continue;
      for (int j = 0; j < break_points_count; ++j) {
        locations->set(count++,
...
Handle<FixedArray> Debug::GetLoadedScripts() {
  ...
  if (factory->shared_script_list()->IsWeakFixedArray()) {
    Handle<WeakFixedArray> array =
        Handle<WeakFixedArray>::cast(factory->shared_script_list());
    array_length += array->Length();
  }
  if (array_length == 0)

...
// <YUNOS> changed: begin
#elif defined(V8_CAN_USE_LOGCAT)
#define LOG_TAG "v8"
#include <log/Log.h>
// <YUNOS> changed: end
#endif
...
// <YUNOS> changed: begin
// Ignored if V8_CAN_USE_LOGCAT isn't defined.
bool g_output_to_logcat = false;
// <YUNOS> changed: end
}  // namespace
...
void OS::Initialize(int64_t random_seed, bool hard_abort,
                    const char* const gc_fake_mmap, bool output_to_logcat) {
  if (random_seed) {
  ...
  // <YUNOS> changed: begin
  g_output_to_logcat  = output_to_logcat;
  // <YUNOS> changed: end
}
...
double PosixTimezoneCache::DaylightSavingsOffset(double time) {
  ...
  struct tm tm;
  // <YUNOS> changed: begin
  // we should do tzset before localtime_r, to update timezone cache
  tzset();
  // <YUNOS> changed: end
...
// <YUNOS> changed: begin
#elif defined(V8_CAN_USE_LOGCAT)
  if (g_output_to_logcat) {
    yunosLogVPrint(kLogIdMain, kLogPriorityInfo, LOG_TAG, format, args);
  } else {
    vprintf(format, args);
  }
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
#elif defined(V8_CAN_USE_LOGCAT)
  if (g_output_to_logcat && (out == stdout || out == stderr)) {
    yunosLogVPrint(kLogIdMain, kLogPriorityInfo, LOG_TAG, format, args);
  } else {
...
#elif defined(V8_CAN_USE_LOGCAT)
  if (g_output_to_logcat) {
    yunosLogVPrint(kLogIdMain, kLogPriorityError, LOG_TAG, format, args);
  } else {

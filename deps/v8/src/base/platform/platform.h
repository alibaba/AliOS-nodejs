...
namespace base {
// <YUNOS> changed: begin
// To distinguish seed-alloced heap from application-alloced heap
class BaseFlags {
 public:
  static bool GetSeedHeap() {
    return use_seed_heap_;
  }

 private:
  static void SetSeedHeap(bool flag) {
    use_seed_heap_ = flag;
  }

  static bool use_seed_heap_;
  friend class v8::internal::Heap;
  friend class v8::internal::V8;
};
// <YUNOS> changed: end
...
class V8_BASE_EXPORT OS {
 public:
  ...
  // - output_to_logcat: If true, OS::VPrint, OS::VFPrint, OS::VFPrintError
  //                     will output to logcat instead of stdout or stderr.
  static void Initialize(int64_t random_seed,
                         bool hard_abort,
                         const char* const gc_fake_mmap,
                         bool output_to_logcat);

  // get RSS of this process
  static int GetRssMemory(size_t* rss);
  // <YUNOS> changed: end
...
class V8_BASE_EXPORT VirtualMemory {
 public:
  ...
  bool Commit(void* address, size_t size, bool is_executable, const char* name = nullptr);
  // <YUNOS> changed: end
  ...
  // <YUNOS> changed: begin
  static bool CommitRegion(void* base, size_t size, bool is_executable, const char* name = nullptr);
  // <YUNOS> changed: end
...
class V8_BASE_EXPORT Thread {
 public:
  ...
  // Change to thread to unjoinable, it will free source by itself then thread exist.
  void Detach();

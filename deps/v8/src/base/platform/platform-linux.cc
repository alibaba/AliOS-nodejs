...
bool BaseFlags::use_seed_heap_ = false;
// <YUNOS> changed: end
...
void* OS::Allocate(const size_t requested, size_t* allocated,
  ...
  prctl(PR_SET_VMA,
        ...
        mbase,
        msize,
        BaseFlags::GetSeedHeap() ? "seed::Allocate" : "v8:OS::Allocate");
  // <YUNOS> changed: end
...
VirtualMemory::VirtualMemory(size_t size, size_t alignment, void* hint)
  ...
  prctl(PR_SET_VMA,
        ...
        reservation,
        request_size,
        BaseFlags::GetSeedHeap() ? "seed:VM" : "v8:VM:VM");
  // <YUNOS> changed: end
...
// <YUNOS> changed: begin
bool VirtualMemory::Commit(void* address, size_t size, bool is_executable, const char* name) {
  CHECK(InVM(address, size));
  return CommitRegion(address, size, is_executable, name);
}
// <YUNOS> changed: end
...
void* VirtualMemory::ReserveRegion(size_t size, void* hint) {
  ...
  prctl(PR_SET_VMA,
        ...
        result,
        size,
        BaseFlags::GetSeedHeap() ? "seed:ReserveRegion" : "v8:VM:ReserveRegion");
  // <YUNOS> changed: end
...
// <YUNOS> changed: begin
bool VirtualMemory::CommitRegion(void* base, size_t size, bool is_executable, const char* name) {
  ...
  prctl(PR_SET_VMA,
        ...
        base,
        size,
        name ? name : (BaseFlags::GetSeedHeap() ? "seed:CommitRegion" : "v8:VM:CommitRegion"));
  return true;
}

bool VirtualMemory::UncommitRegion(void* base, size_t size) {
  if (mmap(base, size, PROT_NONE,
    ...
    prctl(PR_SET_VMA,
          ...
          base,
          size,
          BaseFlags::GetSeedHeap() ? "seed:UncommitRegion" : "v8:VM:UncommitRegion");
    return true;
  }
  return false;
}
...
/// \brief get RSS memmory of the process
///
/// \param   size_t*  rss  the pointer of RSS to return
/// \return  int      0 if success, -1 if failed
int OS::GetRssMemory(size_t* rss) {
  // statm file format:
  // 160020 28647     17769    1    0    96650   0
  // Total  Resident  Shared   Trs  Lrs  Drs     dt
  if (NULL == rss) {
    return -1;
  }
  long rss_in_page = 0L;
  ...
  if (fscanf(fp, "%*s%ld", &rss_in_page) != 1) {
    fclose(fp);
    return -1;          /* Can't read? */
  }
  fclose(fp);
  *rss = (size_t)rss_in_page * (size_t)sysconf(_SC_PAGESIZE);
  return 0;
}

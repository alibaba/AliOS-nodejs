...
void InterpreterAssembler::UpdateInterruptBudget(Node* weight, bool backward) {
  ...
  Node* vector = LoadFeedbackVector();
  // Node* function = LoadRegister(Register::function_closure());
  // Node* cell = LoadObjectField(function, JSFunction::kFeedbackVectorOffset);
  // Node* vector = LoadObjectField(cell, Cell::kValueOffset);
  ...
  Node* old_budget = SmiToWord32(
      LoadFixedArrayElement(vector, FeedbackVector::kInterruptBudgetIndex));
  ...
  // Update budget.
  BIND(&ok);
  // CallRuntime(Runtime::kPrintInterruptBudget, GetContext(), function);
  StoreFixedArrayElement(vector,
                      FeedbackVector::kInterruptBudgetIndex,
                      SmiFromWord32(new_budget.value()));
...
Node* InterpreterAssembler::StackCheckTriggeredInterrupt() {
  ...
  Node* stack_limit = LoadRoot(Heap::kStackLimitRootIndex);
...
Node* InterpreterAssembler::LoadOSRNestingLevel() {
  Node* vector = LoadFeedbackVector();
  return SmiToWord32(
      LoadFixedArrayElement(vector, FeedbackVector::kOSRNestingLevelIndex));
}
...
void InterpreterAssembler::MaybeDropFrames(Node* context) {
  Node* restart_fp_address = LoadTL(ThreadLocalTop::kDebugRestartFpAddress);

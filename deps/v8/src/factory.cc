...
#define CALL_HEAP_FUNCTION(ISOLATE, FUNCTION_CALL, TYPE)                      \
  do {                                                                        \
    ...
    for (int __i__ = 0; __i__ < 2; __i__++) {                                 \
      if (FLAG_stress_shared_gc) {                                            \
        (ISOLATE)->heap()->CollectGarbageForShared(                           \
...
Handle<ConstantElementsPair> Factory::NewConstantElementsPair(
  ...
  Handle<ConstantElementsPair> result =
      Handle<ConstantElementsPair>::cast(NewStruct(TUPLE2_TYPE, pretenure));
...
Handle<Oddball> Factory::NewOddball(Handle<Map> map, const char* to_string,
  ...
  Handle<Oddball> oddball = New<Oddball>(map, SHARED_OLD_SPACE);
...
Handle<BoilerplateDescription> Factory::NewBoilerplateDescription(
  ...
  Handle<BoilerplateDescription> description =
      Handle<BoilerplateDescription>::cast(NewFixedArray(size, pretenure));
...
MaybeHandle<Map> Factory::InternalizedStringMapForString(
  ...
  // only shared old string can be internalized.
  if (!isolate()->heap()->InSharedOldSpace(*string)) return MaybeHandle<Map>();
...
Handle<StringClass> Factory::InternalizeExternalString(Handle<String> string) {
  ...
  Handle<StringClass> external_string = New<StringClass>(map, SHARED_OLD_SPACE);
...
MaybeHandle<String> Factory::NewExternalStringFromOneByte(
    const ExternalOneByteString::Resource* resource, bool need_shared) {
  ...
  AllocationSpace space = need_shared ? SHARED_OLD_SPACE : NEW_SPACE;
  Handle<ExternalOneByteString> external_string =
      New<ExternalOneByteString>(map, space);
...
Handle<ExternalOneByteString> Factory::NewNativeSourceString(
  ...
  Handle<ExternalOneByteString> external_string =
      New<ExternalOneByteString>(map, SHARED_OLD_SPACE);
  external_string->set_length(static_cast<int>(length));
  external_string->set_hash_field(String::kEmptyHashField);
  external_string->set_resource(resource);
...
Handle<Symbol> Factory::NewSymbol(PretenureFlag pretenure) {
  CALL_HEAP_FUNCTION(
      isolate(),
      isolate()->heap()->AllocateSymbol(pretenure),
      Symbol);
}


Handle<Symbol> Factory::NewPrivateSymbol(PretenureFlag pretenure) {
  Handle<Symbol> symbol = NewSymbol(pretenure);
...
Handle<Struct> Factory::NewStruct(InstanceType type, PretenureFlag pretenure) {
  CALL_HEAP_FUNCTION(
      isolate(),
      isolate()->heap()->AllocateStruct(type, pretenure),
      Struct);
}
...
Handle<Script> Factory::NewScript(Handle<String> source) {
  ...
  PretenureFlag pretenure = is_shared ?
                            SHARED_TENURED : TENURED;

  Handle<Script> script = Handle<Script>::cast(
      NewStruct(SCRIPT_TYPE, pretenure));
  script->set_monitor(sync::LockWord::DefaultValue());
  script->SetSource(isolate(), source);
  ...
  if (is_shared) {
    ...
    sync::MutexLock mu(isolate()->vmthread(),
          WorkerRuntime::GetLock(worker_runtime, WorkerRuntime::kScriptList));
    heap->set_shared_script_list(
        *WeakFixedArray::Add(shared_script_list(), script, NULL, true));
  } else {
...
Handle<WeakCell> Factory::NewWeakCell(Handle<HeapObject> value,
  ...
  AllowDeferredHandleDereference convert_to_cell;
  CALL_HEAP_FUNCTION(isolate(),
    isolate()->heap()->AllocateWeakCell(*value, pretenure),
    WeakCell);
}
...
Handle<Map> Factory::NewMap(InstanceType type,
  ...
  CALL_HEAP_FUNCTION(
      isolate(),
      isolate()->heap()->AllocateMap(type, instance_size, NOT_SHARED, elements_kind),
      Map);
}
...
Handle<Object> Factory::NewHeapConstantNumber(double value) {
  PretenureFlag pretenure = isolate()->vmthread()->in_shared_codegen() ?
                            SHARED_TENURED : TENURED;
  return NewNumber(value, pretenure);
}

Handle<String> Factory::NewStringConstantFromAsciiChecked(const char* str) {
  PretenureFlag pretenure = isolate()->vmthread()->in_shared_codegen() ?
      SHARED_TENURED : TENURED;
...
Handle<ScopeInfo> Factory::NewScopeInfo(int length) {
  PretenureFlag pretenure = isolate()->vmthread()->in_shared_codegen() ?
                            SHARED_TENURED : TENURED;
  Handle<FixedArray> array = NewFixedArray(length, pretenure);
...
Handle<ModuleInfo> Factory::NewModuleInfo() {
  PretenureFlag pretenure = isolate()->vmthread()->in_shared_codegen() ?
                            SHARED_TENURED : TENURED;
  Handle<FixedArray> array = NewFixedArray(ModuleInfo::kLength, pretenure);
  array->set_map_no_write_barrier(*module_info_map());
  return Handle<ModuleInfo>::cast(array);
}

Handle<PreParsedScopeData> Factory::NewPreParsedScopeData(PretenureFlag pretenure) {
  Handle<PreParsedScopeData> result =
      Handle<PreParsedScopeData>::cast(
      NewStruct(PREPARSED_SCOPE_DATA_TYPE, pretenure));
...
Handle<Code> Factory::NewCodeRaw(int object_size, bool immovable, bool is_shared) {
  CALL_HEAP_FUNCTION(isolate(),
...
Handle<Code> Factory::NewCode(const CodeDesc& desc,
                              ...
                              bool is_debug) {
  bool is_shared = isolate()->vmthread()->in_shared_codegen();
  Handle<ByteArray> reloc_info = NewByteArray(desc.reloc_size,
                      is_shared ? SHARED_TENURED : TENURED);
  ...
  Handle<Code> code = NewCodeRaw(obj_size, immovable, is_shared);
...
// <YUNOS> START
Handle<JSArrayBuffer> Factory::NewJSArrayBufferex(PretenureFlag pretenure) {
  Handle<JSFunction> array_buffer_fun(
      isolate()->native_context()->shared_array_buffer_ex_fun());
...
Handle<SharedFunctionInfo> Factory::NewSharedFunctionInfo(
  ...
  if (has_shared_name) {
    ...
    if (is_shared)
      shared_name = InternalizeString(shared_name);
  }

  AllocationSpace space = is_shared ? SHARED_OLD_SPACE : OLD_SPACE;
  Handle<Map> map = shared_function_info_map();
  Handle<SharedFunctionInfo> share = New<SharedFunctionInfo>(map, space);

  share->set_monitor(sync::LockWord::DefaultValue());
  // Set pointer fields.
  ...
  share->set_ic_age_and_bailout_reason(0);
  share->set_kind(kind);
  ...
  if (is_shared) {
    ...
    sync::SafepointMutexLock mu(isolate()->vmthread(),
          WorkerRuntime::GetLock(worker_runtime, WorkerRuntime::kNoScriptSharedFunctionInfos));
    Handle<Object> new_noscript_list =
        WeakFixedArray::Add(shared_noscript_shared_function_infos(), share, NULL, true);
    isolate()->heap()->set_shared_noscript_shared_function_infos(*new_noscript_list);
  } else {
    Handle<Object> new_noscript_list =
...
Handle<String> Factory::NumberToString(Handle<Object> number,
                                       ...
                                       bool need_shared_string) {
  isolate()->counters()->number_to_string_runtime()->Increment();
  if (check_number_string_cache) {
    Handle<Object> cached = GetNumberStringCache(number);
    if (!cached->IsUndefined(isolate()) && (!need_shared_string ||
      Handle<HeapObject>::cast(cached)->is_shared_object())) {
  ...
  PretenureFlag pretenure = need_shared_string ? SHARED_TENURED : TENURED;
  Handle<String> js_string = NewStringFromAsciiChecked(str, pretenure);
...
Handle<DebugInfo> Factory::NewDebugInfo(Handle<SharedFunctionInfo> shared) {
  ...
  PretenureFlag pretenure = shared->is_shared_object() ? SHARED_TENURED : TENURED;
  Handle<DebugInfo> debug_info =
      Handle<DebugInfo>::cast(NewStruct(DEBUG_INFO_TYPE, pretenure));
...
Handle<CoverageInfo> Factory::NewCoverageInfo(
  ...
  Handle<CoverageInfo> info =
      Handle<CoverageInfo>::cast(NewFixedArray(length, pretenure));
...
Handle<BreakPointInfo> Factory::NewBreakPointInfo(int source_position,
  ...
  Handle<BreakPointInfo> new_break_point_info =
      Handle<BreakPointInfo>::cast(NewStruct(TUPLE2_TYPE, pretenure));

...
CompilationCache::CompilationCache(Isolate* isolate)
    : isolate_(isolate),
      script_(isolate),
      shared_script_(nullptr),
      eval_global_(isolate),
      eval_contextual_(isolate),
      reg_exp_(isolate, kRegExpGenerations),
      mutex_("compilation cache lock"),
      enabled_(true) {
  if (isolate->is_main_isolate()) {
    shared_script_ = new CompilationCacheScript(isolate);
  }
  CompilationSubCache* subcaches[kSubCacheCount] =
    {&script_, shared_script_, &eval_global_, &eval_contextual_, &reg_exp_};
...
Isolate* CompilationSubCache::isolate() {
  return is_shared() ? isolate_->GetCurrentIsolate() : isolate_;
}
...
Handle<CompilationCacheTable> CompilationSubCache::GetTable(int generation) {
  ...
  if (tables_[generation]->IsUndefined(isolate())) {
    ...
    result = CompilationCacheTable::New(isolate(), kInitialCacheSize, pretenure);
...
bool CompilationSubCache::is_shared() {
  return isolate_->compilation_cache()->shared_script_ == this;
}
...
void CompilationCache::Remove(Handle<SharedFunctionInfo> function_info) {
  ...
  sync::SafepointMutexLock mu(isolate_->vmthread(), mutex_);
  shared_script_->Remove(function_info);
}
...
InfoVectorPair CompilationCache::LookupSharedScript(
    Handle<String> source, Handle<Object> name, int line_offset,
    int column_offset, ScriptOriginOptions resource_options,
    Handle<Context> context, LanguageMode language_mode) {
  InfoVectorPair empty_result;
  ...
  sync::SafepointMutexLock mu(isolate_->vmthread(), mutex_);
  return shared_script_->Lookup(source, name, line_offset, column_offset,
                        resource_options, context, language_mode);
}
...
void CompilationCache::PutSharedScript(Handle<String> source,
                                 Handle<Object> name, int line_offset,
                                 int column_offset,
                                 ScriptOriginOptions resource_options,
  ...
  sync::SafepointMutexLock mu(isolate_->vmthread(), mutex_);
  InfoVectorPair result = shared_script_->Lookup(source, name, line_offset,
                          column_offset, resource_options, context, language_mode);
  if (!result.has_shared()) {
    shared_script_->Put(source, context, language_mode, function_info, literals);
  }
}
...
void CompilationCache::Clear(bool skip_shared) {
  for (int i = 0; i < kSubCacheCount; i++) {
    //JSMT:shared_script_ should not clear here.
    if (skip_shared &&
        i == kSharedCacheIdx) continue;
    subcaches_[i]->Clear();
  }
}

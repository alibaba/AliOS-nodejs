...
// <YUNOS> changed: begin
// For some FLAG, one isolate is doing GC, while other isolate change it would
// cause crash(because of FLAG check). So, we make sure every isolate has it and
// one isolate can only change FLAG for itself. See more detail in Aone#10985675.
// TODO: Save flags to thread local or make sure set flag after isolate initialized,
// because of multi-thread flags issue.
static bool SetFlagToIsolate(Flag* flag, bool is_bool) {
#if DEBUG
  if (!Isolate::KeyCreated()) return false;
#endif
  if (Isolate::Current() != NULL) {
    if ((flag->type() == Flag::TYPE_BOOL) &&
        (flag->bool_variable() == &FLAG_optimize_for_size)) {
      Isolate::Current()->flags().set_optimize_for_size(!is_bool);
      return true;
    }
  }
  return false;

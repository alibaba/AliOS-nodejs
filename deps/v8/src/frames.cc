...
Code* InnerPointerToCodeCache::GcSafeFindCodeForInnerPointer(
  ...
  OldSpace* old_space = nullptr;
  if (heap->code_space()->Contains(inner_pointer)) {
    old_space = heap->code_space();
  } else if (heap->shared_code_space()->Contains(inner_pointer)) {
    old_space = heap->shared_code_space();
  } else {
    return nullptr;
  }

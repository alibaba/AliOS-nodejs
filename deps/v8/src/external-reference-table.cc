...
ExternalReferenceTable* ExternalReferenceTable::instance(Isolate* isolate) {
  if (isolate->is_worker_isolate()) {
    isolate = isolate->worker_runtime()->main_isolate();
    CHECK(isolate->external_reference_table() != NULL);
  }
...
void ExternalReferenceTable::AddReferences(Isolate* isolate) {
  // Miscellaneous
  Add(ExternalReference::current_thread_local_top(isolate).address(),
      "Isolate::current_thread_local_top()");
  Add(ExternalReference::main_thread_local_top(isolate).address(),
      "Isolate::main_thread_local_top()");

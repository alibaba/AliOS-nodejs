...
#ifdef V8_YUNOS_LOGGER
v8::Extension* Bootstrapper::yunos_logger_extension_ = NULL;
#endif // V8_YUNOS_LOGGER
...
#ifdef V8_YUNOS_LOGGER
  yunos_logger_extension_ = new YunOSLoggerExtension;
  v8::RegisterExtension(yunos_logger_extension_);
#endif // V8_YUNOS_LOGGER
}
...
#ifdef V8_YUNOS_LOGGER
  delete yunos_logger_extension_;
  yunos_logger_extension_ = NULL;
#endif // V8_YUNOS_LOGGER
}
...
Handle<JSFunction> Genesis::CreateEmptyFunction(Isolate* isolate) {
  ...
  Handle<JSFunction> empty_function;
  if (isolate->is_main_isolate()) {
    Handle<Code> code(isolate->builtins()->EmptyFunction());
    empty_function = factory->NewFunction(empty_function_map,
                                          factory->empty_string(), code);
    // JSMT: TODO use bootstrpper roots record empty SFI ?
    isolate->worker_runtime()->set_empty_shared_function_info(
                                                empty_function->shared());
  } else {
    WorkerRuntime* runtime = isolate->worker_runtime();
    Handle<SharedFunctionInfo> empty_info = Handle<SharedFunctionInfo>::cast(
                                              runtime->EmptySharedFunctionInfo());
    DCHECK(empty_info->is_shared_object());
    Handle<Context> context(isolate->native_context());
    empty_function = factory->NewFunction(empty_function_map, empty_info, context);
  }
  empty_function->shared()->set_language_mode(STRICT);

  // --- E m p t y ---
  Handle<String> source = factory->NewStringFromStaticChars("() {}", SHARED_TENURED);
  Handle<Script> script = factory->NewScript(source);
  script->set_type(Script::TYPE_NATIVE);
  Handle<FixedArray> infos = factory->NewFixedArray(2, SHARED_TENURED);
...
#undef INSTALL_CAPTURE_GETTER
  ...
  {  // -- S h a r e d A r r a y B u f f e r E x
    Handle<String> name = factory->InternalizeUtf8String("SharedArrayBufferEx");
    Handle<JSFunction> shared_array_buffer_ex_fun = CreateArrayBuffer(
        name, Builtins::kSharedArrayBufferPrototypeGetByteLength,
        BuiltinFunctionId::kSharedArrayBufferByteLength,
        Builtins::kSharedArrayBufferPrototypeSlice, true);
    InstallWithIntrinsicDefaultProto(isolate, shared_array_buffer_ex_fun,
                                     Context::SHARED_ARRAY_BUFFER_EX_FUN_INDEX);
    InstallSpeciesGetter(shared_array_buffer_ex_fun);
  }
...
bool Bootstrapper::CompileNative(Isolate* isolate, Vector<const char> name,
  ...
  Handle<String> script_name =
      isolate->factory()->NewStringFromUtf8(name, SHARED_TENURED).ToHandleChecked();
...
/* YUNOS BEGIN */
void Genesis::InitializeGlobal_harmony_sharedarraybufferex() {
  if (!FLAG_harmony_sharedarraybufferex) return;
  ...
  {
    Handle<String> name = factory->InternalizeUtf8String("SharedArrayBufferEx");
    JSObject::AddProperty(global, name, isolate->shared_array_buffer_ex_fun(),
                          DONT_ENUM);
  }
...
#ifdef DEBUG
    Maybe<bool> maybe = JSReceiver::HasOwnProperty(global, name);
    if (maybe.IsJust() && maybe.FromJust()) {
      DCHECK_EQ(*JSReceiver::GetDataProperty(global, name), *isolate->atomics_object());
      return;
    }
#endif
...
Handle<JSFunction> Genesis::CreateArrayBuffer(Handle<String> name,
  ...
  array_buffer_fun->shared()->SetConstructStub(
      is_array_buffer_ex ?
      *isolate()->builtins()->ArrayBufferExConstructor_ConstructStub() :
...
#ifdef V8_YUNOS_LOGGER
        (!FLAG_expose_logger ||
         InstallExtension(isolate, "v8/YunOSLogger", &extension_states)) &&
#endif // V8_YUNOS_LOGGER

...
GCTracer::Event::Event(Type type, GarbageCollectionReason gc_reason,
    ...
    : type(type),
      ...
      start_memory_size(0),
      end_memory_size(0),
      start_shared_object_size(0),
      end_shared_object_size(0),
      start_holes_size(0),
      end_holes_size(0),
...
const char* GCTracer::Event::TypeName(bool short_name) const {
  switch (type) {
    ...
    case MARK_COMPACTOR_SHARED:
      return (short_name) ? "mcs" : "Mark-Compact for Shared";
    case START:
...
GCTracer::GCTracer(Heap* heap)
    : heap_(heap),
      ...
      shared_space_allocation_counter_bytes_(0),
      allocation_duration_since_gc_(0.0),
      new_space_allocation_in_bytes_since_gc_(0),
      old_generation_allocation_in_bytes_since_gc_(0),
      shared_space_allocation_in_bytes_since_gc_(0),
...
void GCTracer::ResetForTesting() {
  ...
  shared_space_allocation_in_bytes_since_gc_ = 0.0;
  shared_space_allocation_counter_bytes_ = 0.0;
  combined_mark_compact_speed_cache_ = 0.0;
  ...
  recorded_shared_mark_compacts_.Reset();
  ...
  recorded_shared_space_allocations_.Reset();
...
void GCTracer::Start(GarbageCollector collector,
  ...
  size_t shared_space_counter_bytes = 0;
  if (this == worker_runtime->tracer()) {
    shared_space_counter_bytes = worker_runtime->SharedSpaceAllocationCounter();
  }
  SampleAllocation(start_time, heap_->NewSpaceAllocationCounter(),
                   heap_->OldGenerationAllocationCounter(),
                   shared_space_counter_bytes);
  ...
  switch (collector) {
    ...
    case MARK_COMPACTOR:
      ...
      break;
    case MARK_COMPACTOR_SHARED:
      current_ = Event(Event::MARK_COMPACTOR_SHARED, gc_reason, collector_reason);
      break;
  }
  ...
  current_.start_shared_object_size = heap_->SizeOfSharedObjects();
...
void GCTracer::Stop(GarbageCollector collector) {
  ...
  DCHECK((collector == SCAVENGER && current_.type == Event::SCAVENGER) ||
         ...
         (collector == MARK_COMPACTOR_SHARED &&
          current_.type == Event::MARK_COMPACTOR_SHARED));


  current_.end_time = heap_->MonotonicallyIncreasingTimeInMs();
  current_.end_object_size = heap_->SizeOfObjects();
  current_.end_shared_object_size = heap_->SizeOfSharedObjects();
  ...
  if (collector == MARK_COMPACTOR_SHARED) {
    recorded_shared_mark_compacts_.Push(
      MakeBytesAndDuration(current_.start_shared_object_size, duration));
    heap_->isolate()->worker_runtime()->
      tracer()->AddAllocation(current_.end_time);
    heap_->isolate()->worker_runtime()->
      UpdateTotalGCTime(duration);
  } else {
    ...
    switch (current_.type) {
      ...
      case Event::MARK_COMPACTOR_SHARED:
...
void GCTracer::SampleAllocation(double current_ms,
                                ...
                                size_t shared_space_counter_bytes) {
  if (allocation_time_ms_ == 0) {
    ...
    shared_space_allocation_counter_bytes_ = shared_space_counter_bytes;
    return;
  }
  ...
  size_t shared_space_allocated_bytes =
      shared_space_counter_bytes - shared_space_allocation_counter_bytes_;
  ...
  shared_space_allocation_in_bytes_since_gc_ +=
    shared_space_counter_bytes - shared_space_allocated_bytes;
}
  ...
  if (allocation_duration_since_gc_ > 0) {
    ...
    recorded_old_generation_allocations_.Push(
        MakeBytesAndDuration(old_generation_allocation_in_bytes_since_gc_,
                             allocation_duration_since_gc_));
    recorded_shared_space_allocations_.Push(
        MakeBytesAndDuration(shared_space_allocation_in_bytes_since_gc_,
                             allocation_duration_since_gc_));
  }
...
void GCTracer::Print() const {
  ...
  Output(
      ...
      static_cast<double>(current_.start_shared_object_size) / MB,
      static_cast<double>(current_.end_shared_object_size) / MB,
      current_.scopes[Scope::MCS_PAUSE],
      duration,
  ...
  switch (current_.type) {
    ...
    case Event::INCREMENTAL_MARK_COMPACTOR:
      heap_->isolate()->PrintWithTimestamp(
          ...
          // <YUNOS> changed: begin
          current_.scopes[Scope::MC_SWEEP_MUTABLE],
          // <YUNOS> changed: end
          current_.scopes[Scope::MC_INCREMENTAL],
...
double GCTracer::CombinedMarkCompactSharedSpeedInBytesPerMillisecond() {
  return AverageSpeed(recorded_shared_mark_compacts_);
}
...
double GCTracer::SharedSpaceAllocationThroughputInBytesPerMillisecond(
    double time_ms) const {
  size_t bytes = shared_space_allocation_in_bytes_since_gc_;
  double durations = allocation_duration_since_gc_;
  return AverageSpeed(recorded_shared_space_allocations_,
                      MakeBytesAndDuration(bytes, durations), time_ms);
}
...
double GCTracer::CurrentSharedSpaceAllocationThroughputInBytesPerMillisecond() {
  return SharedSpaceAllocationThroughputInBytesPerMillisecond(
      kThroughputTimeFrameMs);
}

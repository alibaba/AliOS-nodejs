...
AllocationResult Heap::AllocateRaw(int size_in_bytes, AllocationSpace space,
...
#ifdef DEBUG
  if (FLAG_verify_thread_suspension)
    isolate()->vmthread()->AssertMutatorRunnable();
  ...
  switch (space) {
    ...
    case MUTABLE_SPACE: {
      allocation = mutable_space_->AllocateRaw(size_in_bytes, alignment);
      break;
    }
    case SHARED_OLD_SPACE: {
      if (large_object) {
        allocation = shared_lo_space_->AllocateRaw(size_in_bytes, NOT_EXECUTABLE);
      } else {
        allocation = shared_old_space_->AllocateRawSynchronized(
    ...
    case SHARED_CODE_SPACE: {
      if (size_in_bytes <= shared_code_space()->AreaSize()) {
        allocation = shared_code_space_->AllocateRawUnalignedSynchronized(
                        size_in_bytes);
      } else {
        allocation = shared_lo_space_->AllocateRaw(size_in_bytes, EXECUTABLE);
      }
      break;
    }
    case LO_SPACE:
    ...
    case SHARED_MAP_SPACE:
      allocation = shared_map_space_->AllocateRawUnalignedSynchronized(size_in_bytes);
      break;
    default:
      UNREACHABLE();
  }
...
void Heap::RegisterExternalString(String* string) {
  if (string->is_shared_object()) {
    isolate()->worker_runtime()->RegisterExternalString(string);
  } else {
...
bool Heap::InSharedSpace(Object* object) {
  bool result =
      object->IsHeapObject() &&
      Page::FromAddress(HeapObject::cast(object)->address())->InSharedSpace();
  return result;
}
...
bool Heap::InSharedOldSpace(Object* object) {
  return shared_old_space_->Contains(object);
}

// <YUNOS> changed: begin
bool Heap::InMutableSpace(Address address) { return mutable_space_->Contains(address); }

bool Heap::InMutableSpace(Object* object) {
  return InMutableSpace(reinterpret_cast<Address>(object));
}
// <YUNOS> changed: end
...
bool Heap::ShouldProcessShared() {
  // return true if
  // 1. during collect shared space
  // 2. heap is the gc thread's heap
  return collect_shared_space() &&
            !isolate()->IsProxyIsolate();
}

StringTable* Heap::string_table() {
  return isolate()->worker_runtime()->StringTableRoot();
}

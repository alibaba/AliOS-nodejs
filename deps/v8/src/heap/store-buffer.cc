...
StoreBuffer::StoreBuffer(Heap* heap)
  ...
  heap_->store_buffer_top_ = nullptr;
  task_running_ = false;
...
void StoreBuffer::SetUp() {
  ...
  if (!virtual_memory_->Commit(reinterpret_cast<Address>(start_[0]),
                               ...
                               false, "StoreBuffer:SetUp")) {  // Not executable.
    V8::FatalProcessOutOfMemory("StoreBuffer::SetUp");
  }
  // <YUNOS> changed: end
  current_ = 0;
  heap_->store_buffer_top_ = start_[current_];
}
...
void StoreBuffer::FlipStoreBuffers() {
  ...
  lazy_top_[current_] = heap_->store_buffer_top_;
  current_ = other;
  heap_->store_buffer_top_ = start_[current_];

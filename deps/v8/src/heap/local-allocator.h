...
class LocalAllocator {
 public:
  ...
  void Finalize() {
    ...
    heap_->shared_old_space()->MergeCompactionSpace(
        compaction_spaces_.Get(SHARED_OLD_SPACE));
    heap_->shared_code_space()->MergeCompactionSpace(
        compaction_spaces_.Get(SHARED_CODE_SPACE));
  ...
  AllocationResult Allocate(AllocationSpace space, int object_size,
    ...
    switch (space) {
      ...
      case SHARED_OLD_SPACE:
        return compaction_spaces_.Get(SHARED_OLD_SPACE)
            ->AllocateRaw(object_size, alignment);

      case SHARED_CODE_SPACE:
        return compaction_spaces_.Get(SHARED_CODE_SPACE)
            ->AllocateRaw(object_size, alignment);

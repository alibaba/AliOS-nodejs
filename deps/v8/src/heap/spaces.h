...
class MemoryChunk {
 public:
  ...
  enum Flag {
    ...
    // <YUNOS> changed:
    // whether this page is seed-alloced or application-alloced
    IN_SEED_HEAP = 1u << 19,

    // [AOT_COMPILED]: The flag is set if the page is ahead-of-time compiled.
    AOT_COMPILED = 1u << 20,

    // |IN_SHARED_SPACE|: Flag is set if page is in shared space.
    IN_SHARED_SPACE = 1u << 21,
  };
  ...
  // <YUNOS> changed: begin
  // Print all infomation in this chunk.
  void PrintChunkInfo();

  // Print all page infomation in this paged space.
  static void PrintChunkInfos(Heap* heap);
  // <YUNOS> changed: end
  ...
  inline Heap* heap() {
    //TODO(JSMT): remove this check after release
    DCHECK(!InSharedSpace() ||
        !heap_->collect_shared_space());
    return heap_;
  }

  inline Heap* heap_safe() {
    return heap_;
  }
  ...
  bool InSharedSpace() { return IsFlagSet(IN_SHARED_SPACE); }
...
class Page : public MemoryChunk {
 public:
  ...
  V8_EXPORT_PRIVATE void MarkBlackArea(Address start, Address end);
...
class Space : public Malloced {
 public:
  ...
  Heap* heap_safe() const {
    return heap_;
  }

  inline Heap* GetProxyHeap();
  ...
  // <YUNOS> changed: begin
  const char* GetShowmapName() {
    if (FLAG_enable_seed_heap) {
      switch(id_) {
        case NEW_SPACE: return "seed:new";
        case OLD_SPACE: return "seed:old";
        case MUTABLE_SPACE: return "seed:mutable";
        case CODE_SPACE: return "seed:code";
        case MAP_SPACE: return "seed:map";
        case LO_SPACE: return "seed:lo";
        case SHARED_OLD_SPACE: return "seed:shared_old";
        case SHARED_CODE_SPACE: return "seed:shared_code_space";
        case SHARED_MAP_SPACE: return "seed:shared_map_space";
        case SHARED_LO_SPACE: return "seed:shraed_lo_space";
        default:
          UNREACHABLE();
      }
    } else {
      switch(id_) {
        case NEW_SPACE: return "v8:new";
        case OLD_SPACE: return "v8:old";
        case MUTABLE_SPACE: return "v8:mutable";
        case CODE_SPACE: return "v8:code";
        case MAP_SPACE: return "v8:map";
        case LO_SPACE: return "v8:lo";
        case SHARED_OLD_SPACE: return "v8:shared_old";
        case SHARED_CODE_SPACE: return "v8:shared_code_space";
        case SHARED_MAP_SPACE: return "v8:shared_map_space";
        case SHARED_LO_SPACE: return "v8:shraed_lo_space";
        default:
          UNREACHABLE();
      }
    }
  ...
  // <YUNOS> changed: end
  bool sharable() { return sharable_ == SHARED; }

  void mark_shared() { sharable_ = SHARED; }
  ...
  virtual int RoundSizeDownToObjectAlignment(int size) {
    if (id_ == CODE_SPACE || id_ == SHARED_CODE_SPACE) {
...
class V8_EXPORT_PRIVATE MemoryAllocator {
 public:
  ...
  static size_t PageAreaSize(AllocationSpace space) {
    ...
    return (space == CODE_SPACE || space == SHARED_CODE_SPACE)
                                 ? CodePageAreaSize()
  ...
  void Free(MemoryChunk* chunk, Heap* heap);
  ...
  // Update the max capacity of the total space if the large heap enable
  void UpdateMaxCapacity(intptr_t max_capacity);
  // <YUNOS> changed: end
...
class Unmapper {
  public:
    ...
    explicit Unmapper(MemoryAllocator* allocator, Heap* heap)
...
class V8_EXPORT_PRIVATE HeapObjectIterator : public ObjectIterator {
 public:
  ...
  explicit HeapObjectIterator(PagedSpace* space, Heap* heap);
  explicit HeapObjectIterator(Page* page, Heap* heap);
...
class V8_EXPORT_PRIVATE PagedSpace : NON_EXPORTED_BASE(public Space) {
 public:
  ...
  void PrepareForMarkCompact(Heap* heap);

  // <YUNOS> changed: begin
  // set flag for seed-alloced heap in a space, also free top object and reset
  // free list
  void MarkPagesFromSeedAndDisposeFreeSpace();

  // Change all codes to young age
  void MakeAllCodesYoung();
  // <YUNOS> changed: end
  ...
  virtual void Verify(ObjectVisitor* visitor, Heap* heap);
  ...
  // sweeper.
  virtual void RefillFreeList(Heap* heap = NULL);

  FreeList* free_list() { return &free_list_; }

  base::RecursiveMutex* mutex() { return &space_mutex_; }
 ...
 protected:
  ...
  AllocationInfo* allocation_info_;
  AllocationInfo original_allocation_info_;

  // Mutex guarding any concurrent access to the space.
  base::RecursiveMutex space_mutex_;
...
class NewSpace : public Space {
 public:
  ...
  void set_allocation_info(AllocationInfo* allocation_info) {
...
class CompactionSpaceCollection : public Malloced {
 public:
  explicit CompactionSpaceCollection(Heap* heap)
      : old_space_(heap, OLD_SPACE, Executability::NOT_EXECUTABLE),
        // <YUNOS> changed: begin
        mutable_space_(heap, MUTABLE_SPACE, Executability::NOT_EXECUTABLE),
        // <YUNOS> changed: end
        code_space_(heap, CODE_SPACE, Executability::EXECUTABLE),
        shared_old_space_(heap, SHARED_OLD_SPACE, Executability::NOT_EXECUTABLE),
        shared_code_space_(heap, SHARED_CODE_SPACE, Executability::EXECUTABLE){
  ...
  CompactionSpace* Get(AllocationSpace space) {
    switch (space) {
      ...
      // <YUNOS> changed: begin
      case MUTABLE_SPACE:
        return &mutable_space_;
      // <YUNOS> changed: end
      case CODE_SPACE:
        return &code_space_;
      case SHARED_OLD_SPACE:
        return &shared_old_space_;
      case SHARED_CODE_SPACE:
        return &shared_code_space_;
      default:
        UNREACHABLE();
    }
    UNREACHABLE();
 ...
 private:
  ...
  // <YUNOS> changed: begin
  CompactionSpace mutable_space_;
  // <YUNOS> changed: end
  CompactionSpace code_space_;
  CompactionSpace shared_old_space_;
  CompactionSpace shared_code_space_;
};
...
// <YUNOS> changed: begin
// -----------------------------------------------------------------------------
// Mutable object space (includes WeakCell)

class MutableSpace : public PagedSpace {
 public:
  // Creates an mutable space object. The constructor does not allocate pages
  // from OS.
  MutableSpace(Heap* heap, AllocationSpace id, Executability executable)
      : PagedSpace(heap, id, executable) {}
...
class MemoryChunkIterator BASE_EMBEDDED {
 ...
 private:
  ...
  // <YUNOS> changed: begin
  PageIterator mutable_iterator_;
  // <YUNOS> changed: end
  PageIterator code_iterator_;
  PageIterator map_iterator_;
  PageIterator shared_old_iterator_;
  PageIterator shared_code_iterator_;
  PageIterator shared_map_iterator_;
  LargePageIterator lo_iterator_;
  LargePageIterator shared_lo_iterator_;
};

...
#include "src/heap/allocation-info.h"
...
#define STRONG_ROOT_LIST(V)                                                    \
  ...
  V(Object, shared_script_list, SharedScriptList)                              \
  ...
  V(Object, shared_noscript_shared_function_infos,                             \
      SharedNoScriptSharedFunctionInfos)                                       \
  V(Object, weak_wasm_data_table, WeakWasmDataTable)                           \
  V(Object, weak_debug_points_table, WeakDebugPointsTable)                     \
...
#undef ROOT_INDEX_DECLARATION
        kRootListLength,
    kStrongRootListLength = kStackLimitRootIndex,
    kSmiRootsStart = kStackLimitRootIndex,
    kRecordWriteStubModeRootIndex = -7,
    kNewSpaceAllocationTopRootIndex = -6,
    kNewSpaceAllocationLimitRootIndex = -5,
    kOldSpaceAllocationTopRootIndex = -4,
    kOldSpaceAllocationLimitRootIndex = -3,
    kStoreBufferTopRootIndex = -2,
    kAllocationSitesListRootIndex = -1,
  };
  ...
  static const int kInitalSharedSpacesLimitFactor = 2;
  ...
  static const int kMinSharedSpacesSize = 128 * kPointerMultiplier;
  static const int kMaxSharedSpacesSize = 1024 * kPointerMultiplier;

  // The max old space size(if large heap enable) has to be a multiple
  // of Page::kPageSize.
  // Sizes are in MB.
  static const int kMaxOldSpaceCapacityLowMemoryDevice = 256 * kPointerMultiplier;
  static const int kMaxOldSpaceCapacityMediumMemoryDevice =
      384 * kPointerMultiplier;
  static const int kMaxOldSpaceCapacityHighMemoryDevice = 640 * kPointerMultiplier;
  static const int kMaxOldSpaceCapacityHugeMemoryDevice = 828 * kPointerMultiplier;

  static const int kTraceRingBufferSize = 512;
  static const int kStacktraceBufferSize = 512;

  V8_EXPORT_PRIVATE static double kMinHeapGrowingFactor;
  V8_EXPORT_PRIVATE static double kMaxHeapGrowingFactor;
  static double kMaxHeapGrowingFactorMemoryConstrained;
  ...
  static const int kThresholdForFullGC = 8;
  static const int kThresholdForScavengerGC = 2;
  ...
  static bool RootIsReference(RootListIndex root_index);
  ...
  V8_EXPORT_PRIVATE static double MaxSharedHeapGrowingFactor(
      size_t max_shared_space_size);
  ...
  bool ShouldDoScavengerGC();

  void AdviceGC();

  enum WorkStatus { STATUS_FOREGROUND, STATUS_BACKGROUND };
  // switch work status, the parameter type should be one of follow:
  // STATUS_FOREGROUND: switch to foreground
  // STATUS_BACK      : switch to background
  bool SwitchWorkStatus(WorkStatus type);
  ...
  bool ConfigureWorkerHeap();

  bool ConfigureHeapDefault();
  // <YUNOS> changed: begin
  // Do a alway-compact gc and set flag for seed-alloced heap
  void MarkSeedHeapPreFork();
  // Print all chunks's information in v8 heap spaces.
  // TODO(moyu.ycj): support new space.
  void PrintChunkInfoInSpaces();
  // After MarkSeedHeapPreFork, Object that in seed heap return true.
  static bool InSeedHeap(Address obj);
  ...
  bool CreateWorkerHeapObjects(Isolate* main_isolate);
  ...
  // <YUNOS> changed: begin
  MutableSpace* mutable_space() { return mutable_space_; }
  // <YUNOS> changed: end
  OldSpace* code_space() { return code_space_; }
  MapSpace* map_space() { return map_space_; }
  OldSpace* shared_old_space() { return shared_old_space_; }
  OldSpace* shared_code_space() { return shared_code_space_; }
  MapSpace* shared_map_space() { return shared_map_space_; }
  LargeObjectSpace* shared_lo_space() { return shared_lo_space_; }
  ...
  bool collect_shared_space();
  inline bool ShouldProcessShared();
...
#undef SYMBOL_ACCESSOR
  ...
  void SetRootSharedScriptList(Object* value) {
    roots_[kSharedScriptListRootIndex] = value;
  }

  void set_shared_code_stubs_cache(CodeHashMap* cache) {
    shared_code_stubs_cache_ = cache;
  }

  CodeHashMap* shared_code_stubs_cache() {
    return shared_code_stubs_cache_;
  }

  int record_write_stub_mode() const {
    return static_cast<int>(record_write_stub_mode_);
  }
  void set_record_write_stub_mode(int mode) {
    record_write_stub_mode_ = mode;
  }

  V8_INLINE StringTable* string_table();

  void SetRootNoScriptSharedFunctionInfos(Object* value) {
    roots_[kNoScriptSharedFunctionInfosRootIndex] = value;
  }

  void SetRootSharedNoScriptSharedFunctionInfos(Object* value) {
    roots_[kSharedNoScriptSharedFunctionInfosRootIndex] = value;
  }

  void SetRootWeakWasmDataTable(Object* value) {
    roots_[kWeakWasmDataTableRootIndex] = value;
  }

  void SetRootWeakDebugPointsTable(Object* value) {
    roots_[kWeakDebugPointsTableRootIndex] = value;
  }
  ...
  void DoGcPrologueCallbacks(GCType type);

  void DoGcEpilogueCallbacks(GCType type, GCCallbackFlags gc_callback_flags );

  // Performs garbage collection operation for all spaces(include shared space).
  void CollectGarbageForShared(GarbageCollectionReason gc_reason,
                               const char* collector_reason,
                               const v8::GCCallbackFlags gc_callback_flags = kNoGCCallbackFlags);
  ...
  inline bool InSharedSpace(Object* object);
  inline bool InFromSpace(Object* object);
  inline bool InToSpace(Object* object);

  // Returns whether the object resides in old space.
  inline bool InOldSpace(Object* object);
  inline bool InSharedOldSpace(Object* object);

  // <YUNOS> changed: begin
  // Returns whether the object resides in mutable space.
  inline bool InMutableSpace(Address address);
  inline bool InMutableSpace(Object* object);
  // <YUNOS> changed: end

  // Checks whether an address/object in the heap (including auxiliary
  // area and unused area).
  bool Contains(HeapObject* value);
  bool SharedContains(HeapObject* value);
  ...
  bool SharedContainsSlow(Address addr);
  ...
  size_t MaxSharedSpaceSize() { return max_shared_spaces_size_; }
  ...
  // Returns the shared space size
  size_t SharedSpaceCapacity();

  // Returns the capacity of the old generation.
  size_t OldGenerationCapacity();

  // Returns the amount of memory currently committed for the shared space
  size_t CommittedSharedMemory();
  ...
  // Returns of size of shared objects residing in the heap.
  size_t SizeOfSharedObjects();
  ...
  // Returns the size of objects residing in shared spaces.
  size_t SharedSpaceSizeOfObjects();
...
#undef ROOT_ACCESSOR
  ...
  void UpdateSharedSpaceAllocationLimit(size_t value = 0);

  void CollectSharedPrologueStep1(GarbageCollectionReason gc_reason,
                                  const char* collector_reason);
  void CollectSharedPrologueStep2(int& start_new_space_size,
                                  uint64_t& size_of_objects_before_gc,
                                  bool need_verify);
  void CollectSharedEpilogueStep1(const v8::GCCallbackFlags gc_callback_flags,
                                  int start_new_space_size,
                                  uint64_t size_of_objects_before_gc);
  void CollectSharedEpilogueStep2();

  void EpilogueForShared();
  ...
  void CreateInitialObjects(bool is_worker);
  ...
  void UpdateReferencesInSharedExternalStringTable(
      ExternalStringTableUpdaterCallback updater_func);
  ...
  inline size_t SharedSpaceAvailable() {
    if (shared_spaces_allocation_limit_ <= SharedSpaceSizeOfObjects()) return 0;
    return shared_spaces_allocation_limit_ -
           static_cast<size_t>(SharedSpaceSizeOfObjects());
  }
  ...
  // Calculates the shared space allocation limit based on a given growing factor and a
  // given shared generation size.
  size_t CalculateSharedSpaceAllocationLimit(double factor,
                                             size_t shared_gen_size);

  // Sets the allocation limit to trigger the next full garbage collection.
  void SetGenerationAllocationLimit(size_t old_gen_size,
                                    double gc_speed,
                                    double mutator_speed,
                                    Sharability shared = NOT_SHARED);
  ...
  inline void set_shared_spaces_allocation_limit(size_t value) {
    shared_spaces_allocation_limit_ = value;
  }

  size_t shared_spaces_allocation_limit() {
    return shared_spaces_allocation_limit_;
  }
  ...
  bool CanExpandSharedSpace(size_t size) {
    if (force_oom_) return false;
    return (SharedSpaceCapacity() + size) < MaxSharedSpaceSize();
  }
  ...
  inline bool ShouldExpandOnSlowAllocation(Sharability shared) {
    if (shared == SHARED) {
      return ShouldExpandSharedSpaceOnSlowAllocation();
    } else {
      return ShouldExpandOldGenerationOnSlowAllocation();
    }
  }

  bool ShouldExpandOldGenerationOnSlowAllocation();

  bool ShouldExpandSharedSpaceOnSlowAllocation();
  ...
  AllocateMap(InstanceType instance_type, int instance_size,
              Sharability sharable = NOT_SHARED,
  ...
  MUST_USE_RESULT AllocationResult AllocateSymbol(PretenureFlag pretenure);
  ...
  MUST_USE_RESULT AllocationResult
      AllocateWeakCell(HeapObject* value, PretenureFlag pretenure);
  ...
  MUST_USE_RESULT AllocationResult
      AllocateCode(int object_size, bool immovable, bool is_shared = false);
  ...
  intptr_t record_write_stub_mode_;
  AllocationInfo new_space_allocation_info_;
  AllocationInfo old_space_allocation_info_;
  Address* store_buffer_top_;
  ...
  intptr_t max_old_generation_capacity_;
  size_t initial_max_old_generation_size_;
  size_t initial_old_generation_size_;
  size_t max_shared_spaces_size_;
  ...
  OldSpace* old_space_;
  // <YUNOS> changed: begin
  MutableSpace* mutable_space_;
  // <YUNOS> changed: end
  OldSpace* code_space_;
  MapSpace* map_space_;
  OldSpace* shared_old_space_;
  OldSpace* shared_code_space_;
  MapSpace* shared_map_space_;
  LargeObjectSpace* shared_lo_space_;
  LargeObjectSpace* lo_space_;
  // Map from the space id to the space.
  Space* space_[LAST_SPACE + 1];
  // Added for shared space
  HeapState gc_state_;
  ...
  // Limit that tiggers a shared gc on the next GC.
  // This is checked to decied if we should expand a shared paged space
  // on allocation fail.
  size_t shared_spaces_allocation_limit_;
  ...
  AllocationObserver* shared_observer_;
  ...
  CodeHashMap* shared_code_stubs_cache_;
...
class VerifySharedObjectVisitor : public ObjectVisitor, public RootVisitor {
 public:
  void VisitPointers(HeapObject* host, Object** start, Object** end) override;
  void VisitRootPointers(Root root, Object** start, Object** end) override;
...
class PrivateSpaces BASE_EMBEDDED {
 public:
  explicit PrivateSpaces(Heap* heap) : heap_(heap), counter_(FIRST_SPACE) {}
  Space* next();
...
class PrivatePagedSpaces BASE_EMBEDDED {
 public:
  explicit PrivatePagedSpaces(Heap* heap) : heap_(heap), counter_(OLD_SPACE) {}
  PagedSpace* next();
...
class SharedPagedSpaces BASE_EMBEDDED {
  public:
    explicit SharedPagedSpaces(Heap* heap) :
      heap_(heap),
      counter_(FIRST_SHARED_SPACE) {}
    PagedSpace* next();

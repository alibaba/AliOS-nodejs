...
int MarkingVisitor<ConcreteVisitor>::VisitWeakCell(Map* map,
  ...
  if (weak_cell->next_cleared() && !weak_cell->cleared()) {
    ...
    if (ObjectMarking::IsBlackOrGrey<IncrementalMarking::kAtomicity>(
      ...
      // closure.
      // JSMT: TODO shared_encountered_weak_cells
      if (!Page::FromAddress(weak_cell->address())->InSharedSpace()) {
        ...
        sync::MutexLock mu(VMThread::Current(),
            WorkerRuntime::GetLock(worker_runtime,
              WorkerRuntime::kWeakCells));
        // Should check that the weak_cell is not linked before
        CHECK(weak_cell->next_cleared());
        weak_cell->set_next(worker_runtime->encountered_shared_weak_cells(),
                          UPDATE_WEAK_WRITE_BARRIER);
        worker_runtime->set_encountered_shared_weak_cells(weak_cell);
      }
    }
  }

...
Heap* Space::GetProxyHeap() {
  if (!sharable()) {
    return heap_;
  }
  return heap_->isolate()->GetCurrentHeap();
}
...
HeapObject* HeapObjectIterator::FromCurrentPage() {
  while (cur_addr_ != cur_end_) {
    ...
    if (!obj->IsFiller()) {
      if (obj->IsCode()) {
        DCHECK((space_ == heap_->code_space()) ||
               (space_ == heap_->shared_code_space()));
...
bool PagedSpace::Contains(Address addr) {
  return MemoryChunk::FromAnyPointerAddress(heap_safe(), addr)->owner() == this;
}
...
MemoryChunk* MemoryChunk::FromAnyPointerAddress(Heap* heap, Address addr) {
  ...
  if (offset < MemoryChunk::kHeaderSize || !chunk->HasPageHeader()) {
    ...
    if (chunk == nullptr) chunk = heap->shared_lo_space()->FindPageThreadSafe(addr);
  }
  return chunk;
}
...
MemoryChunkIterator::MemoryChunkIterator(Heap* heap)
    : heap_(heap),
      ...
      // <YUNOS> changed: begin
      mutable_iterator_(heap->mutable_space()->begin()),
      // <YUNOS> changed: end
      code_iterator_(heap->code_space()->begin()),
      map_iterator_(heap->map_space()->begin()),
      shared_old_iterator_(heap->shared_old_space()->begin()),
      shared_code_iterator_(heap->shared_code_space()->begin()),
      shared_map_iterator_(heap->shared_map_space()->begin()),
      lo_iterator_(heap->lo_space()->begin()),
      shared_lo_iterator_(heap->shared_lo_space()->begin()) {}
...
MemoryChunk* MemoryChunkIterator::next() {
  switch (state_) {
    case kOldSpaceState: {
      ...
      // <YUNOS> changed: begin
      state_ = kMutableSpaceState;
      // Fall through.
    }
    case kMutableSpaceState: {
      if (mutable_iterator_ != heap_->mutable_space()->end()) return *(mutable_iterator_++);
    // <YUNOS> changed: end
      state_ = kMapState;
      // Fall through.
    }
    ...
    case kCodeState: {
      ...
      state_ = kSharedOldState;
      // Fall through.
    }
    case kSharedOldState: {
      if (shared_old_iterator_ != heap_->shared_old_space()->end())
        return *(shared_old_iterator_++);
      state_ = kSharedCodeState;
      // Fall through.
    }
    case kSharedCodeState: {
      if (shared_code_iterator_ != heap_->shared_code_space()->end())
        return *(shared_code_iterator_++);
      state_ = kSharedMapState;
      // Fall through.
    }
    case kSharedMapState: {
      if (shared_map_iterator_ != heap_->shared_map_space()->end())
        return *(shared_map_iterator_++);
      state_ = kLargeObjectState;
      // Fall through.
    }
    case kLargeObjectState: {
      if (lo_iterator_ != heap_->lo_space()->end()) return *(lo_iterator_++);
      state_ = kSharedLargeObjectState;
      // Fall through;
    }
    case kSharedLargeObjectState: {
      if (shared_lo_iterator_ !=
          heap_->shared_lo_space()->end()) {
        return *(shared_lo_iterator_++);
      }
...
AllocationResult PagedSpace::AllocateRawUnalignedSynchronized(
  ...
  return AllocateRawUnaligned(size_in_bytes, update_skip_list);
}
...
AllocationResult PagedSpace::AllocateRawAligned(int size_in_bytes,
                                                AllocationAlignment alignment) {
  DCHECK(identity() == OLD_SPACE || identity() == SHARED_OLD_SPACE);
...
AllocationResult PagedSpace::AllocateRawSynchronized(int size_in_bytes,
                                         AllocationAlignment alignment) {
  AllocationResult result;
  {
    base::LockGuard<base::RecursiveMutex> lock_guard(&space_mutex_);
#ifdef V8_HOST_ARCH_32_BIT
    result = alignment == kDoubleAligned
      ? AllocateRawAligned(size_in_bytes, kDoubleAligned)
      : AllocateRawUnaligned(size_in_bytes);
#else
    result = AllocateRawUnaligned(size_in_bytes);
#endif
  }
...
#ifdef DEBUG
bool AllocationInfo::VerifyPagedAllocation() {

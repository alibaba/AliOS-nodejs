...
bool IncrementalMarking::WhiteToGreyAndPush(HeapObject* obj) {
  if (obj->is_shared_object()) return false;
...
void IncrementalMarking::MarkBlackAndPush(HeapObject* obj) {
  if (obj->is_shared_object()) return;
...
class IncrementalMarkingMarkingVisitor final
 ...
 public:
  ...
  V8_INLINE int VisitFixedArray(Map* map, FixedArray* object) {
    ...
    if (object->is_shared_object()) return object_size;
  ...
  V8_INLINE bool MarkObjectWithoutPush(Object* obj) {
    ...
    if (heap_object->is_shared_object()) return false;
...
class IncrementalMarkingRootMarkingVisitor : public RootVisitor {
 ...
 private:
  void MarkObjectByPointer(Object** p) {
    ...
    // Incremental marking do not process shared object
    if (HeapObject::cast(obj)->is_shared_object()) return;
...
void IncrementalMarking::DeactivateIncrementalWriteBarrier() {
  ...
  // <YUNOS> changed: begin
  DeactivateIncrementalWriteBarrierForSpace(heap_->mutable_space());
  // <YUNOS> changed: end
  DeactivateIncrementalWriteBarrierForSpace(heap_->map_space());
  DeactivateIncrementalWriteBarrierForSpace(heap_->code_space());
  DeactivateIncrementalWriteBarrierForSpace(heap_->new_space());
  // TODO JSMT: shared space do not support incremental marking
  // so do need support black allocation
  /*
  DeactivateIncrementalWriteBarrierForSpace(heap_->shared_old_space());
  DeactivateIncrementalWriteBarrierForSpace(heap_->shared_map_space());
  DeactivateIncrementalWriteBarrierForSpace(heap_->shared_code_space());
  */
  ...
  for (LargePage* lop : *heap_->shared_lo_space()) {
    SetOldSpacePageFlags(lop, false, false);
  }
}
...
void IncrementalMarking::ActivateIncrementalWriteBarrier() {
  ...
  // <YUNOS> changed: begin
  ActivateIncrementalWriteBarrier(heap_->mutable_space());
  // <YUNOS> changed: end
  ...
  // so do need support black allocation
  /*
  ActivateIncrementalWriteBarrier(heap_->shared_old_space());
  ActivateIncrementalWriteBarrier(heap_->shared_map_space());
  ActivateIncrementalWriteBarrier(heap_->shared_code_space());
  */
...
static void PatchIncrementalMarkingRecordWriteStubs(
  ...
  heap->set_record_write_stub_mode(mode);
}
...
// Marking bits are cleared by the sweeper.
#ifdef VERIFY_HEAP
  if (FLAG_verify_heap) {
    heap_->mark_compact_collector()->VerifyMarkbitsAreClean(false);
  }
#endif
  ...
  heap_->IterateStrongRoots(&visitor, VISIT_ONLY_STRONG_WITHOUT_SHARED);
...
void IncrementalMarking::StartBlackAllocation() {
  ...
  heap()->mutable_space()->MarkAllocationInfoBlack();
  // TODO JSMT: shared space do not support incremental marking
  // so do need support black allocation
  /*
  heap()->shared_old_space()->MarkAllocationInfoBlack();
  heap()->shared_map_space()->MarkAllocationInfoBlack();
  heap()->shared_code_space()->MarkAllocationInfoBlack();
  */
...
void IncrementalMarking::PauseBlackAllocation() {
  ...
  heap()->mutable_space()->UnmarkAllocationInfo();
  if (FLAG_trace_incremental_marking) {
    heap()->isolate()->PrintWithTimestamp(
...
void IncrementalMarking::MarkObjectGroups() {
  ...
  IncrementalMarkingRootMarkingVisitor visitor(this);
  heap_->mark_compact_collector()->MarkImplicitRefGroups(&visitor);
...
void IncrementalMarking::ProcessWeakCells() {
  ...
  // JSMT: shared weak cell is not process during incremental marking
  // the incremental marking result will abort before ALL GC
...
void IncrementalMarking::FinalizeIncrementally() {
  // JSMT: may stopped by other thread which do all gc
  if (IsStopped()) return;
...
void IncrementalMarking::RevisitObject(HeapObject* obj) {
  ...
  if ((page->owner() != nullptr) && (page->owner()->identity() == LO_SPACE ||
        page->owner()->identity() == SHARED_LO_SPACE)) {

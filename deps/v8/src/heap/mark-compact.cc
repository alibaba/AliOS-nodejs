...
class FullMarkingVerifier : public MarkingVerifier {
 public:
  ...
  void Run() override {
    VerifyRoots(heap_->ShouldProcessShared()?
        VISIT_ONLY_STRONG : VISIT_ONLY_STRONG_WITHOUT_SHARED);
    VerifyMarking(heap_->new_space());
    VerifyMarking(heap_->old_space());
    VerifyMarking(heap_->code_space());
    VerifyMarking(heap_->map_space());
    // <YUNOS> changed: begin
    VerifyMarking(heap_->mutable_space());
    // <YUNOS> changed: end
    ...
    if (heap_->ShouldProcessShared()) {
      VerifyMarking(heap_->shared_old_space());
      VerifyMarking(heap_->shared_code_space());
      VerifyMarking(heap_->shared_map_space());

      LargeObjectIterator sit(heap_->shared_lo_space());
      for (HeapObject* obj = sit.Next(); obj != NULL; obj = sit.Next()) {
        if (ObjectMarking::IsBlackOrGrey(obj, marking_state(obj))) {
          obj->Iterate(this);
        }
      }
    ...
    for (Object** current = start; current < end; current++) {
      if ((*current)->IsHeapObject()) {
        ...
        CHECK(ObjectMarking::IsBlackOrGrey(object, marking_state(object)) ||
            (object->is_shared_object() &&
            !object->GetHeap()->
            mark_compact_collector()->collect_shared_space()));
      }
    }
  }
...
class FullEvacuationVerifier : public EvacuationVerifier {
 public:
  ...
  void Run() override {
    VerifyRoots(heap()->ShouldProcessShared()?
        VISIT_ALL: VISIT_ALL_WITHOUT_SHARED);
    VerifyEvacuation(heap_->new_space());
    VerifyEvacuation(heap_->old_space());
    VerifyEvacuation(heap_->code_space());
    VerifyEvacuation(heap_->map_space());
    // <YUNOS> changed: begin
    VerifyEvacuation(heap_->mutable_space());
    // <YUNOS> changed: end
    if (heap()->ShouldProcessShared()) {
      VerifyEvacuation(heap_->shared_old_space());
      VerifyEvacuation(heap_->shared_code_space());
      VerifyEvacuation(heap_->shared_map_space());
    }
  }
...
bool MarkCompactCollector::StartCompactionForShared() {
  CHECK(collect_shared_space());
  CollectEvacuationCandidates(heap()->shared_old_space());
  if (FLAG_compact_code_space) {
    CollectEvacuationCandidates(heap()->shared_code_space());
  } else if (FLAG_trace_fragmentation) {
    TraceFragmentation(heap()->shared_code_space());
  }

  if (FLAG_trace_fragmentation) {
    TraceFragmentation(heap()->shared_map_space());
  }
...
void MarkCompactCollector::CollectGarbageForShared() {
  WorkerRuntime* runtime = heap()->isolate()->worker_runtime();
  // Step1: Mark all live objects
  runtime->ForAllWorkers([](VMThread* thread) {
      IsolateCurrentScope scs(thread->isolate());
      MarkCompactCollector* collector =
        thread->isolate()->heap()->mark_compact_collector();
      // Mark the mark_compact_collector can collect shared space
      collector->MarkLiveObjects();
  });

  // Step2: Clean Non Live References
  runtime->ForAllWorkers([](VMThread* thread) {
      IsolateCurrentScope scs(thread->isolate());
      thread->isolate()->heap()->mark_compact_collector()->ClearNonLiveReferences();
  });

  // Step3: RecordObjectStats
  runtime->ForAllWorkers([](VMThread* thread) {
      IsolateCurrentScope scs(thread->isolate());
      thread->isolate()->heap()->mark_compact_collector()->RecordObjectStats();
  });
  ...
  // Step4: StartSweepSpaces for other threads
  runtime->ForAllWorkers([](VMThread* thread) {
      IsolateCurrentScope scs(thread->isolate());
      thread->isolate()->heap()->mark_compact_collector()->StartSweepSpaces();
  });

  // Sweep the shared space
  SweepSharedSpaces();

  // Step6: Evacuat and updatePointers
  EvacuateForShared();

  // Step7: Finish
  // Just need do onece for shared lo space
  FinishForShared();

  runtime->ForAllWorkers([](VMThread* thread) {
      IsolateCurrentScope scs(thread->isolate());
      MarkCompactCollector* collector = thread->isolate()->heap()->mark_compact_collector();
      collector->Finish();
  });
}
...
void MarkCompactCollector::VerifyMarkbitsAreClean(bool verify_shared) {
  VerifyMarkbitsAreClean(heap_->old_space());
  // <YUNOS> changed: begin
  VerifyMarkbitsAreClean(heap_->mutable_space());
  // <YUNOS> changed: end
  VerifyMarkbitsAreClean(heap_->code_space());
  VerifyMarkbitsAreClean(heap_->map_space());
  VerifyMarkbitsAreClean(heap_->new_space());
  if (verify_shared) {
    VerifyMarkbitsAreClean(heap_->shared_old_space());
    VerifyMarkbitsAreClean(heap_->shared_code_space());
    VerifyMarkbitsAreClean(heap_->shared_map_space());
  }
  ...
  if (verify_shared) {
    LargeObjectIterator it(heap_->shared_lo_space());
    for (HeapObject* obj = it.Next(); obj != NULL; obj = it.Next()) {
      CHECK(ObjectMarking::IsWhite(obj, MarkingState::Internal(obj)));
      CHECK_EQ(0, MarkingState::Internal(obj).live_bytes());
    }
...
void MarkCompactCollector::VerifyWeakEmbeddedObjectsInCode() {
  HeapObjectIterator code_iterator(heap()->code_space(), heap());
...
void MarkCompactCollector::VerifyOmittedMapChecks() {
  HeapObjectIterator iterator(heap()->map_space(), heap());
...
void MarkCompactCollector::ClearMarkbits() {
  ...
  // <YUNOS> changed: begin
  ClearMarkbitsInPagedSpace(heap_->mutable_space());
  // <YUNOS> changed: end
  ClearMarkbitsInPagedSpace(heap_->shared_code_space());
  ClearMarkbitsInPagedSpace(heap_->shared_map_space());
  ClearMarkbitsInPagedSpace(heap_->shared_old_space());
  ClearMarkbitsInNewSpace(heap_->new_space());
  heap_->lo_space()->ClearMarkingStateOfLiveObjects();
  heap_->shared_lo_space()->ClearMarkingStateOfLiveObjects();
}
...
void MarkCompactCollector::EnsureSweepingCompleted() {
  ...
  // <YUNOS> changed: begin
  heap()->mutable_space()->RefillFreeList();
  // <YUNOS> changed: end
  ...
  if (heap()->unmapper()->has_delayed_chunks())
...
void MarkCompactCollector::CollectEvacuationCandidates(PagedSpace* space) {
  DCHECK(space->identity() == OLD_SPACE || space->identity() == CODE_SPACE ||
         space->identity() == MUTABLE_SPACE ||
         space->identity() == SHARED_OLD_SPACE || space->identity() == SHARED_CODE_SPACE);
  ...
  for (Page* p : *space) {
    if (p->NeverEvacuate() ||
        p == owner_of_linear_allocation_area) continue;
...
void MarkCompactCollector::Prepare(bool need_verify) {
  ...
  // them here.
  heap()->unmapper()->WaitUntilCompleted();

  heap()->concurrent_marking()->EnsureCompleted();

  // Clear marking bits if incremental marking is aborted.
  if (was_marked_incrementally_ &&
      (heap_->ShouldAbortIncrementalMarking() ||
       collect_shared_space())) {
    heap()->incremental_marking()->Stop();
  ...
  PrivatePagedSpaces spaces(heap());
  for (PagedSpace* space = spaces.next(); space != NULL;
       space = spaces.next()) {
    space->PrepareForMarkCompact(heap());
  }
  heap()->account_external_memory_concurrently_freed();

#ifdef VERIFY_HEAP
  if (!was_marked_incrementally_ && FLAG_verify_heap) {
    VerifyMarkbitsAreClean(need_verify);
  }
#endif
}

void MarkCompactCollector::PrepareForShared() {
/*
 * Incremental marking may mark shared objects make verify fail
 * So we should clean shared space's markbit before doning full gc
 * */
  SharedPagedSpaces spaces(heap_);
  for (Space* space = spaces.next(); space != nullptr; space = spaces.next()) {
    PagedSpace* pspace = reinterpret_cast<PagedSpace*>(space);
    ClearMarkbitsInPagedSpace(pspace);
    pspace->PrepareForMarkCompact(heap());
  }
...
#endif
  heap_->isolate()->worker_runtime()->
    UpdateSharedSpacesAllocationCounter();

  // StartCompaction for shraed
  StartCompactionForShared();
}
...
void MarkCompactCollector::Finish() {
  ...
  if (heap()->weak_wasm_data_table() != Smi::kZero) {
    WeakHashTable::cast(heap()->weak_wasm_data_table())->Rehash();
  }

  if (heap()->weak_debug_points_table() != Smi::kZero) {
    WeakHashTable::cast(heap()->weak_debug_points_table())->Rehash();
  }
 ...
 private:
  void MarkObjectByPointer(HeapObject* host, Object** p,
    ...
    if (ObjectMarking::WhiteToBlack<AccessMode::NON_ATOMIC>(
      ...
      // JSMT: All shared object should have been marked black
      // before normal gc begin
      DCHECK(collector_->collect_shared_space() ||
              !object->is_shared_object());
      if (V8_UNLIKELY(FLAG_track_retaining_path)) {
        if (host) {
          collector_->heap()->AddRetainer(host, object);
        } else {
          collector_->heap()->AddRetainingRoot(root, object);
        }
      }
      Map* map = object->map();
...
class ProfilingMigrationObserver final : public MigrationObserver {
 public:
  ...
  inline void Move(AllocationSpace dest, HeapObject* src, HeapObject* dst,
                   int size) final {
    if (dest == CODE_SPACE || dest == SHARED_CODE_SPACE ||
       ((dest == OLD_SPACE || dest == SHARED_OLD_SPACE) && dst->IsBytecodeArray())) {
...
class EvacuateNewSpacePageVisitor final : public HeapObjectVisitor {
 public:
  ...
  static void Move(Page* page, Heap* heap) {
    switch (mode) {
      case NEW_TO_NEW:
        heap->new_space()->MovePageFromSpaceToSpace(page);
...
bool MarkCompactCollector::IsUnmarkedHeapObject(Object** p) {
  ...
  bool ret = ObjectMarking::IsWhite(HeapObject::cast(o),
                                MarkingState::Internal(HeapObject::cast(o)));
  DCHECK(!ret || (!HeapObject::cast(o)->is_shared_object() ||
                   HeapObject::cast(o)->GetHeap()->
...
bool MarkCompactCollector::IsUnmarkedHeapObjectWithHeap(Heap* heap, Object** p) {
  USE(heap);
  Object* o = *p;
  if (!o->IsHeapObject()) return false;
...
#endif
bool MarkCompactCollector::IsSharedAndUnmarkedHeapObject(Object** p) {
  Object* o = *p;
  if (!o->IsHeapObject()) return false;
  HeapObject* heap_object = HeapObject::cast(o);
  // This function is just called during all gc
  DCHECK(heap_object->GetHeap()
      ->mark_compact_collector()
      ->collect_shared_space());
  return IsUnmarkedHeapObject(p) &&
    heap_object->is_shared_object();
}
...
void MarkCompactCollector::MarkRoots(RootMarkingVisitor* visitor) {
  ...
  heap()->IterateStrongRoots(visitor,
      heap()->ShouldProcessShared() ?
      VISIT_ONLY_STRONG :
      VISIT_ONLY_STRONG_WITHOUT_SHARED);

  CHECK(heap()->string_table()
      ->is_shared_object());
...
void MarkCompactCollector::MarkSharedObjects() {
  SharedPagedSpaces spaces(heap_);
  for (PagedSpace* space = spaces.next();
       space != nullptr;
       space = spaces.next()) {
    for (Page* p : *space) {
      p->MarkBlackArea(p->area_start(), p->area_end());
    }
  }

  LargeObjectIterator sit(heap_->shared_lo_space());
  for (HeapObject* obj = sit.Next(); obj != NULL; obj = sit.Next()) {
    MarkBit mark = ObjectMarking::MarkBitFrom(obj, marking_state(obj));
    if (Marking::IsBlackOrGrey(mark)) continue;
    ObjectMarking::WhiteToBlack<AccessMode::NON_ATOMIC>(obj, marking_state(obj));
  }
}
...
void MarkCompactCollector::RefillMarkingWorklist() {
  ...
  if (marking_worklist()->IsFull()) return;
   ...
   for (Space* space = spaces.next(); space != nullptr; space = spaces.next()) {
     if (space->sharable() && !collect_shared_space()) continue;
     if (space->identity() == LO_SPACE ||
         space->identity() == SHARED_LO_SPACE) {
       LargeObjectIterator lo_it(reinterpret_cast<LargeObjectSpace*>(space));
       DiscoverGreyObjectsWithIterator(&lo_it);
       if (marking_worklist()->IsFull()) return;
     } else if (space->identity() == NEW_SPACE) {
       continue;
     } else {
       DiscoverGreyObjectsInSpace(reinterpret_cast<PagedSpace*>(space));
       if (marking_worklist()->IsFull()) return;
     }
   }
...
void MarkCompactCollector::MarkImplicitRefGroups(
  ...
  for (int i = 0; i < ref_groups->length(); i++) {
    ...
    HeapObject* object = *entry->parent;
    ...
    for (size_t j = 0; j < entry->length; ++j) {
      visitor->VisitRootPointer(Root::kGlobalHandles, &children[j][0]);
    }
...
void MarkCompactCollector::ProcessEphemeralMarking(
    RootVisitor* visitor, bool only_process_harmony_weak_collections) {
...
void MarkCompactCollector::MarkLiveObjects() {
  ...
  if (!collect_shared_space()) {
    // JSMT: if we do not collect shared space,
    // we should mark all shared objects to shared
    // TODO Shared objects may be allocated by other threads
    // we should make sure our marking visitor will not point to these objects
    MarkSharedObjects();
  }
...

  {
    ...
    if (collect_shared_space()) {
      // JSMT: process global handle callback
      {
        TRACE_GC(heap()->tracer(),
            GCTracer::Scope::MC_MARK_WEAK_CLOSURE_WEAK_HANDLES);
        // If the object is unmarked and the is shared object,
        // mark the global handle as pending
        heap()->isolate()->global_handles()->IdentifyWeakHandles(
            &IsSharedAndUnmarkedHeapObject);
        ProcessMarkingWorklist();
      }
      {
        // > For normal object, mark all global handles as live
        // > For shared object, just process the PendingPhantomResetHandle
        TRACE_GC(heap()->tracer(),
            GCTracer::Scope::MC_MARK_WEAK_CLOSURE_WEAK_ROOTS);
        heap()->isolate()->global_handles()->
          IterateWeakRootsForAllGc(&root_visitor);
        ProcessMarkingWorklist();
      }
    } else {
...
void MarkCompactCollector::ClearNonLiveReferences() {
...

  {
    ...
    if (heap()->ShouldProcessShared()) {
      WorkerRuntime* worker_runtime = heap()->isolate()->worker_runtime();
      worker_runtime->IterateSharedExternalStringTable(&external_visitor);
      worker_runtime->CleanUpSharedExternalStringTable();
    }
  }
  ...
  DependentCode* dependent_code_list =
    DependentCode::cast(heap_->empty_fixed_array());
  Object* non_live_map_list = Smi::kZero;

  ClearWeakCellsForNormal(&non_live_map_list, &dependent_code_list);
  if (heap()->ShouldProcessShared()) {
    ClearWeakCellsForShared(&non_live_map_list, &dependent_code_list);
  }
...
void MarkCompactCollector::ClearWeakCollections() {
  ...
  while (weak_collection_obj != Smi::kZero) {
    ...
    CHECK(!HeapObject::cast(weak_collection_obj)->is_shared_object());
    weak_collection_obj = weak_collection->next();
...
void MarkCompactCollector::ClearWeakCellsForNormal(Object** non_live_map_list,
                                          DependentCode** dependent_code_list) {
  ClearWeakCellsInternal(non_live_map_list, dependent_code_list,
                          heap_->encountered_weak_cells());
  heap_->set_encountered_weak_cells(Smi::kZero);
  return;
}

void MarkCompactCollector::ClearWeakCellsForShared(Object** non_live_map_list,
                                          DependentCode** dependent_code_list) {
  WorkerRuntime* worker_runtime = heap_->isolate()->worker_runtime();
  // process the Work cell should get the shared lock
  // JSMT: we must during Pause scope
  CHECK(collect_shared_space());
  ClearWeakCellsInternal(non_live_map_list, dependent_code_list,
                          worker_runtime->encountered_shared_weak_cells());
  heap_->isolate()->worker_runtime()
    ->set_encountered_shared_weak_cells(Smi::kZero);
  return;
}

void MarkCompactCollector::ClearWeakCellsInternal(Object** non_live_map_list,
                                          DependentCode** dependent_code_list,
                                          Object* weak_cell_obj) {
  TRACE_GC(heap_->tracer(), GCTracer::Scope::MC_CLEAR_WEAK_CELLS);
  Object* the_hole_value = heap_->the_hole_value();
  DependentCode* dependent_code_head = *dependent_code_list;
  Object* non_live_map_head = *non_live_map_list;
  ...
  while (weak_cell_obj != Smi::kZero) {
    ...
    CHECK(!HeapObject::cast(weak_cell)->is_shared_object()
        || value->is_shared_object());
    ...
    if (!ObjectMarking::IsBlackOrGrey(value, MarkingState::Internal(value))) {
      ...
      if (value->IsCell()) {
        ...
        // The Shared weak cell should not hold shared weak cells agin
        CHECK(!HeapObject::cast(cell_value)->is_shared_object());
...
void MarkCompactCollector::AbortWeakCells() {
  ...
  WorkerRuntime* worker_runtime = heap()->isolate()->worker_runtime();
  Object* shared_weak_cell_obj = worker_runtime->encountered_shared_weak_cells();
  while (shared_weak_cell_obj != Smi::kZero) {
    WeakCell* weak_cell = reinterpret_cast<WeakCell*>(shared_weak_cell_obj);
    shared_weak_cell_obj = weak_cell->next();
    weak_cell->clear_next(the_hole_value);
  }
  worker_runtime->set_encountered_shared_weak_cells(Smi::kZero);
}
  ...
  for (Page* page : new_space_evacuation_pages_) {
    ...
    if (ShouldMovePage(page, live_bytes_on_page)) {
      if (page->IsFlagSet(MemoryChunk::NEW_SPACE_BELOW_AGE_MARK)) {
        EvacuateNewSpacePageVisitor<NEW_TO_OLD>::Move(page, heap());
      } else {
        EvacuateNewSpacePageVisitor<NEW_TO_NEW>::Move(page, heap());
      }
    }
    evacuation_job.AddItem(new PageEvacuationItem(page));
  }
...
int MarkCompactCollector::Sweeper::RawSweep(
    Heap* heap, Page* p, FreeListRebuildingMode free_list_mode,
    FreeSpaceTreatmentMode free_space_mode) {
  Space* space = p->owner();
  ...
  DCHECK(free_list_mode == IGNORE_FREE_LIST || space->identity() == OLD_SPACE ||
         ...
         space->identity() == SHARED_CODE_SPACE || space->identity() == SHARED_MAP_SPACE ||
         space->identity() == SHARED_OLD_SPACE || space->identity() == MUTABLE_SPACE);
  ...
  const bool rebuild_skip_list =
      (space->identity() == CODE_SPACE ||
       space->identity() == SHARED_CODE_SPACE)
        && p->skip_list() != nullptr;
  ...
  for (auto object_and_size : LiveObjectRange<kBlackObjects>(p, state)) {
    ...
    if (free_end != free_start) {
      ...
      if (free_list_mode == REBUILD_FREE_LIST) {
        ...
        heap->CreateFillerObjectAt(free_start, static_cast<int>(size),
                                        ClearRecordedSlots::kNo);
      }
...
void MarkCompactCollector::EvacuateForSharedStep1() {
  //Heap::RelocationLock
...
void MarkCompactCollector::EvacuateForSharedStep2() {
  if (!heap()->new_space()->Rebalance()) {
    FatalProcessOutOfMemory("NewSpace::Rebalance");
  }
...
void MarkCompactCollector::EvacuateForShared() {
  WorkerRuntime* runtime = heap()->isolate()->worker_runtime();
  // Step1 Evacuate
  runtime->ForAllWorkers([](VMThread* thread){
    IsolateCurrentScope scs(thread->isolate());
    MarkCompactCollector* collector =
      thread->isolate()->heap()->mark_compact_collector();
    collector->EvacuateForSharedStep1();
  });
  ...
  runtime->ForAllWorkers([](VMThread* thread){
    ...
    MarkCompactCollector* collector =
      thread->isolate()->heap()->mark_compact_collector();
      collector->UpdatePointersAfterEvacuation();
  });
  ...
  runtime->ForAllWorkers([](VMThread* thread){
    ...
    MarkCompactCollector* collector =
      thread->isolate()->heap()->mark_compact_collector();
    collector->EvacuateForSharedStep2();
  });

  // JSMT: because some shared pages maybe abort compaction
  // and be added to sweeper(), so we should ensure the shared
  // sweep has been done again.
  EnsureSweepSharedSpaces();
}
...
void MarkCompactCollector::UpdatePointersAfterEvacuation() {
...

  {
    ...
    if (heap()->ShouldProcessShared()) {
      heap_->UpdateReferencesInSharedExternalStringTable(
        &UpdateReferenceInExternalStringTableEntry);
    }
...
int MarkCompactCollector::Sweeper::ParallelSweepPage(Page* page,
  ...
  int max_freed = 0;
  {
    ...
    if (identity == NEW_SPACE) {
      RawSweep(heap_, page, IGNORE_FREE_LIST, free_space_mode);
    } else {
      max_freed = RawSweep(heap_, page, REBUILD_FREE_LIST, free_space_mode);
    }
    DCHECK(page->SweepingDone());
...
void MarkCompactCollector::StartSweepSpace(PagedSpace* space) {
  ...
  for (auto it = space->begin(); it != space->end();) {
    ...
    if (p->IsFlagSet(Page::NEVER_ALLOCATE_ON_PAGE)) {
      ...
      Sweeper::RawSweep(heap_, p, Sweeper::IGNORE_FREE_LIST,
...
void MarkCompactCollector::FinishForShared() {
  heap_->shared_lo_space()->ClearMarkingStateOfLiveObjects();
}

void MarkCompactCollector::SweepSharedSpaces() {
  {
    GCTracer::Scope sweep_scope(heap()->tracer(),
        GCTracer::Scope::MC_SWEEP_OLD_SHARED);
    StartSweepSpace(heap()->shared_old_space());
  }
  {
    GCTracer::Scope sweep_scope(heap()->tracer(),
        GCTracer::Scope::MC_SWEEP_CODE_SHARED);
    StartSweepSpace(heap()->shared_code_space());
  }
  {
    GCTracer::Scope sweep_scope(heap()->tracer(),
        GCTracer::Scope::MC_SWEEP_MAP_SHARED);
    StartSweepSpace(heap()->shared_map_space());
  }

  // Just sort the sweeping_list
  sweeper().StartSweeping();

  heap_->shared_lo_space()->FreeUnmarkedObjects();
...
void MarkCompactCollector::EnsureSweepSharedSpaces() {
  /*Actual sweep the shared space*/
  sweeper().ParallelSweepSpace(SHARED_OLD_SPACE, 0);
  sweeper().ParallelSweepSpace(SHARED_CODE_SPACE, 0);
  sweeper().ParallelSweepSpace(SHARED_MAP_SPACE, 0);
  heap()->shared_old_space()->RefillFreeList(heap());
  heap()->shared_code_space()->RefillFreeList(heap());
  heap()->shared_map_space()->RefillFreeList(heap());
}
...
void MarkCompactCollector::StartSweepSpaces() {
  ...
  {
      GCTracer::Scope sweep_scope(heap()->tracer(),
        GCTracer::Scope::MC_SWEEP_MUTABLE);
     StartSweepSpace(heap()->mutable_space());
  }
  {
    GCTracer::Scope sweep_scope(heap()->tracer(),

...
void MarkCompactCollector::PushBlack(HeapObject* obj) {
  ...
  // We can not vist shared object during normal gc
  if (!collect_shared_space() &&
      obj->is_shared_object()) return;
...
LiveObjectRange<mode>::iterator::iterator(MemoryChunk* chunk,
    ...
    : chunk_(chunk),
      one_word_filler_map_(chunk->heap_safe()->one_pointer_filler_map()),
      two_word_filler_map_(chunk->heap_safe()->two_pointer_filler_map()),
      free_space_map_(chunk->heap_safe()->free_space_map()),
      it_(chunk, state) {
  it_.Advance(Bitmap::IndexToCell(
      Bitmap::CellAlignIndex(chunk_->AddressToMarkbitIndex(start))));
  DCHECK(one_word_filler_map_->is_shared_object());
  DCHECK(two_word_filler_map_->is_shared_object());
  DCHECK(free_space_map_->is_shared_object());
  if (!it_.Done()) {

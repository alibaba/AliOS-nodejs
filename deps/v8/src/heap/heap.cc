...
class SharedObserver : public AllocationObserver {
 public:
  SharedObserver(Heap& heap, intptr_t step_size)
      : AllocationObserver(step_size), heap_(heap) {}

  void Step(int bytes_allocated, Address, size_t) override {
    Isolate* current = heap_.isolate()->GetCurrentIsolate();
    current->heap()->incremental_marking()->AdvanceIncrementalMarkingOnAllocation();
  }
...
Heap::Heap()
    : external_memory_(0),
      ...
      max_old_generation_capacity_(828ul * (kPointerSize / 4) * MB),
      initial_max_old_generation_size_(max_old_generation_size_),
      initial_old_generation_size_(max_old_generation_size_ /
                                   kInitalOldGenerationLimitFactor),
      max_shared_spaces_size_(700ul * (kPointerSize / 4) * MB),
      ...
      shared_spaces_allocation_limit_(max_shared_spaces_size_ /
          kInitalSharedSpacesLimitFactor),
      ...
      work_status_(STATUS_FOREGROUND),
      pending_layout_change_object_(nullptr) {
  // Ensure old_generation_size_ is a multiple of kPageSize.
  DCHECK((max_old_generation_size_ & (Page::kPageSize - 1)) == 0);
  DCHECK((max_shared_spaces_size_ & (Page::kPageSize - 1)) == 0);
  ...
  set_record_write_stub_mode(RecordWriteStub::STORE_BUFFER_ONLY);
...
// <YUNOS> changed: begin
void Heap::MarkSeedHeapPreFork() {
  bool temp_for_flag_stress_compaction = FLAG_stress_compaction;
  FLAG_stress_compaction = true;
  CollectAllAvailableGarbage(GarbageCollectionReason::kTesting);
  FLAG_stress_compaction = temp_for_flag_stress_compaction;
  DCHECK_EQ(static_cast<intptr_t>(0), new_space()->Size());

  shared_old_space()->MarkPagesFromSeedAndDisposeFreeSpace();
  shared_code_space()->MarkPagesFromSeedAndDisposeFreeSpace();
  old_space()->MarkPagesFromSeedAndDisposeFreeSpace();
  code_space()->MarkPagesFromSeedAndDisposeFreeSpace();

  if (FLAG_freeze_code_for_seed) {
    // Change all codes to young age.
    code_space()->MakeAllCodesYoung();
    shared_code_space()->MakeAllCodesYoung();
  }

  base::BaseFlags::SetSeedHeap(false);
  // Because of GC's lazy sweep, sweep task will lazy finished
  // at next GC. But, GC in MarkSeedHeapPreFork shouldn't do
  // that, for COW issue. Here, we force to finish sweep task.
  mark_compact_collector()->EnsureSweepingCompleted();
}


void Heap::PrintChunkInfoInSpaces() {
  // high_water_mark didn't update in time, so we update it manually.
  MemoryChunk::UpdateHighWaterMark(old_space()->top());
  MemoryChunk::UpdateHighWaterMark(code_space()->top());
  MemoryChunk::UpdateHighWaterMark(map_space()->top());
  MemoryChunk::PrintChunkInfos(this);
}


bool Heap::InSeedHeap(Address obj) {
  MemoryChunk* mc = MemoryChunk::FromAddress(obj);
  return mc->IsFlagSet(MemoryChunk::IN_SEED_HEAP);
}


void Heap::ResetMaxOldGenerationSize() {
  max_old_generation_size_ = max_old_generation_capacity_;
  memory_allocator()->UpdateMaxCapacity(MaxReserved());
}


// For some interactive apps, the heap may increase quickly, if we use
// the grow factor as system, the gc caused frequently, so the app stop
// frequently, this will take bad experience;
// Since the cms-gc conflicts with the incremental gc, and if full gc
// occures when cms-gc enables, the STW time is longer than incremental
// gc, so we should disable cms-gc.
void Heap::SetInteractiveModeHeapStrategy() {
  Heap::kMinHeapGrowingFactor = 1.1;
  Heap::kMaxHeapGrowingFactor = 4.0;
  Heap::kMaxHeapGrowingFactorMemoryConstrained = 2.0;
}
// <YUNOS> changed: end
...
size_t Heap::Capacity() {
  ...
  return new_space_->Capacity() + OldGenerationCapacity() + SharedSpaceCapacity();
}

size_t Heap::SharedSpaceCapacity() {
  if (!HasBeenSetUp()) return 0;

  return shared_old_space_->Capacity() + shared_code_space_->Capacity() +
         shared_map_space_->Capacity();
}

size_t Heap::CommittedSharedMemory() {
  if (!HasBeenSetUp()) return 0;

  return shared_old_space_->CommittedMemory() + shared_code_space_->CommittedMemory() +
         shared_map_space_->Capacity();
}
...
size_t Heap::CommittedOldGenerationMemory() {
  ...
  return old_space_->CommittedMemory() + code_space_->CommittedMemory() +
         ...
         mutable_space_->CommittedMemory();
}
// <YUNOS> changed: end
...
size_t Heap::CommittedPhysicalMemory() {
  ...
  return new_space_->CommittedPhysicalMemory() +
         ...
         mutable_space_->CommittedPhysicalMemory() +
         code_space_->CommittedPhysicalMemory() +
         map_space_->CommittedPhysicalMemory() +
         shared_old_space_->CommittedPhysicalMemory() +
         shared_code_space_->CommittedPhysicalMemory() +
         shared_map_space_->CommittedPhysicalMemory() +
         shared_lo_space_->CommittedPhysicalMemory();
}
// <YUNOS> changed: end
...
bool Heap::HasBeenSetUp() {
  return old_space_ != NULL && code_space_ != NULL && map_space_ != NULL &&
         lo_space_ != NULL && shared_old_space_ != NULL &&
         mutable_space_ != NULL &&
         shared_code_space_ != NULL && shared_map_space_ != NULL;
}

bool static RSSLimitReached() {
  // Only when a appilication set a rss_limit can we do this strategy
  // To count the times RSS larger than limit
  static int gc_counter_for_increase = 0;
  // To count the times RSS smaller than limit
  static int gc_counter_for_decrease = 0;
  if (FLAG_rss_limit > 0) {
    size_t rss = 0;
    // If RSS is larger than limit 6 times, we increase rss_limit
    const int gc_count_check_increase = 6;
    // If RSS is smaller than limit 3 times, we decrease rss_limit
    const int gc_count_check_decrease = 3;

    if (base::OS::GetRssMemory(&rss) == 0) {
      // Get the RSS of this process
      PrintF("GetRssMemory %zu\n", rss / MB);
      if (rss > (size_t)(FLAG_rss_limit * MB)) {
        // If RSS is larger then limit, we will do a full gc.
        // Also, we count the times for this situation and if
        // it repeat continuously for 6 times, we increase the
        // limit for that the limit is too small now.
        gc_counter_for_increase ++;
        gc_counter_for_decrease = 0;
        if (gc_counter_for_increase > gc_count_check_increase) {
          FLAG_rss_limit += FLAG_rss_step;
          gc_counter_for_increase = 0;
        }
        // Will do a full gc
        return true;
      } else {
        // If RSS is smaller then limit, we just count the times
        // for this situation and if it repeat continuously for
        // 3 times, we decrease the limit for that the limit is
        // too big now.
        gc_counter_for_increase = 0;
        gc_counter_for_decrease ++;
        if (gc_counter_for_decrease > gc_count_check_decrease) {
          FLAG_rss_limit = Max(1, FLAG_rss_limit - FLAG_rss_step);
          gc_counter_for_decrease = 0;
        }
      // just go ahead
      }
    } else {
      PrintF("error: GetRssMemory failed");
    }
  }
  return false;
}
...
GarbageCollector Heap::SelectGarbageCollector(AllocationSpace space,
  ...
  // <YUNOS> change: begin
  if (RSSLimitReached()) {
    *reason = "RSS limit reached";
    return MARK_COMPACTOR;
  }
  // <YUNOS> change: end
  // Is allocted in shared space?
  if (space == SHARED_OLD_SPACE ||
      space == SHARED_CODE_SPACE ||
      space == SHARED_MAP_SPACE ||
      space == SHARED_LO_SPACE) {
    return MARK_COMPACTOR_SHARED;
  }
...
GCTracer* Heap::tracer() {
  if (collect_shared_space()) {
    return isolate_->worker_runtime()->tracer();
  } else {
    return tracer_;
  }
}
...
void Heap::PrintShortHeapStatistics() {
  ...
  PrintIsolate(isolate_, "Shared old space,          used: %6" PRIuS
                         " KB"
                         ", available: %6" PRIuS
                         " KB"
                         ", committed: %6" PRIuS " KB\n",
               shared_old_space_->SizeOfObjects() / KB,
               shared_old_space_->Available() / KB,
               shared_old_space_->CommittedMemory() / KB);

  // <YUNOS> changed: begin
  PrintIsolate(isolate_, "Mutable space,      used: %6" PRIuS
                         "d KB"
                         ", available: %6" PRIuS
                         "d KB"
                         ", committed: %6" PRIuS "d KB\n",
               mutable_space_->SizeOfObjects() / KB,
               mutable_space_->Available() / KB,
               mutable_space_->CommittedMemory() / KB);
  // <YUNOS> changed: end
  //
  ...
  PrintIsolate(isolate_, "Shared code space,         used: %6" PRIuS
                         " KB"
                         ", available: %6" PRIuS
                         " KB"
                         ", committed: %6" PRIuS "KB\n",
               shared_code_space_->SizeOfObjects() / KB,
               shared_code_space_->Available() / KB,
               shared_code_space_->CommittedMemory() / KB);
  ...
  PrintIsolate(isolate_, "Shared map space,          used: %6" PRIuS
                         " KB"
                         ", available: %6" PRIuS
                         " KB"
                         ", committed: %6" PRIuS " KB\n",
               shared_map_space_->SizeOfObjects() / KB,
               shared_map_space_->Available() / KB,
               shared_map_space_->CommittedMemory() / KB);
  ...
  PrintIsolate(isolate_, "Shared large object space, used: %6" PRIuS
                         " KB"
                         ", available: %6" PRIuS
                         " KB"
                         ", committed: %6" PRIuS " KB\n",
               shared_lo_space_->SizeOfObjects() / KB,
               shared_lo_space_->Available() / KB,
               shared_lo_space_->CommittedMemory() / KB);
  ...
  PrintIsolate(isolate_, "Total time spent in shared GC  : %.1f ms\n",
               isolate_->worker_runtime()->total_shared_gc_time());
...
void Heap::GarbageCollectionPrologue() {
  TRACE_GC(isolate()->worker_runtime()->tracer(), GCTracer::Scope::HEAP_PROLOGUE);
  {
...
size_t Heap::SizeOfSharedObjects() {
  size_t total = 0;
  SharedPagedSpaces spaces(this);
  for (Space* space = spaces.next(); space != NULL; space = spaces.next()) {
    total += reinterpret_cast<PagedSpace*>(space)
      ->SizeOfObjects();
  }
...
const char* Heap::GetSpaceName(int idx) {
  switch (idx) {
    ...
    case SHARED_OLD_SPACE:
      return "shared_old_space";
    case SHARED_CODE_SPACE:
      return "shared_code_space";
    case SHARED_MAP_SPACE:
      return "shared_map_space";
...
bool Heap::collect_shared_space() {
  return mark_compact_collector_ == NULL?
    false : mark_compact_collector_->collect_shared_space();
}
...
void Heap::GarbageCollectionEpilogue() {
  ...
  if (CommittedMemory() > 0) {
    ...
    // <YUNOS> changed: begin
    isolate_->counters()->heap_fraction_mutable_space()->AddSample(static_cast<int>(
        (mutable_space()->CommittedMemory() * 100.0) / CommittedMemory()));
    // <YUNOS> changed: end
...
#define UPDATE_COUNTERS_AND_FRAGMENTATION_FOR_SPACE(space) \
  ...
  // <YUNOS> changed: begin
  UPDATE_COUNTERS_AND_FRAGMENTATION_FOR_SPACE(mutable_space)
  // <YUNOS> changed: end
...
void Heap::CollectAllGarbage(int flags, GarbageCollectionReason gc_reason,
  ...
  if (gc_reason == GarbageCollectionReason::kDebugger) {
    space = SHARED_OLD_SPACE;
  }
  set_current_gc_flags(flags);
  CollectGarbage(space, gc_reason, gc_callback_flags);
...
void Heap::CollectAllAvailableGarbage(GarbageCollectionReason gc_reason) {
  ...
  // JSMT: TODO shared script table should be collected?
  isolate_->compilation_cache()->Clear();
  const int kMaxNumberOfAttempts = 7;
  const int kMinNumberOfAttempts = 2;
  // JSMT: TODO CollectGarbageForShared support return can collect_more
  if (gc_reason != GarbageCollectionReason::kLowMemoryNotification) {
    CollectGarbageForShared(gc_reason, "ForAllAvailableGarbage",
        v8::kGCCallbackFlagCollectAllAvailableGarbage);
  }
  // shared object's weak global handle need a all shared gc
...
// <YUNOS> changed:
bool Heap::ShouldDoFullGC() {
  // TODO: make condition more reasonable
  if (PromotedSinceLastGC() > (kThresholdForFullGC * Page::kPageSize)) {
    if(FLAG_trace_gc) {
      PrintF("Do full GC because promoted size since last gc has %zu B", PromotedSinceLastGC());
    }
    return true;
  }
  if(FLAG_trace_gc) {
    PrintF("Should not do full GC because promoted size since last gc has only %zu B", PromotedSinceLastGC());
  }
  return false;
}

bool Heap::ShouldDoScavengerGC() {
  // TODO: make condition more reasonable
  int count = 0;
  NewSpace* ns = new_space();
  for (auto it = ns->begin(); it != ns->end(); it++) {
    count++;
  }
  if (count > kThresholdForScavengerGC) {
    if(FLAG_trace_gc) {
      PrintF("Do scavenger GC because new space has %d pages", count);
    }
    return true;
  }
  if(FLAG_trace_gc) {
    PrintF("Should not do scavenger GC because new space has only %d pages", count);
  }
  return false;
}
...
void Heap::AdviceGC() {
  if(ShouldDoFullGC()) {
    PrintF("Do full GC for advice GC");
    CollectAllAvailableGarbage(i::GarbageCollectionReason::kAdviceGC);
  } else if(ShouldDoScavengerGC()) {
    PrintF("Do Scavenger GC advice GC");
    // TODO: prevent promote in new space
    CollectGarbage(NEW_SPACE,
                   GarbageCollectionReason::kAdviceGC,
                   v8::kGCCallbackFlagForced);
    new_space_->Shrink();
    UncommitFromSpace();
  } else {
    PrintF("Do nothing for advice GC");
  }
}

bool Heap::SwitchWorkStatus(WorkStatus type) {
  // Currently the parameter is not be used.We just reset the new space to free
  // the memory which used by newspace.This will save about 2MB if the app dont
  // new any object after calling this function.
  if (type == work_status_) {
    return false;
  }
  switch (type) {
    ...
    case STATUS_FOREGROUND:
         ...
         break;
    default:
         PrintF("Unsupported work status!\n");
         break;
  }
  return true;
}
...
void Heap::DoGcPrologueCallbacks(GCType type) {
  CHECK(isolate_->current_vm_state() == GC);
  GCCallbacksScope scope(this);
  if (scope.CheckReenter()) {
    AllowHeapAllocation allow_allocation;
    ...
    VMState<EXTERNAL> state(isolate_);
    HandleScope handle_scope(isolate_);
    // We do not introduct a GCType, but just use the one is same with mark-compact
    CallGCPrologueCallbacks(type, kNoGCCallbackFlags);
  }
}

void Heap::DoGcEpilogueCallbacks(GCType type,
    GCCallbackFlags gc_callback_flags) {
  if (isolate_->current_vm_state() == GC) {
    GCCallbacksScope scope(this);
    if (scope.CheckReenter()) {
      AllowHeapAllocation allow_allocation;
      ...
      VMState<EXTERNAL> state(isolate_);
      HandleScope handle_scope(isolate_);
      CallGCEpilogueCallbacks(type, gc_callback_flags);
    }
  }
}
...
void Heap::CollectGarbageForShared(GarbageCollectionReason gc_reason,
                                   const char* collector_reason,
                                   const v8::GCCallbackFlags gc_callback_flags) {
  WorkerRuntime* worker_runtime = isolate_->worker_runtime();
  // To Prevenet shared space gc during Isolate::Init
  sync::SafepointMutexLock mu(isolate_->vmthread(), WorkerRuntime::GetLock(worker_runtime,
          WorkerRuntime::kSharedHeap));
  // Mark gc thread enter GC state
  VMState<GC> state(isolate_);
  // JSMT:
  // Do gc prologue call backs
  // It is processed outof pause.
  // And is just called by gc thread.
  DoGcPrologueCallbacks(kGCTypeMarkSweepCompact);
  {
    // Stop the world
    PauseScope pause(isolate_->vmthread());
    worker_runtime->tracer()->Start(MARK_COMPACTOR_SHARED, gc_reason, collector_reason);

    TRACE_GC(worker_runtime->tracer(), GCTracer::Scope::MCS_PAUSE);
    // It is use to count the times during the C++ Call
    RuntimeCallTimerScope(isolate(), &RuntimeCallStats::GC);
    // Collect Shared Epilogue
    worker_runtime->ForAllWorkers([collector_reason, gc_reason](VMThread* thread) {
        Heap* heap = thread->isolate()->heap();
        heap->mark_compact_collector()->set_collect_shared_space(true);
        IsolateCurrentScope scs(thread->isolate());
        heap->CollectSharedPrologueStep1(gc_reason, collector_reason);
        });

    {
      HistogramTimer* gc_type_timer = GCTypeTimer(MARK_COMPACTOR_SHARED);
      HistogramTimerScope histogram_timer_scope(gc_type_timer);
      TRACE_EVENT0("v8", gc_type_timer->name());
      int start_new_space_size = 0;
      uint64_t size_of_objects_before_gc = 0;
      worker_runtime->ForAllWorkers([&start_new_space_size,
          &size_of_objects_before_gc](VMThread* thread) {
          IsolateCurrentScope scs(thread->isolate());
          thread->isolate()->heap()
          ->CollectSharedPrologueStep2(start_new_space_size,
              size_of_objects_before_gc, false);
          });

      mark_compact_collector()->PrepareForShared();

      worker_runtime->ForAllWorkers([](VMThread* thread) {
          IsolateCurrentScope scs(thread->isolate());
          thread->isolate()->heap()->MarkCompactPrologue();
          });
      // Do MarkCompact
      mark_compact_collector()->CollectGarbageForShared();

      // Compute the new shared space allocation limit in gc thread
      UpdateSharedSpaceAllocationLimit();

      size_t shared_space_allocation_limit = shared_spaces_allocation_limit();
      worker_runtime->ForAllWorkers([&start_new_space_size,
          &size_of_objects_before_gc,
          gc_callback_flags,
          shared_space_allocation_limit](VMThread* thread) {
          IsolateCurrentScope scs(thread->isolate());
          thread->isolate()->heap()
          ->CollectSharedEpilogueStep1(gc_callback_flags,
              start_new_space_size,
              size_of_objects_before_gc);
          // Update the shared space allocation limit
          // All heaps have a same shared space allocation limit
          thread->isolate()->heap()->
          UpdateSharedSpaceAllocationLimit(shared_space_allocation_limit);
      });
    }
    ...
    worker_runtime->ForAllWorkers([](VMThread* thread) {
        Heap* heap = thread->isolate()->heap();
        IsolateCurrentScope scs(thread->isolate());
        heap->CollectSharedEpilogueStep2();
        heap->mark_compact_collector()->set_collect_shared_space(false);
        });
    worker_runtime->tracer()->Stop(MARK_COMPACTOR_SHARED);
  }
  // JSMT:
  // Just called by gc thread once
  DoGcEpilogueCallbacks(kGCTypeMarkSweepCompact, gc_callback_flags);
...
// <YUNOS> changed: end
  ...
  if (collector == MARK_COMPACTOR_SHARED) {
    CollectGarbageForShared(gc_reason, collector_reason, gc_callback_flags);
    return true;
  }
...
static void VerifyStringTable(Heap* heap) {
  // JSMT: VerifyStringTable may has competition with other threads, but
  // it is ok, because the string table will not be freed
  StringTableVerifier verifier;
  heap->string_table()->IterateElements(&verifier);
}
#endif  // VERIFY_HEAP

bool Heap::ReserveSpace(Reservation* reservations, List<Address>* non_shared_maps,
                        List<Address>* shared_maps) {
  bool gc_performed = true;
  int counter = 0;
  static const int kThreshold = 20;
  while (gc_performed && counter++ < kThreshold) {
    gc_performed = false;
    for (int space = kSerializerNewSpace;
         // Add the SerializerSharedMapSpace
         space < SerializerDeserializer::kNumberOfReservations;
         space++) {
      Reservation* reservation = &reservations[space];
      bool is_shared = SerializerSpaceToSharability(
                       static_cast<SerializerSpace>(space)) == SHARED;
      DCHECK_LE(1, reservation->size());
      if (reservation->at(0).size == 0) continue;
      bool perform_gc = false;
      if (space == kSerializerMapSpace ||
          space == kSerializerSharedMapSpace) {
        // We allocate each map individually to avoid fragmentation.
        List<Address>* maps = is_shared ? shared_maps : non_shared_maps;
        maps->Clear();
        ...
        for (int i = 0; i < num_maps; i++) {
          ...
          MapSpace* space = is_shared ? shared_map_space() : map_space();
          if (is_shared && !isolate()->single_thread_mode()) {
            allocation = space->AllocateRawUnalignedSynchronized(
                Map::kSize, PagedSpace::IGNORE_SKIP_LIST);
          } else {
            allocation = space->AllocateRawUnaligned(
                Map::kSize, PagedSpace::IGNORE_SKIP_LIST);
          }
      ...
      } else if (space == kSerializerLoSpace) {
        ...
        for (auto& chunk : *reservation) {
          ...
          DCHECK_LE(static_cast<size_t>(size),
                    ...
                    SerializerSpaceToAllocationSpace(
                    static_cast<SerializerSpace>(space))));
          if (space == kSerializerNewSpace) {
            allocation = new_space()->AllocateRawUnaligned(size);
          } else {
            PagedSpace* paged_space;
            if (is_shared) {
              paged_space = (space == kSerializerSharedCodeSpace) ?
                            shared_code_space() : shared_old_space();
            } else {
              paged_space = (space == kSerializerCodeSpace) ?
                            code_space() : old_space();
            }
            // The deserializer will update the skip list.
            if (is_shared && !isolate()->single_thread_mode()) {
              allocation = paged_space->AllocateRawUnalignedSynchronized(
                  size, PagedSpace::IGNORE_SKIP_LIST);
            } else {
              allocation = paged_space->AllocateRawUnaligned(
                  size, PagedSpace::IGNORE_SKIP_LIST);
            }
          }
          HeapObject* free_space = nullptr;
...
void Heap::CollectSharedPrologueStep1(GarbageCollectionReason gc_reason,
                                      const char* collector_reason) {
  // The VM is in the GC state until exiting this function.
  // RuntimeCallTimerScope(isolate(), &RuntimeCallStats::GC);
  // EnsureFillerObjectAtTop();
  ...
  // Just Debug mode open
  DisallowHeapAllocation no_allocation_during_gc;
  GarbageCollectionPrologue();
}

void Heap::CollectSharedPrologueStep2(int& start_new_size,
                                      uint64_t& size_of_objects_before_gc,
                                      bool need_verify) {
  // Enter PerformGarbageCollection
  ...
  start_new_size = static_cast<int>(Heap::new_space()->Size());

  //Do the things is same to someting done
  //in PretenuringScope and SkipStoreBufferScope
  ...
  // <- MarkCompact()
  // Enter PauseAllocationObserversScope
  AllSpaces spaces(this);
  for (Space* space = spaces.next(); space != NULL; space = spaces.next()) {
    space->PauseAllocationObservers();
  }
  // Do things in MarkCompact
  SetGCState(MARK_COMPACT);
  LOG(isolate_, ResourceEvent("markcompact", "begin"));
  size_of_objects_before_gc = SizeOfObjects();
  mark_compact_collector()->Prepare(need_verify);
  ms_count_++;
}

void Heap::UpdateSharedSpaceAllocationLimit(size_t value) {
  if (value != 0) {
    set_shared_spaces_allocation_limit(value);
    return;
  }
  //TODO JSMT: use shared space's gc_speed
  double gc_speed = isolate()->worker_runtime()->tracer()->
    CombinedMarkCompactSharedSpeedInBytesPerMillisecond();
  double mutator_speed = isolate()->worker_runtime()->tracer()->
    CurrentSharedSpaceAllocationThroughputInBytesPerMillisecond();
  size_t shared_gen_size = SharedSpaceSizeOfObjects();

  SetGenerationAllocationLimit(shared_gen_size, gc_speed, mutator_speed);

  return;
}

void Heap::CollectSharedEpilogueStep1(const v8::GCCallbackFlags gc_callback_flags,
  ...
  // <- Leave PauseAllocationObserversScope
  AllSpaces spaces(this);
  for (Space* space = spaces.next(); space != NULL; space = spaces.next()) {
  ...
  // Exit SkipStoreBufferScope
  DCHECK(store_buffer_->Empty());
  store_buffer_->SetMode(StoreBuffer::NOT_IN_GC);
  // Exit PretenuringScope
  delete global_pretenuring_feedback_;
  global_pretenuring_feedback_ = nullptr;
  //<- Leave PretenuringScope
  ...
  // global_handles is not processed during full gc
  ...
  SetGenerationAllocationLimit(old_gen_size, gc_speed, mutator_speed);
...
#endif
  //<- Leve PerformGarbageCollection
}

void Heap::CollectSharedEpilogueStep2() {
  GarbageCollectionEpilogue();
  ...
  // - there is high fragmentation,
  // - there are live detached contexts.
  event.next_gc_likely_to_collect_more = false;
  event.committed_memory = committed_memory_after;
  if (deserialization_complete_) {
    memory_reducer_->NotifyMarkCompact(event);
  }
...
void Heap::EpilogueForShared() {
  isolate()->worker_runtime()->
    UpdateSharedSpaceSizeAtLastGc(SharedSpaceSizeOfObjects());
}
...
bool Heap::PerformGarbageCollection(GarbageCollector collector,
  ...
  DoGcEpilogueCallbacks(gc_type, gc_callback_flags);
...
void Heap::Scavenge() {
  ...
  Scavenger* scavengers[kMaxScavengerTasks] = {nullptr};
...
void Heap::ComputeFastPromotionMode(double survival_rate) {
  ...
  fast_promotion_mode_ =
      !isolate()->flags().optimize_for_size() && FLAG_fast_promotion_new_space &&
...
void Heap::UpdateNewSpaceReferencesInExternalStringTable(
  ...
  for (Object** p = start; p < end; ++p) {
    ...
    if (InNewSpace(target)) {
      ...
      *last = target;
      ++last;
    } else if (target->is_shared_object()) {
      isolate()->worker_runtime()->RegisterExternalString(target);
    } else {
...
void Heap:: UpdateReferencesInSharedExternalStringTable(
    ExternalStringTableUpdaterCallback updater_func) {
  WorkerRuntime* worker_runtime = isolate()->worker_runtime();

  class SharedExternalStringTableUpdater : public RootVisitor {
   public:
    explicit SharedExternalStringTableUpdater(
        ExternalStringTableUpdaterCallback update_func,
        Heap* heap)
    ...
    virtual void VisitRootPointers(Root root, Object** start, Object** end) {
      for (Object** p = start; p < end; p++) {
        *p = update_func_(heap_, p);
      }
    }

   private:
    ExternalStringTableUpdaterCallback update_func_;
    Heap* heap_;
  } external_string_table_visitor(updater_func, this);

  worker_runtime->IterateSharedExternalStringTable(&external_string_table_visitor);
}
...
void Heap::VisitExternalResources(v8::ExternalResourceVisitor* visitor) {
  ...
  WorkerRuntime* worker_runtime = isolate()->worker_runtime();
  worker_runtime->IterateSharedExternalStringTableWithLock(
      &external_string_table_visitor);
}
...
AllocationResult Heap::AllocatePartialMap(InstanceType instance_type,
  ...
  AllocationResult allocation = AllocateRaw(Map::kSize, SHARED_MAP_SPACE);
...
AllocationResult Heap::AllocateMap(InstanceType instance_type,
  ...
  AllocationResult allocation = AllocateRaw(Map::kSize,
                                  (sharable == SHARED) ? SHARED_MAP_SPACE : MAP_SPACE);
  if (!allocation.To(&result)) return allocation;
...
#undef ALLOCATE_PARTIAL_MAP
  ...
  {
    AllocationResult allocation = Allocate(null_map(), SHARED_OLD_SPACE);
    if (!allocation.To(&obj)) return false;
  }
  ...
  {
    AllocationResult allocation = Allocate(undefined_map(), SHARED_OLD_SPACE);
    if (!allocation.To(&obj)) return false;
  }
  ...
  {
    AllocationResult allocation = Allocate(the_hole_map(), SHARED_OLD_SPACE);
    if (!allocation.To(&obj)) return false;
  }
...
#define ALLOCATE_MAP(instance_type, size, field_name)                       \
  {                                                                         \
    Map* map;                                                               \
    if (!AllocateMap((instance_type), size, SHARED).To(&map)) return false; \
    set_##field_name##_map(map);                                            \
  }

#define ALLOCATE_NON_SHARED_MAP(instance_type, size, field_name)                \
  {                                                                             \
    Map* map;                                                                   \
    if (!AllocateMap((instance_type), size, NOT_SHARED).To(&map)) return false; \
    set_##field_name##_map(map);                                                \
  }
...
#define ALLOCATE_PRIMITIVE_MAP(instance_type, size, field_name, \
  ...
  {                                                             \
    ...
    ALLOCATE_NON_SHARED_MAP(JS_PROMISE_CAPABILITY_TYPE, JSPromiseCapability::kSize,
                 js_promise_capability);

    for (unsigned i = 0; i < arraysize(string_type_table); i++) {
      const StringTypeTable& entry = string_type_table[i];
      {
        AllocationResult allocation = AllocateMap(entry.type, entry.size, SHARED);
        if (!allocation.To(&obj)) return false;
      }
    ...
    {  // Create a separate external one byte string map for native sources.
      AllocationResult allocation =
          AllocateMap(SHORT_EXTERNAL_ONE_BYTE_STRING_TYPE,
                      ExternalOneByteString::kShortSize, SHARED);
      if (!allocation.To(&obj)) return false;
      Map* map = Map::cast(obj);
      map->SetConstructorFunctionIndex(Context::STRING_FUNCTION_INDEX);
...
#undef ALLOCATE_FIXED_TYPED_ARRAY_MAP
    ...
    for (unsigned i = 0; i < arraysize(struct_table); i++) {
      ...
      Map* map;
      if (!AllocateMap(entry.type, entry.size, SHARED).To(&map)) return false;
      roots_[entry.index] = map;
    }
...
#undef ALLOCATE_MAP
  ...
  {
    AllocationResult allocation = Allocate(boolean_map(), SHARED_OLD_SPACE);
    if (!allocation.To(&obj)) return false;
  }
  ...
  {  // Empty arrays
    {
      ...
      if (!AllocateByteArray(0, SHARED_TENURED).To(&byte_array)) return false;
      set_empty_byte_array(byte_array);
    }

    {
      PropertyArray* property_array;
      if (!AllocatePropertyArray(0, SHARED_TENURED).To(&property_array)) return false;
...
{
    AllocationSpace space;
    if (FLAG_use_mutable_space &&
        pretenure != SHARED_TENURED) {
      space = MUTABLE_SPACE;
    } else {
...
void Heap::CreateInitialObjects(bool is_worker) {
  HandleScope scope(isolate());
  Factory* factory = isolate()->factory();

  if (!is_worker) {
    // The -0 value must be set before NewNumber works.
    set_minus_zero_value(*factory->NewHeapNumber(-0.0, IMMUTABLE, SHARED_TENURED));
    DCHECK(std::signbit(minus_zero_value()->Number()) != 0);

    set_nan_value(*factory->NewHeapNumber(
          std::numeric_limits<double>::quiet_NaN(), IMMUTABLE, SHARED_TENURED));
    set_hole_nan_value(
        *factory->NewHeapNumberFromBits(kHoleNanInt64, IMMUTABLE, SHARED_TENURED));
    set_infinity_value(*factory->NewHeapNumber(V8_INFINITY, IMMUTABLE,
          SHARED_TENURED));
    set_minus_infinity_value(
        *factory->NewHeapNumber(-V8_INFINITY, IMMUTABLE, SHARED_TENURED));

    // Allocate
    isolate()->worker_runtime()->SetStringTableRoot(
  ...
  if (!is_worker) shared_code_stubs_cache()->EnsureCapacity(Smi::FromInt(0), 128);
...
#define SYMBOL_INIT(name, description)                                      \
  Handle<Symbol> name = factory->NewSymbol(SHARED_TENURED);                 \
...
#undef SYMBOL_INIT
  ...
  if (!is_worker) {
    Handle<WeakCell> cell = factory->NewWeakCell(
                     factory->undefined_value(), SHARED_TENURED);
  ...
  set_shared_script_list(Smi::kZero);
  ...
  set_shared_noscript_shared_function_infos(Smi::kZero);
  set_weak_wasm_data_table(Smi::kZero);
  set_weak_debug_points_table(Smi::kZero);
  ...
  isolate_->compilation_cache()->Clear(true);
  if (!is_worker) {
    isolate_->compilation_cache()->ClearSharedScriptTable();
  }
...
bool Heap::RootCanBeWrittenAfterInitialization(Heap::RootListIndex root_index) {
  switch (root_index) {
    ...
    case kSharedScriptListRootIndex:
    ...
    case kWeakWasmDataTableRootIndex:
    case kWeakDebugPointsTableRootIndex:
    case kSharedNoScriptSharedFunctionInfosRootIndex:
...
#undef SMI_ENTRY
    case kNewSpaceAllocationTopRootIndex:
    case kNewSpaceAllocationLimitRootIndex:
    case kOldSpaceAllocationTopRootIndex:
    case kOldSpaceAllocationLimitRootIndex:
    case kStoreBufferTopRootIndex:
    case kAllocationSitesListRootIndex:
    case kRecordWriteStubModeRootIndex:
      return true;
...
bool Heap::RootIsReference(Heap::RootListIndex root_index) {
  return root_index >= -1 && root_index < kSmiRootsStart;
}
...
AllocationResult Heap::AllocateBytecodeArray(int length,
  ...
  bool is_shared = isolate()->vmthread()->in_shared_codegen();
  int size = BytecodeArray::SizeFor(length);
  HeapObject* result = nullptr;
  {
    AllocationSpace space = is_shared ? SHARED_OLD_SPACE : OLD_SPACE;
    AllocationResult allocation = AllocateRaw(size, space);
    if (!allocation.To(&result)) return allocation;
  }
...
FixedArrayBase* Heap::LeftTrimFixedArray(FixedArrayBase* object,
  ...
  DCHECK(!object->is_shared_object());
...
AllocationResult Heap::AllocateCode(int object_size, bool immovable, bool is_shared) {
  DCHECK(IsAligned(static_cast<intptr_t>(object_size), kCodeAlignment));
  AllocationResult allocation = is_shared
                    ? AllocateRaw(object_size, SHARED_CODE_SPACE)
                    : AllocateRaw(object_size, CODE_SPACE);
  ...
  if (immovable) {
    ...
    OldSpace* code_space = is_shared ? shared_code_space_ : code_space_;
    if (!Heap::IsImmovable(result) &&
        !code_space->FirstPage()->Contains(address)) {
      if (isolate()->serializer_enabled()) {
        ...
        CreateFillerObjectAt(result->address(), object_size,
                             ClearRecordedSlots::kNo);
        LargeObjectSpace* lo_space = is_shared ? shared_lo_space_ : lo_space_;
        allocation = lo_space->AllocateRaw(object_size, EXECUTABLE);
        if (!allocation.To(&result)) return allocation;
...
AllocationResult Heap::CopyBytecodeArray(BytecodeArray* bytecode_array) {
  ...
  HeapObject* result = nullptr;
  bool is_shared = bytecode_array->is_shared_object();
  {
    AllocationResult allocation = AllocateRaw(size,
                                  is_shared ?
                                  SHARED_OLD_SPACE :
                                  OLD_SPACE);
    if (!allocation.To(&result)) return allocation;
...
AllocationResult Heap::AllocateInternalizedStringImpl(T t, int chars,
  ...
  HeapObject* result = nullptr;
  {
    AllocationResult allocation = AllocateRaw(size, SHARED_OLD_SPACE);
    if (!allocation.To(&result)) return allocation;
  }
...
AllocationResult Heap::AllocateEmptyFixedTypedArray(
    ExternalArrayType array_type) {
  return AllocateFixedTypedArray(0, array_type, false, SHARED_TENURED);
}
...
AllocationResult Heap::AllocateSymbol(PretenureFlag pretenure) {
  // Statically ensure that it is safe to allocate symbols in paged spaces.
  STATIC_ASSERT(Symbol::kSize <= kMaxRegularHeapObjectSize);

  HeapObject* result = nullptr;
  AllocationSpace space = SelectSpace(pretenure);
  AllocationResult allocation = AllocateRaw(Symbol::kSize, space);
  if (!allocation.To(&result)) return allocation;
...
bool Heap::ShouldOptimizeForMemoryUsage() {
  return isolate()->flags().optimize_for_size() ||
...
void Heap::RegisterDeserializedObjectsForBlackAllocation(
  ...
  for (int i = kSerializerOldSpace; i < Serializer::kNumberOfReservations; i++) {
...
void Heap::ReportHeapStatistics(const char* title) {
  ...
  // <YUNOS> changed: begin
  PrintF("Mutable space : ");
  mutable_space_->ReportStatistics();
  // <YUNOS> changed: end
  PrintF("Code space : ");
...
const char* Heap::GarbageCollectionReasonToString(
  ...
  switch (gc_reason) {
    ...
    case GarbageCollectionReason::kAdviceGC:
      return "advice gc";
...
bool Heap::Contains(HeapObject* value) {
  ...
  return HasBeenSetUp() &&
         (new_space_->ToSpaceContains(value) || old_space_->Contains(value) ||
          ...
          mutable_space_->Contains(value) ||
          lo_space_->Contains(value) ||
          SharedContains(value));
}
...
bool Heap::ContainsSlow(Address addr) {
  ...
  return HasBeenSetUp() &&
         (new_space_->ToSpaceContainsSlow(addr) ||
          ...
          mutable_space_->ContainsSlow(addr) ||
          SharedContainsSlow(addr));
}


bool Heap::SharedContains(HeapObject* value) {
  if (memory_allocator()->IsOutsideAllocatedSpace(value->address())) {
    return false;
  }
  return HasBeenSetUp() &&
         (shared_old_space_->Contains(value) ||
          shared_code_space_->Contains(value) ||
          shared_map_space_->Contains(value) ||
          shared_lo_space_->Contains(value));
}

bool Heap::SharedContainsSlow(Address addr) {
  if (memory_allocator()->IsOutsideAllocatedSpace(addr)) {
    return false;
  }
  return HasBeenSetUp() &&
         (shared_old_space_->ContainsSlow(addr) ||
          shared_code_space_->ContainsSlow(addr) ||
          shared_map_space_->ContainsSlow(addr) ||
          shared_lo_space_->ContainsSlow(addr));
}
// <YUNOS> changed: end
...
bool Heap::InSpace(HeapObject* value, AllocationSpace space) {
  ...
  switch (space) {
    ...
    // <YUNOS> changed: begin
    case MUTABLE_SPACE:
      return mutable_space_->Contains(value);
    // <YUNOS> changed: end
      return old_space_->Contains(value);
    case CODE_SPACE:
      return code_space_->Contains(value);
    case SHARED_CODE_SPACE:
      return shared_code_space_->Contains(value);
    case MAP_SPACE:
      return map_space_->Contains(value);
    case SHARED_MAP_SPACE:
      return shared_map_space_->Contains(value);
    case SHARED_LO_SPACE:
...
bool Heap::InSpaceSlow(Address addr, AllocationSpace space) {
  ...
  switch (space) {
    ...
    case SHARED_OLD_SPACE:
      return shared_old_space_->ContainsSlow(addr);
    // <YUNOS> changed: begin
    case MUTABLE_SPACE:
      return mutable_space_->ContainsSlow(addr);
    // <YUNOS> changed: end
    case CODE_SPACE:
      return code_space_->ContainsSlow(addr);
    case SHARED_CODE_SPACE:
      return shared_code_space_->ContainsSlow(addr);
    case MAP_SPACE:
      return map_space_->ContainsSlow(addr);
    case SHARED_MAP_SPACE:
      return shared_map_space_->ContainsSlow(addr);
    case SHARED_LO_SPACE:
...
void Heap::Verify() {
  ...
  old_space_->Verify(&visitor, this);
  map_space_->Verify(&visitor, this);
  mutable_space_->Verify(&visitor, this);

  VerifyPointersVisitor no_dirty_regions_visitor;
  code_space_->Verify(&no_dirty_regions_visitor, this);

  lo_space_->Verify();
  if (ShouldProcessShared()) {
    shared_old_space_->Verify(&visitor, this);
    shared_map_space_->Verify(&visitor, this);
    shared_code_space_->Verify(&no_dirty_regions_visitor, this);
    shared_lo_space_->Verify();
  }
 ...
 public:
  ...
  bool ShouldHaveBeenRecorded(HeapObject* host, Object* target) override {
    ...
    return target->IsHeapObject() && heap_->InNewSpace(target) &&
           ...
           /* see WeakCell::set_next it skip UPDATE_WEAK_WRITE_BARRIER*/
           !(host->IsWeakCell() && target->IsWeakCell());
  }
...
void Heap::IterateWeakRoots(RootVisitor* v, VisitMode mode) {
  ...
  WorkerRuntime* worker_runtime = isolate()->worker_runtime();
  worker_runtime->IterateStringTableRoots(v);
  v->Synchronize(VisitorSynchronization::kStringTable);
  if (!isMinorGC && mode != VISIT_ALL_IN_SWEEP_NEWSPACE) {
    // Scavenge collections have special processing for this.
    external_string_table_.IterateAll(v);
    if  (mode != VISIT_ALL_WITHOUT_SHARED) {
      worker_runtime->IterateSharedExternalStringTable(v);
    }
  }
...
void Heap::IterateStrongRoots(RootVisitor* v, VisitMode mode) {
  const bool isMinorGC = mode == VISIT_ALL_IN_SCAVENGE ||
                         mode == VISIT_ALL_IN_MINOR_MC_MARK ||
                         mode == VISIT_ALL_IN_MINOR_MC_UPDATE;
  const bool visitShared = !isMinorGC &&
      mode != VISIT_ONLY_STRONG_WITHOUT_SHARED &&
      mode != VISIT_ALL_WITHOUT_SHARED;
  ...
  if (visitShared) {
    shared_code_stubs_cache()->Iterate(v, mode);
    v->Synchronize(VisitorSynchronization::kCodeStubs);
  }
  ...
  switch (mode) {
    ...
    case VISIT_ONLY_STRONG_FOR_DESERIALIZATION:
    case VISIT_ONLY_STRONG_WITHOUT_SHARED:
  ...
  if (visitShared) {
    isolate()->worker_runtime()->IterateRuntimeRoots(v);
  }
...
bool Heap::ConfigureHeap(size_t max_semi_space_size_in_kb,
  ...
  if (max_old_space_capacity != 0) {
    max_old_generation_capacity_ = static_cast<intptr_t>(max_old_space_capacity) * MB;
  }
  ...
  if (FLAG_max_old_space_capacity > 0) {
    if (FLAG_max_old_space_capacity < FLAG_max_old_space_size) {
      max_old_generation_capacity_ =
          static_cast<intptr_t>(FLAG_max_old_space_size) * MB;
    } else {
     max_old_generation_capacity_ =
         static_cast<intptr_t>(FLAG_max_old_space_capacity) * MB;
    }
  }
  if (FLAG_max_shared_space_size > 0) {
    max_shared_spaces_size_ =
        static_cast<size_t>(FLAG_max_shared_space_size) * MB;
  }
  ...
  if (Page::kPageSize > MB) {
    ...
    max_shared_spaces_size_ =
        ROUND_UP(max_shared_spaces_size_, Page::kPageSize);
  }
  ...
  int paged_space_count = LAST_PRIVATE_PAGED_SPACE - FIRST_PAGED_SPACE + 1;
  initial_max_old_generation_size_ = max_old_generation_size_ =
      Max(static_cast<size_t>(paged_space_count * Page::kPageSize),
          max_old_generation_size_);

  int shared_paged_space_count = LAST_PAGED_SPACE - FIRST_SHARED_SPACE + 1;
  max_shared_spaces_size_ =
      Max(static_cast<size_t>(shared_paged_space_count * Page::kPageSize),
          max_shared_spaces_size_);
  ...
  old_generation_allocation_limit_ = initial_old_generation_size_;
  shared_spaces_allocation_limit_ =
    max_shared_spaces_size_ / kInitalSharedSpacesLimitFactor;
...
void Heap::RecordStats(HeapStats* stats, bool take_snapshot) {
  ...
  *stats->old_space_size = old_space_->SizeOfObjects()
                           + shared_old_space_->SizeOfObjects()
                           + mutable_space_->SizeOfObjects();
  *stats->old_space_capacity = old_space_->Capacity()
                           + shared_old_space_->Capacity()
                           + mutable_space_->Capacity();
  *stats->code_space_size = code_space_->SizeOfObjects()
                           + shared_code_space_->SizeOfObjects();
  *stats->code_space_capacity = code_space_->Capacity()
                           + shared_code_space_->Capacity();
  *stats->map_space_size = map_space_->SizeOfObjects()
                           + shared_map_space_->SizeOfObjects();
...
size_t Heap::SharedSpaceSizeOfObjects() {
  return shared_old_space_->SizeOfObjects() + shared_code_space_->SizeOfObjects() +
         shared_map_space_->SizeOfObjects() + shared_lo_space_->SizeOfObjects();
}
...
double Heap::kMinHeapGrowingFactor = 1.01;
double Heap::kMaxHeapGrowingFactor = 1.05;
double Heap::kMaxHeapGrowingFactorMemoryConstrained = 1.03;
...
double Heap::MaxSharedHeapGrowingFactor(size_t max_shared_space_size) {
  const double min_small_factor = 1.3;
  const double max_small_factor = 2.0;
  const double high_factor = 4.0;

  size_t max_shared_spaces_size_in_mb = max_shared_space_size / MB;
  max_shared_spaces_size_in_mb =
      Max(max_shared_spaces_size_in_mb,
          static_cast<size_t>(kMinSharedSpacesSize));

  // If we are on a device with lots of memory, we allow a high heap
  // growing factor.
  if (max_shared_spaces_size_in_mb >= kMaxSharedSpacesSize) {
    return high_factor;
  }

  DCHECK_GE(max_shared_spaces_size_in_mb, kMinSharedSpacesSize);
  DCHECK_LT(max_shared_spaces_size_in_mb, kMaxSharedSpacesSize);

  // On smaller devices we linearly scale the factor: (X-A)/(B-A)*(D-C)+C
  double factor = (max_shared_spaces_size_in_mb - kMinSharedSpacesSize) *
                      (max_small_factor - min_small_factor) /
                      (kMaxSharedSpacesSize - kMinSharedSpacesSize) +
                  min_small_factor;
  return factor;
}

double Heap::MaxHeapGrowingFactor(size_t max_old_generation_size) {
  const double min_small_factor = 1.01;
  const double max_small_factor = 1.03;
  const double high_factor = 1.05;
...
size_t Heap::CalculateSharedSpaceAllocationLimit(double factor,
                                                   size_t shared_gen_size) {
  CHECK(factor > 1.0);
  CHECK(shared_gen_size > 0);
  uint64_t limit = static_cast<uint64_t>(shared_gen_size * factor);
  limit = Max(limit, static_cast<uint64_t>(shared_gen_size) +
                         MinimumAllocationLimitGrowingStep());
  uint64_t halfway_to_the_max =
      (static_cast<uint64_t>(shared_gen_size) + max_shared_spaces_size_) / 2;
  return static_cast<size_t>(Min(limit, halfway_to_the_max));
}
...
void Heap::SetGenerationAllocationLimit(size_t gen_size, double gc_speed,
                                           double mutator_speed, Sharability shared) {
  double max_factor = (shared == SHARED) ?
                      MaxSharedHeapGrowingFactor(max_shared_spaces_size_) :
                      MaxHeapGrowingFactor(max_old_generation_size_);
  ...
  if (shared != SHARED) {
    old_generation_allocation_limit_ =
      CalculateOldGenerationAllocationLimit(factor, gen_size);
  } else {
    shared_spaces_allocation_limit_ =
      CalculateSharedSpaceAllocationLimit(factor, gen_size);
  }

  if (FLAG_trace_gc_verbose) {
    isolate_->PrintWithTimestamp(
        "Grow: %s size: %" PRIuS " KB, new limit: %" PRIuS " KB (%.1f)\n",
        (shared == SHARED) ? "shared" : "old",
        gen_size / KB,
        old_generation_allocation_limit_ / KB,
        factor);
  }
}
...
bool Heap::ShouldExpandSharedSpaceOnSlowAllocation() {
  if (always_allocate() || SharedSpaceAvailable() > 0) return true;

  if (ShouldOptimizeForMemoryUsage()) return false;

  if (ShouldOptimizeForLoadTime()) return true;

  // JSMT: shared gc do not care about incremeantal marking
  return false;
}
...
bool Heap::SetUp() {
  ...
  if (isolate_->is_main_isolate()) {
    memory_allocator_ = new MemoryAllocator(isolate_);
    if (!memory_allocator_->SetUp(MaxReserved(), code_range_size_)) return false;
  } else {
    WorkerRuntime* worker_runtime = isolate_->worker_runtime();
    Heap* main_heap = worker_runtime->main_isolate()->heap();
    memory_allocator_ = main_heap->memory_allocator();
    ConfigureWorkerHeap();
  }

  unmapper_ = new Unmapper(memory_allocator_, this);
  ...
  new_space_->set_allocation_info(&new_space_allocation_info_);
  ...
  old_space_->set_allocation_info(&old_space_allocation_info_);
  if (!old_space_->SetUp()) return false;

  // <YUNOS> changed: begin
  // Initialize mutable space.
  space_[MUTABLE_SPACE] = mutable_space_ =
      new MutableSpace(this, MUTABLE_SPACE, NOT_EXECUTABLE);
  if (!mutable_space_->SetUp()) return false;
  // <YUNOS> changed: end
  ...
  // Set up for shared space
  if (isolate_->is_main_isolate()) {
    space_[SHARED_OLD_SPACE] = shared_old_space_ =
                 new OldSpace(this, SHARED_OLD_SPACE, NOT_EXECUTABLE);
    if (!shared_old_space_->SetUp()) return false;
    shared_old_space_->mark_shared();
    space_[SHARED_CODE_SPACE] = shared_code_space_ =
                 new OldSpace(this, SHARED_CODE_SPACE, EXECUTABLE);
    if (!shared_code_space_->SetUp()) return false;
    shared_code_space_->mark_shared();
    space_[SHARED_MAP_SPACE] = shared_map_space_ =
                 new MapSpace(this, SHARED_MAP_SPACE);
    if (!shared_map_space_->SetUp()) return false;
    shared_map_space_->mark_shared();
    space_[SHARED_LO_SPACE] = shared_lo_space_ = new LargeObjectSpace(this, SHARED_LO_SPACE);
    if (!shared_lo_space_->SetUp()) return false;
    shared_lo_space_->mark_shared();

    shared_observer_ = new SharedObserver(*this, IncrementalMarking::kAllocatedThreshold);
    shared_old_space()->AddAllocationObserver(shared_observer_);
    shared_code_space()->AddAllocationObserver(shared_observer_);
    shared_map_space()->AddAllocationObserver(shared_observer_);
    shared_lo_space()->AddAllocationObserver(shared_observer_);
    set_shared_code_stubs_cache(new CodeHashMap(isolate_));
  } else {
    WorkerRuntime* worker_runtime = isolate_->worker_runtime();
    Heap* main_heap = worker_runtime->main_isolate()->heap();
    space_[SHARED_OLD_SPACE] = shared_old_space_ = main_heap->shared_old_space();
    space_[SHARED_CODE_SPACE] = shared_code_space_ = main_heap->shared_code_space();
    space_[SHARED_MAP_SPACE] = shared_map_space_ = main_heap->shared_map_space();
    space_[SHARED_LO_SPACE] = shared_lo_space_ = main_heap->shared_lo_space();
    set_shared_code_stubs_cache(main_heap->shared_code_stubs_cache());
  }
...
void Heap::InitializeHashSeed() {
  if (FLAG_test_d8) {
    ...
    set_hash_seed(Smi::FromInt(0x12347));
  }
}
...
bool Heap::CreateWorkerHeapObjects(Isolate* main_isolate) {
  // Create initial maps.
  Object** heap_roots = main_isolate->heap()->roots_array_start();
  for (int i = 0; i < Heap::kStrongRootListLength; i++) {
    roots_[i] = heap_roots[i];
  }
  // Worker's hash seed should use main isolate's, so
  // string table lookup key will find the same value
  roots_[kHashSeedRootIndex] = heap_roots[kHashSeedRootIndex];

  Map* map;
  if (!AllocateMap(JS_PROMISE_CAPABILITY_TYPE,
                   JSPromiseCapability::kSize).To(&map))
    return false;
  set_js_promise_capability_map(map);
  if (!CreateApiObjects()) return false;

  // Create initial objects
  CreateInitialObjects(true);
  CHECK_EQ(0u, gc_count_);
  ...
  WorkerRuntime* worker_runtime = isolate_->worker_runtime();
  {
    sync::SafepointMutexLock mu(isolate_->vmthread(),
        WorkerRuntime::GetLock(worker_runtime, WorkerRuntime::kScriptList));
    set_shared_script_list(main_isolate->heap()->shared_script_list());
  }
  {
    sync::SafepointMutexLock mu(isolate_->vmthread(),
        WorkerRuntime::GetLock(worker_runtime,
        WorkerRuntime::kNoScriptSharedFunctionInfos));
    set_shared_noscript_shared_function_infos(shared_noscript_shared_function_infos());
  }
  return true;
}
...
void Heap::NotifyDeserializationComplete() {
  ...
  DCHECK(isolate()->is_main_isolate());
  PagedSpaces spaces(this);
...
void Heap::TearDown() {
  ...
  if (isolate_->is_main_isolate()) {
    isolate_->worker_runtime()->TearDownSharedExternalStringTable();
  }
  ...
  // <YUNOS> changed: begin
  if (mutable_space_ != NULL) {
    delete mutable_space_;
    mutable_space_ = NULL;
  }
  // <YUNOS> changed: end
  ...
  if (isolate_->is_main_isolate()) {
    shared_old_space()->RemoveAllocationObserver(shared_observer_);
    shared_code_space()->RemoveAllocationObserver(shared_observer_);
    shared_map_space()->RemoveAllocationObserver(shared_observer_);
    delete shared_observer_;
    shared_observer_ = nullptr;
    if (shared_old_space_ != NULL) {
      delete shared_old_space_;
      shared_old_space_ = NULL;
    }

    if (shared_code_space_ != NULL) {
      delete shared_code_space_;
      shared_code_space_ = NULL;
    }

    if (shared_map_space_ != NULL) {
      delete shared_map_space_;
      shared_map_space_ = NULL;
    }

    if (shared_lo_space_ != NULL) {
      shared_lo_space_->TearDown();
      delete shared_lo_space_;
      shared_lo_space_ = NULL;
    }

    delete shared_code_stubs_cache_;
    shared_code_stubs_cache_ = NULL;
  }
...
void Heap::CompactWeakFixedArrays() {
  ...
  if (ShouldProcessShared()) {
    CompactWeakFixedArray(shared_noscript_shared_function_infos());
    CompactWeakFixedArray(shared_script_list());
  }
  CompactWeakFixedArray(weak_stack_trace_list());
  if (weak_wasm_data_table() != Smi::kZero) {
    WeakHashTable::cast(weak_wasm_data_table())->Compact(isolate());
  }
  if (weak_debug_points_table() != Smi::kZero) {
    WeakHashTable::cast(weak_debug_points_table())->Compact(isolate());
  }
}
...
void Heap::ClearRecordedSlotRange(Address start, Address end) {
  ...
  if (!page->InNewSpace()) {
    DCHECK(page->owner()->identity() == OLD_SPACE ||
          page->owner()->identity() == SHARED_OLD_SPACE);
...
Space* AllSpaces::next() {
  switch (counter_++) {
    ...
    case SHARED_OLD_SPACE:
      return heap_->shared_old_space();
    case SHARED_CODE_SPACE:
      return heap_->shared_code_space();
    case SHARED_MAP_SPACE:
      return heap_->shared_map_space();
    case SHARED_LO_SPACE:
...
PagedSpace* PrivatePagedSpaces::next() {
  switch (counter_++) {
    case OLD_SPACE:
      return heap_->old_space();
    case MUTABLE_SPACE:
...
PagedSpace* SharedPagedSpaces::next() {
  switch (counter_++) {
    case SHARED_OLD_SPACE:
      return heap_->shared_old_space();
    case SHARED_CODE_SPACE:
...
HeapIterator::HeapIterator(Heap* heap,
    ...
    : pause_scope_(heap->isolate()->vmthread()),
      no_heap_allocation_(),
      heap_(heap),
  ...
  for (int i = 0; i < new_space_strings_.length(); ++i) {
    ...
    if (o->IsThinString()) {
      ...
      if (!o->IsExternalString() ||
          HeapObject::cast(o)->is_shared_object()) continue;
    }
    // SharedExternalString is processed
    // by SharedExternalStringTable TearDown
    DCHECK(!HeapObject::cast(o)->is_shared_object());
...
void VerifyPointersVisitor::VerifyPointers(Object** start, Object** end) {
  for (Object** current = start; current < end; current++) {
    if ((*current)->IsHeapObject()) {
      ...
      CHECK(object->GetHeap()->Contains(object));
...
void VerifySharedObjectVisitor::VisitPointers(HeapObject* host, Object** start,
                                                Object** end) {
  VerifyPointers(start, end);
}

void VerifySharedObjectVisitor::VisitRootPointers(Root root, Object** start,
                                                    Object** end) {
  VerifyPointers(start, end);
}

void VerifySharedObjectVisitor::VerifyPointers(Object** start, Object** end) {
  for (Object** current = start; current < end; current++) {
    if ((*current)->IsHeapObject()) {
      HeapObject* object = HeapObject::cast(*current);
      CHECK(object->GetIsolate()->heap()->InSharedSpace(object));
    }
  }
}
...
bool Heap::AllowedToBeMigrated(HeapObject* obj, AllocationSpace dst) {
  ...
  switch (src) {
    ...
    case SHARED_OLD_SPACE:
      return dst == src &&
             (dst == SHARED_OLD_SPACE || obj->IsFiller() || obj->IsExternalString());
    case CODE_SPACE:

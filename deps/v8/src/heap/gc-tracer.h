...
#define TRACER_SCOPES(F)                            \
  ...
  F(MC_SWEEP_MUTABLE)                               \
  /* <YUNOS> changed: end */                        \
  F(MC_SWEEP_CODE_SHARED)                           \
  F(MC_SWEEP_MAP_SHARED)                            \
  F(MC_SWEEP_OLD_SHARED)                            \
  F(MCS_PAUSE)                                      \
  F(MINOR_MC)                                       \
  F(MINOR_MC_CLEAR)                                 \
...
#undef DEFINE_SCOPE
  ...
  class Event {
   public:
    ...
    // Size of objects in shared heap in constructor
    size_t start_shared_object_size;

    // Size of objects in shared heap in destructor
    size_t end_shared_object_size;
  ...
  void SampleAllocation(double current_ms, size_t new_space_counter_bytes,
                        ...
                        size_t shared_spaces_counter_bytes);
  ...
  double CombinedMarkCompactSharedSpeedInBytesPerMillisecond();
  ...
  // last time_ms milliseconds.
  // Returns 0 if no allocation events have been recorded.
  double SharedSpaceAllocationThroughputInBytesPerMillisecond(
      double time_ms = 0) const;
  ...
  double CurrentSharedSpaceAllocationThroughputInBytesPerMillisecond();
 ...
 private:
  ...
  size_t shared_space_allocation_counter_bytes_;
  ...
  size_t shared_space_allocation_in_bytes_since_gc_;
  ...
  base::RingBuffer<BytesAndDuration> recorded_shared_mark_compacts_;
  ...
  base::RingBuffer<BytesAndDuration> recorded_shared_space_allocations_;

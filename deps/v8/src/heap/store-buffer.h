...
class StoreBuffer {
 public:
  ...
  void InsertDeletionIntoStoreBuffer(Address start, Address end) {
    Address* top = heap_->store_buffer_top_;
    if (top + sizeof(Address) * 2 > limit_[current_]) {
      StoreBufferOverflow(heap_->isolate());
      top = heap_->store_buffer_top_;
    }
    *top = MarkDeletionAddress(start);
    top++;
    *top = end;
    top++;
    heap_->store_buffer_top_ = top;
  }

  static void InsertDuringGarbageCollection(StoreBuffer* store_buffer,
                                            Address slot) {
    DCHECK(store_buffer->mode() != StoreBuffer::NOT_IN_GC);
    CHECK(!Page::FromAddress(slot)->InSharedSpace());
  ...
  void InsertIntoStoreBuffer(Address slot) {
    Address* top = heap_->store_buffer_top_;
    if (top + sizeof(Address) > limit_[current_]) {
      StoreBufferOverflow(heap_->isolate());
      top = heap_->store_buffer_top_;
    }
    *top = slot;
  ...
  bool Empty() {
    ...
    return heap_->store_buffer_top_ == start_[current_];
  }

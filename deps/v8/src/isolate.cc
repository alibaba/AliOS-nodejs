...
bool ThreadLocalTop::AddressIsReference(ThreadLocalTop::AddressId index) {
  switch(index) {
#define CASE(CamelName, hacker_name, is_reference) \
    case k##CamelName##Address: \
      return is_reference;
    FOR_EACH_THREAD_LOCAL_OBJECT_ADDRESS_NAME(CASE)
#undef CASE
    default:
      return false;
  }
...
void Isolate::InitializeOncePerProcess() {
  ...
  WorkerRuntime::InitializeOncePerProcess();
}
...
class CaptureStackTraceHelper {
 public:
  ...
  Handle<StackFrameInfo> NewStackFrameObject(
    ...
    Object* maybe_cache = nullptr;
    Handle<UnseededNumberDictionary> cache;
    bool use_source_position_cache = !FLAG_optimize_for_size &&
        !summ.abstract_code()->source_position_table()->is_shared_object();
    if (!isolate_->flags().optimize_for_size() &&
          use_source_position_cache) {
...
// <YUNOS> changed: begin
#if DEBUG
bool Isolate::KeyCreated() {
  return (base::Relaxed_Load(&isolate_key_created_) == 1);
}
#endif


// Add an array-buffer-allocator
bool Isolate::AddArrayBufferAllocator(v8::ArrayBuffer::Allocator *allocator) {
  const int id = GenerateArrayBufferAllocatorID();
  if (id >= 0) {
    ArrayBufferAllocatorListNode* node = new ArrayBufferAllocatorListNode;
    node->allocator_ = allocator;
    node->next_ = array_buffer_allocator_list_head_;
    node->id_ = id;
    array_buffer_allocator_list_head_ = node;
    return true;
  }
  return false;
}
...
// Delete the array-buffer-allocator-list, please
// note that we won't delete the allocator from outside.
void Isolate::DeleteArrayBufferAllocatorList() {
  while (array_buffer_allocator_list_head_ != NULL) {
    ArrayBufferAllocatorListNode* next =
        array_buffer_allocator_list_head_->next_;
    delete array_buffer_allocator_list_head_;
    array_buffer_allocator_list_head_ = next;
  }
}


// generator array-buffer-allocator's id
int Isolate::GenerateArrayBufferAllocatorID() {
  const int max_id = JSArrayBuffer::kMaxExternalAllocatorCount - 1;
  ++array_buffer_allocator_id_generator_;
  if (array_buffer_allocator_id_generator_ <= max_id) {
    return array_buffer_allocator_id_generator_;
  } else {
    PrintF("register too much array buffer allocators, "
           "v8 support %d allocator at most!\n", max_id + 1);
    return -1;
  }
}


// default get the top allocator in allocator-list
v8::ArrayBuffer::Allocator* Isolate::array_buffer_allocator(int id) {
  if (array_buffer_allocator_list_head_ == NULL) {
    return NULL;
  } else if (id < 0) {
    return array_buffer_allocator_list_head_->allocator_;
  }
  for (ArrayBufferAllocatorListNode* node = array_buffer_allocator_list_head_;
       node != NULL;
       node = node->next_) {
    if (node->id_ == id) return node->allocator_;
  }
  return NULL;
}
// <YUNOS> changed: end
...
Isolate::Isolate(bool enable_serializer, WorkerRuntime* worker_runtime)
    : embedder_data_(),
      regexp_stack_(),
      ...
      array_buffer_allocator_list_head_(NULL),
      array_buffer_allocator_id_generator_(-1),
  ...
  if (worker_runtime_ == nullptr) {
    worker_runtime_ = new WorkerRuntime(this);
    vm_thread_.MarkMainThreadFlags();
  }
  vm_thread_.thread_list_ = worker_runtime_->thread_list();
  base::Thread::SetThreadLocal(WorkerRuntime::runtime_key_, worker_runtime_);
...
void Isolate::TearDown() {
  ...
  if (is_main_isolate()) worker_runtime_->TearDown();

  {
    // We do not use ThreadStateChangeScope because vmthread will be delete before
    // ThreadStateChangeScope exited
    vm_thread_.TransitionFromRunnableToSuspended(kWait);
    sync::MutexLock mu(NULL, WorkerRuntime::GetLock(worker_runtime_,
          WorkerRuntime::kSharedHeap));
    vm_thread_.TransitionFromSuspendedToRunnable();
  ...
  DeleteArrayBufferAllocatorList();
  vm_thread_.Destroy();
  }
  delete this;
...
void Isolate::SetIsolateThreadLocals(Isolate* isolate,
  ...
  if (isolate != nullptr) {
    base::Thread::SetThreadLocal(WorkerRuntime::runtime_key_, isolate->worker_runtime());
  } else {
    base::Thread::SetThreadLocal(WorkerRuntime::runtime_key_, nullptr);
  }
...
Isolate::~Isolate() {
  ...
  regexp_stack_.thread_local_.Clear();
  ...
  if (is_main_isolate()) delete worker_runtime_;
  worker_runtime_ = NULL;
...
#if USE_SIMULATOR
  Simulator::TearDown(simulator_i_cache_, nullptr);
...
#define THREAD_LOCAL_STATIC_ASSERT(CamelName, hacker_name, is_reference) \
  DCHECK(                                                                \
    (reinterpret_cast<uintptr_t>(&thread_local_top_.hacker_name) -       \
    reinterpret_cast<uintptr_t>(&thread_local_top_)) ==                  \
    ThreadLocalTop::k##CamelName##Address * kPointerSize);
  FOR_EACH_THREAD_LOCAL_OBJECT_ADDRESS_NAME(THREAD_LOCAL_STATIC_ASSERT)
#undef THREAD_LOCAL_STATIC_ASSERT
  ...
  if (is_worker_isolate()) {
    compilation_cache_->SetSharedScriptTable(
        worker_runtime_->main_isolate()->compilation_cache());
  }
  ...
  jsregexp_static_offsets_vector_address_ = &jsregexp_static_offsets_vector_[0];
  // Enable logging before setting up the heap
  logger_->SetUp(this);
  vm_thread_.AllocateThreadId();
  ...
  date_cache_stamp_address_ = reinterpret_cast<Address>(
                              date_cache()->stamp_address());
  debug_is_active_address_ = debug()->is_active_address();
  debug_hook_on_function_call_address_ = debug()->hook_on_function_call_address();
  debug_last_step_action_address_ = debug()->last_step_action_address();
  debug_suspended_generator_address_ = debug()->suspended_generator_address();
  debug_restart_fp_address_ = debug()->restart_fp_address();
...
#undef INTERFACE_DESCRIPTOR
  ...
  {
    sync::SafepointMutexLock mu(vmthread(),
        WorkerRuntime::GetLock(worker_runtime(),
        WorkerRuntime::kSharedHeap));
    if (create_heap_objects) {
      bool success = is_main_isolate() ? heap_.CreateHeapObjects() :
        heap_.CreateWorkerHeapObjects(worker_runtime_->main_isolate());
      if (!success) {
    ...
    if (is_main_isolate()) {
      ...
      // Set up the workerruntime's gc tracer
      worker_runtime()->set_tracer(new GCTracer(&heap_));
      if (create_heap_objects) heap_.CreateFixedStubs();
    } else {
      Heap* main_heap = worker_runtime_->main_isolate()->heap();
      heap_.set_arguments_adaptor_deopt_pc_offset(
          main_heap->arguments_adaptor_deopt_pc_offset());
      heap_.set_construct_stub_create_deopt_pc_offset(
          main_heap->construct_stub_create_deopt_pc_offset());
      heap_.set_construct_stub_invoke_deopt_pc_offset(
          main_heap->construct_stub_invoke_deopt_pc_offset());
      heap_.set_setter_stub_deopt_pc_offset(
          main_heap->setter_stub_deopt_pc_offset());
      heap_.set_getter_stub_deopt_pc_offset(
          main_heap->getter_stub_deopt_pc_offset());
    }
    ...
    // If we are deserializing, read the state into the now-empty heap.
    {
      ...
      if (!create_heap_objects) {
        DCHECK(is_main_isolate());
        des->Deserialize(this);
      }
      load_stub_cache_->Initialize();
      store_stub_cache_->Initialize();
      if (is_main_isolate()) {
        setup_delegate_->SetupInterpreter(worker_runtime_->interpreter(),
            create_heap_objects);
        heap_.NotifyDeserializationComplete();
      } else {
        Heap* main_heap = worker_runtime_->main_isolate()->heap();
        heap_.set_interpreter_entry_return_pc_offset(
            main_heap->interpreter_entry_return_pc_offset());
      }
    ...
    vm_thread_.RegisterAndToRunnable();
  }
  return true;
}
...
// <YUNOS> changed:
// Initialize OptimizingCompileDispatcher after seed fork
void Isolate::InitializeOptimizingCompileDispatcher() {
  CHECK(FLAG_concurrent_recompilation);
  CHECK(optimizing_compile_dispatcher_ == NULL);

  if (FLAG_trace_hydrogen || FLAG_trace_hydrogen_stubs) {
    PrintF("Concurrent recompilation has been disabled for tracing.\n");
  } else {
    optimizing_compile_dispatcher_ = new OptimizingCompileDispatcher(this);
    CHECK(optimizing_compile_dispatcher_);
  }
}

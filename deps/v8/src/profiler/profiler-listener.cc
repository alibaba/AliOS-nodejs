...
// <YUNOS> changed: begin
// util function for ic counting
static void GetICCounts(AbstractCode* abstract_code,
                        int* ic_monomorphic_and_polymorphic_count,
                        int* ic_megamorphic_and_generic_count,
                        int* ic_total_count) {
  if (!abstract_code->IsCode()) return;
  Code* code = abstract_code->GetCode();
  if (code->kind() != Code::FUNCTION) return;
  *ic_monomorphic_and_polymorphic_count = 0;
  *ic_megamorphic_and_generic_count = 0;
  *ic_total_count = 0;
  Object* raw_info = code->type_feedback_info();
  if (raw_info->IsTypeFeedbackInfo()) {
    TypeFeedbackInfo* info = TypeFeedbackInfo::cast(raw_info);
    *ic_monomorphic_and_polymorphic_count = info->ic_with_type_info_count();
    *ic_megamorphic_and_generic_count = info->ic_generic_count();
...
/* <YUNOS>: changed
 * record more information such as ic-count ... in the CodeEntry
 */
void ProfilerListener::CodeCreateEvent(CodeEventListener::LogEventsAndTags tag,
  ...
  // <YUNOS> changed: begin
  if (code->IsCode()) {
    Code* rcode = code->GetCode();
    rec->entry->set_code_kind(rcode->kind());
  }
  int poly_mono_ic_count = 0, mega_generic_ic_count = 0, total_ic_count = 0;
  GetICCounts(code, &poly_mono_ic_count,
              &mega_generic_ic_count, &total_ic_count);
  rec->entry->set_ic_count(CodeEntry::kMonomorphicAndPolymorphic,
                           poly_mono_ic_count);
  rec->entry->set_ic_count(CodeEntry::kMegamorphicAndGeneric,
                           mega_generic_ic_count);
  rec->entry->set_ic_count(CodeEntry::kAllIC, total_ic_count);
  // <YUNOS> changed: end
...
void ProfilerListener::CodeCreateEvent(CodeEventListener::LogEventsAndTags tag,
  ...
  if (abstract_code->IsCode()) {
    Code* code = abstract_code->GetCode();
    rec->entry->set_code_kind(code->kind());
  }
  rec->entry->set_map_count(0);
  int poly_mono_ic_count = 0, mega_generic_ic_count = 0, total_ic_count = 0;
  GetICCounts(abstract_code, &poly_mono_ic_count,
              &mega_generic_ic_count, &total_ic_count);
  rec->entry->set_ic_count(CodeEntry::kMonomorphicAndPolymorphic,
                           poly_mono_ic_count);
  rec->entry->set_ic_count(CodeEntry::kMegamorphicAndGeneric,
...
// <YUNOS> changed: begin
void ProfilerListener::SharedFunctionInfoNewMapEvent(Address addr, int map_count) {
  CodeEventsContainer evt_rec(CodeEventRecord::SHARED_FUNC_MAP_COUNT);
  SharedFunctionInfoNewMapEventRecord* rec =
      &evt_rec.SharedFunctionInfoNewMapEventRecord_;
  rec->addr = addr;

...
HeapSnapshot::HeapSnapshot(HeapProfiler* profiler, int swap_fd)
#else
...
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
      swap_fd_(swap_fd),
      swap_space_(swap_fd, SwapSpace::NewFileChunkSize * MB),
      swap_space_allocator_(&swap_space_),
      entries_(swap_space_allocator_),
      edges_(swap_space_allocator_),
      children_(swap_space_allocator_),
#endif // V8_OPT_MEM_FOR_HEAPDUMP
...
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  DCHECK(!entries_.size());  // Root entry must be the first one.
#else
...
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  DCHECK(static_cast<int>(gc_subroot_indexes_[tag]) == HeapEntry::kNoEntry);
#else
...
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
    int entries_size = static_cast<int>(entries_.size());
    sorted_entries_.Allocate(entries_size);
...
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  return sizeof(*this) + entries_.size() * sizeof(decltype(entries_)::value_type) +
#else
...
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  SwapDeque<HeapGraphEdge*>& edges = snapshot_->children();
#else
...
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  SwapVector<HeapEntry>& entries = snapshot_->entries();
  for (int i = 0; i < static_cast<int>(entries.size()); ++i) {
#else
...
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  // <YUNOS> changed: begin
  writer_->AddNumber(static_cast<int>(snapshot_->entries().size()));
  // <YUNOS> changed: end
#else

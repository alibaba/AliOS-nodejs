...
v8::RetainedObjectInfo* HeapProfiler::ExecuteWrapperClassCallback(
  ...
  // <YUNOS> changed: begin
  if (wrapper_callbacks_.length() <= class_id ||
      wrapper_callbacks_[class_id] == NULL) return NULL;
  // <YUNOS> changed: end
...
HeapSnapshot* HeapProfiler::TakeSnapshot(
  ...
  static const unsigned int kMaxBufSize = 64;
  char temp_data_path[kMaxBufSize] = {0};
  snprintf(temp_data_path,
           kMaxBufSize,
#if defined(ANDROID) || defined(OS_YUNOS)
           "/var/tools/v8-temp-%d.dat",
#else
           "./v8-temp-%d.dat",
#endif
           parent_pid);

  swap_fd = open(temp_data_path, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
  if (swap_fd == -1) {
    PrintF("Error: Fail to open temp data file for heapdump, heapdump stopped!\n");
    return NULL;
  }

  HeapSnapshot* result = new HeapSnapshot(this, swap_fd);
#else

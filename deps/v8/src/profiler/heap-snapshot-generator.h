...
class HeapEntry BASE_EMBEDDED {
 ...
 private:
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  INLINE(SwapDeque<HeapGraphEdge*>::iterator children_begin());
  INLINE(SwapDeque<HeapGraphEdge*>::iterator children_end());
#else
...
class HeapSnapshot {
 public:
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  explicit HeapSnapshot(HeapProfiler* profiler, int swap_fd);
#else
...
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  SwapVector<HeapEntry>& entries() { return entries_; }
  SwapDeque<HeapGraphEdge*>& children() { return children_; }
  SwapDeque<HeapGraphEdge>& edges() { return edges_; }
#else
 ...
 private:
  ...
  int swap_fd_;
#ifdef V8_OPT_MEM_FOR_HEAPDUMP
  size_t gc_subroot_indexes_[VisitorSynchronization::kNumberOfSyncTags];
  SwapSpace swap_space_;
  SwapAllocator<void> swap_space_allocator_;
  SwapVector<HeapEntry> entries_;
  SwapDeque<HeapGraphEdge> edges_;
  SwapDeque<HeapGraphEdge*> children_;
#else

...
/* <YUNOS>: changed
 * add more field to class CodeEntry
 */
class CodeEntry {
 public:
  // <YUNOS> changed: begin
  enum ICType {
    kMonomorphicAndPolymorphic = 0,
    kMegamorphicAndGeneric,
    kAllIC
  };
  // <YUNOS> changed: end
  ...
  // <YUNOS> changed: begin
  void set_code_kind(int kind) { code_kind_ = kind; }
  int code_kind() const { return code_kind_; }
  void set_opt_count(int count) { opt_count_ = count; }
  int opt_count() const { return opt_count_; }
  void set_deopt_count(int count) { deopt_count_ = count; }
  int deopt_count() const { return deopt_count_; }
  void set_map_count(int count) { map_count_ = count; }
  int map_count() const { return map_count_; }
  void set_ic_count(ICType type, int count) { ic_counts_[type] = count; }
  int ic_count(ICType type) const { return ic_counts_[type]; }
  InlineCacheState ic_state() const { return ic_state_; }
  void set_ic_state(InlineCacheState state) { ic_state_ = state; }
  // <YUNOS> changed: end
 ...
 private:
  ...
  int deopt_count_;
  int map_count_;
  int ic_counts_[kAllIC + 1];
  InlineCacheState ic_state_;
  // <YUNOS> changed: end
...
class CpuProfileJSONSerializer {
 public:
  explicit CpuProfileJSONSerializer(const CpuProfile* profile)
      : profile_(profile),
        writer_(NULL) {
  }
 ...
 private:
  ...
  ProfileOutputStreamWriter* writer_;

  friend class CpuProfileJSONSerializerEnumerator;
  friend class CpuProfileJSONSerializerIterator;

  void SerializeImpl();
  void SerializeHead();
  void SerializeSamples();
  void SerializeTimestamps();
  void SerializeString(const unsigned char* s);
  void SerializeNode(ProfileNode* node);
  void SerializeFunctionInfo(CodeEntry* code_entry,
                             Vector<char>& info);

  DISALLOW_COPY_AND_ASSIGN(CpuProfileJSONSerializer);
};
// <YUNOS> changed: end

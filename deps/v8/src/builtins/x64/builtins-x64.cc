...
// new.target.
    ...
    __ LoadTL(rsi, ThreadLocalTop::kContextAddress);
...
void Builtins::Generate_ResumeGeneratorTrampoline(MacroAssembler* masm) {
  ...
  __ LoadTL(kScratchRegister, ThreadLocalTop::kDebugHookOnnFunctionCallAddress);
  __ cmpb(Operand(kScratchRegister, 0), Immediate(0));
  __ j(not_equal, &prepare_step_in_if_stepping);

  // Flood function if we need to continue stepping in the suspended generator.
  __ LoadTL(kScratchRegister, ThreadLocalTop::kDebugSuspendedGeneratorAddress);
  __ cmpp(rbx, Operand(kScratchRegister, 0));
...
static void GenerateMakeCodeYoungAgainCommon(MacroAssembler* masm) {
  ...
  __ Pushad();
  __ LoadIsolateAddress(arg_reg_2);

...
// at compilation.
  ...
  // for ATOM regexps to avoid unnecessary trips to runtime.
  {
    Node* const stack_size =
        LoadTL(Is64() ? compiler::RootType::kInt64RootType :
        compiler::RootType::kInt32RootType,
        ThreadLocalTop::kRegexpStackMemorySizeAddress);
  ...
  Label if_success(this), if_exception(this, Label::kDeferred);
  {
    ...
    Node* const arg4 = LoadTL(ThreadLocalTop::kJSRegexpStaticOffsetsVectorAddress);
    ...
    Node* const stack_start = LoadTL(ThreadLocalTop::kRegexpStackMemoryAdress);
    Node* const stack_size = LoadTL(Is64() ?
        compiler::RootType::kInt64RootType : compiler::RootType::kInt32RootType,
        ThreadLocalTop::kRegexpStackMemorySizeAddress);
    ...
    Node* const arg8 = LoadTL(ThreadLocalTop::kIsolateAddress);
    ...
    // Fill match and capture offsets in match_info.
    {
      ...
      BuildFastLoop(
          ...
          [=, &var_to_offset](Node* offset) {
            Node* const static_offsets_vector_address =
                  LoadTL(ThreadLocalTop::kJSRegexpStaticOffsetsVectorAddress);
...
#ifdef DEBUG
    CSA_ASSERT(this, IsTheHole(LoadTL(ThreadLocalTop::kPendingExceptionAddress)));
#endif  // DEBUG
  ...
  BIND(&if_isnotjsregexp);
  {
    ...
    BIND(&if_isnotprototype);
    {
      ...
      Node* const method_name_str =
          HeapConstant(isolate->factory()->NewStringFromAsciiChecked(
              "RegExp.prototype.source", SHARED_TENURED));
      TailCallRuntime(Runtime::kThrowTypeError, context, message_id,
                      method_name_str);
    }
  }

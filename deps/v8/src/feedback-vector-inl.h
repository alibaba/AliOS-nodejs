...
int FeedbackVector::interrupt_budget() const {
  return Smi::cast(get(kInterruptBudgetIndex))->value();
}

void FeedbackVector::set_interrupt_budget(int budget) {
  set(kInterruptBudgetIndex, Smi::FromInt(budget));
}

int FeedbackVector::osr_loop_nesting_level() const {
  return Smi::cast(get(kOSRNestingLevelIndex))->value();
}

void FeedbackVector::set_osr_loop_nesting_level(int level) {
  set(kOSRNestingLevelIndex, Smi::FromInt(level));
}

int FeedbackVector::opt_count() const {
  return Smi::cast(get(kOptCountIndex))->value();
}

void FeedbackVector::set_opt_count(int count) {
  set(kOptCountIndex, Smi::FromInt(count));
}

int FeedbackVector::counters() const {
#if V8_HOST_ARCH_64_BIT
  return static_cast<int>(reinterpret_cast<long long>(get(kCountersIndex)));
#else
  return reinterpret_cast<int>(get(kCountersIndex));
#endif
}

void FeedbackVector::set_counters(int count) {
  set(kCountersIndex, reinterpret_cast<Smi*>(count));
}

int FeedbackVector::deopt_count() const {
  return DeoptCountBits::decode(counters());
}

void FeedbackVector::set_deopt_count(int count) {
  set_counters(DeoptCountBits::update(counters(), count));
}
...
void FeedbackVector::TryReenableOptimization() {
...
int FeedbackVector::opt_reenable_tries() const {
  return OptReenableTriesBits::decode(counters());
}

void FeedbackVector::set_opt_reenable_tries(int tries) {
  set_counters(OptReenableTriesBits::update(counters(), tries));
}

bool FeedbackVector::optimization_disabled() const {
  return OptimizationDisabledBit::decode(counters());
}

void FeedbackVector::set_optimization_disabled(bool disabled) {
  set_counters(OptimizationDisabledBit::update(counters(), disabled));
}

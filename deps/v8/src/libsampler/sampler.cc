 ...
 private:
  ...
  static void Restore() {
    if (signal_handler_installed_) {
      // <YUNOS> changed: begin
      if (!old_signal_handler_.sa_sigaction) {
        old_signal_handler_.sa_sigaction = &DefaultHandleProfilerSignal;
      }
      // <YUNOS> changed: end
  ...
  // <YUNOS> changed: begin
  // VM can still receive the SIGPROF signal from profiling thread, even
  // after the profiling thread has been destroy. So use this default empty
  // handler to deal with this case to avoid the VM exit !
  static void DefaultHandleProfilerSignal(

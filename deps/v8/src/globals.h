...
enum AllocationSpace {
  ...
  SHARED_LO_SPACE,    // Promoted shared large objects.


  FIRST_SPACE = NEW_SPACE,
  LAST_SPACE = SHARED_LO_SPACE,
  FIRST_PAGED_SPACE = OLD_SPACE,
  FIRST_SHARED_SPACE = SHARED_OLD_SPACE,
  LAST_PAGED_SPACE = SHARED_MAP_SPACE,
  LAST_PRIVATE_PAGED_SPACE = MAP_SPACE,
};

enum Sharability { SHARED, NOT_SHARED};

enum SerializerSpace {
  kSerializerNewSpace = 0,
  kSerializerOldSpace,
  kSerializerMutableSpace,
  kSerializerSharedOldSpace,
  kSerializerCodeSpace,
  kSerializerSharedCodeSpace,
  kSerializerMapSpace,
  kSerializerLoSpace,
  NUMBER_OF_SERIALIZER_SPACE,
  kSerializerSharedMapSpace = NUMBER_OF_SERIALIZER_SPACE,
  kSerializerSharedLoSpace = kSerializerLoSpace,
  kInvalidSerializerSpace = NUMBER_OF_SERIALIZER_SPACE + 1,
};
...
// allows).
enum PretenureFlag { NOT_TENURED, TENURED, SHARED_TENURED };
...
#define FOR_EACH_THREAD_LOCAL_OBJECT_ADDRESS_NAME(C)                      \
  C(Handler, handler_, false)                                             \
  C(CEntryFP, c_entry_fp_, false)                                         \
  C(CFunction, c_function_, false)                                        \
  C(Context, context_, true)                                              \
  C(PendingException, pending_exception_, true)                           \
  C(PendingHandlerContext, pending_handler_context_, true)                \
  C(PendingHandlerCode, pending_handler_code_, true)                      \
  C(PendingHandlerOffset, pending_handler_offset_, false)                 \
  C(PendingHandlerFP, pending_handler_fp_, false)                         \
  C(PendingHandlerSP, pending_handler_sp_, false)                         \
  C(ExternalCaughtException, external_caught_exception_, false)           \
  C(JSEntrySP, js_entry_sp_, false)                                       \
  C(PendingMessageObj, pending_message_obj_, true)                        \
  C(ScheduledException, scheduled_exception_, true)                       \

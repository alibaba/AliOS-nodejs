...
CompilationInfo::~CompilationInfo() {
  if (GetFlag(kDisableFutureOptimization) && has_shared_info()) {
    BailoutReason reason = bailout_reason();
    if (reason != kDeoptimizedTooManyTimes) {
      shared_info()->DisableOptimization(bailout_reason());
    } else if (!closure_.is_null()) {
      // disable opt for deopt too many times
      closure_->feedback_vector()->set_optimization_disabled(true);
    } else {
      base::OS::PrintError(
              "abort: opt abort while sfi exists but js function is null!\n");

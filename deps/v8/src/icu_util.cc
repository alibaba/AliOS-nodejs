...
void free_icu_data_ptr() {
  // <YUNOS> changed:
  // Currently webengine will refer this icu too, free it without notify
  // webengine could cause webengine crash.
  // Temporarily comments out this free operation.
  // TODO(zhiping.lzp): refine icu usage between node and webengine.
  // Track bug: https://aone.alibaba-inc.com/issue/8941519

#ifdef OS_YUNOS
  // if (g_icu_data_ptr) {
  //   munmap(g_icu_data_ptr, g_map_size);
  //   g_icu_data_ptr = NULL;
  // }
  // if (g_file) {
  //   fclose(g_file);
...
bool InitializeICU(const char* icu_data_file) {
  ...
  rewind(inf);
#ifdef OS_YUNOS
  const int64_t mask = static_cast<int64_t>(getpagesize()) - 1;
  size_t aligned_size = (size + mask) & ~mask;
  g_icu_data_ptr = (char*)mmap(NULL,
                               aligned_size,
                               PROT_READ,
                               MAP_SHARED,
                               fileno(inf),

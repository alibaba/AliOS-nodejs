...
MONITOR_ACCESSORS(SharedFunctionInfo, kPointerSize)
...
INT_ACCESSORS(SharedFunctionInfo, ic_age_and_bailout_reason,
...
void SharedFunctionInfo::set_scope_info(ScopeInfo* value,
  ...
  DCHECK(!is_shared_object() || value->is_shared_object());
...
bool SharedFunctionInfo::NeedReCompileWasmData(Isolate* isolate) {
  DisallowHeapAllocation no_gc;
  return HasAsmWasmData() &&
           (GetAsmWasmData(isolate) == isolate->heap()->empty_fixed_array());
}
...
void SharedFunctionInfo::set_bytecode_array(BytecodeArray* bytecode) {
  DCHECK(function_data()->IsUndefined(GetIsolate()));
  DCHECK(bytecode->is_shared_object() == is_shared_object());
...
BIT_FIELD_ACCESSORS(SharedFunctionInfo, ic_age_and_bailout_reason, ic_age,
                    SharedFunctionInfo::ICAgeBits)

BIT_FIELD_ACCESSORS(SharedFunctionInfo, ic_age_and_bailout_reason,

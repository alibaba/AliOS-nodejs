...
class SharedFunctionInfo : public HeapObject {
 public:
  ...
  inline bool NeedReCompileWasmData(Isolate* isolate);
  inline BytecodeArray* bytecode_array() const;
  inline void set_bytecode_array(BytecodeArray* bytecode);
  inline void ClearBytecodeArray();
  inline bool HasAsmWasmData() const;
  FixedArray* GetAsmWasmData(Isolate* isolate);
  void SetAsmWasmData(Isolate* isolate, Handle<FixedArray> data);
  void ClearAsmWasmData(Isolate* isolate);
  ...
  // Stores ic_age and bailout_reason as bit-fields.
  DECL_INT_ACCESSORS(ic_age_and_bailout_reason)
  ...
  class GlobalIterator {
   ...
   private:
    ...
    WeakFixedArray::Iterator shared_noscript_sfi_iterator_;
...
// Bit fields in |ic_age_and_bailout_reason|.
#define IC_AGE_AND_BAILOUT_REASON_BIT_FIELDS(V, _)       \
  V(ICAgeBits, int, 8, _)                                \
  V(DisabledOptimizationReasonBits, BailoutReason, 8, _)

  DEFINE_BIT_FIELDS(IC_AGE_AND_BAILOUT_REASON_BIT_FIELDS)
#undef IC_AGE_AND_BAILOUT_REASON_BIT_FIELDS

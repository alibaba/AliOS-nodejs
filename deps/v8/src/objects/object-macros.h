...
#define DECLARE_MONITOR()                                  \
  inline uint32_t monitor() const;                         \
  inline Address monitor_address() const;                  \
  inline void set_monitor(uint32_t mon);                   \

#define MONITOR_ACCESSORS(type, size)                                \
void type::set_monitor(uint32_t mon) {                               \
  DCHECK(!(mon & 0x1));                                              \
  if ((kInt32Size != kPointerSize) && (size == kPointerSize)) {      \
    WRITE_UINT32_FIELD(this, type::kMonitorOffset + kInt32Size, 0);  \
  }                                                                  \
  WRITE_UINT32_FIELD(this, type::kMonitorOffset, mon);               \
}                                                                    \
uint32_t type::monitor() const {                                     \
  return READ_UINT32_FIELD(this, type::kMonitorOffset);              \
}                                                                    \
Address type::monitor_address() const {                              \
  return AddressFrom<Address>(                                       \
      reinterpret_cast<const intptr_t>(                              \
        FIELD_ADDR_CONST(this, type::kMonitorOffset)));              \
}

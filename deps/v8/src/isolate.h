...
class ThreadLocalTop BASE_EMBEDDED {
 public:
  enum AddressId {
#define DECLARE_ENUM(CamelName, hacker_name, is_reference) \
                    k##CamelName##Address,
    FOR_EACH_THREAD_LOCAL_OBJECT_ADDRESS_NAME(DECLARE_ENUM)
#undef DECLARE_ENUM
    kAddressCount,
    kIsolateAddress = 15,
    kRegexpStackMemoryAdress =
        Internals::kIsolateThreadLocalSize >> kPointerSizeLog2,
    kRegexpStackMemorySizeAddress,
    kRegexpStackLimitAddress,
    // skip regexp stack isolate
    kJSRegexpStaticOffsetsVectorAddress = kRegexpStackLimitAddress + 2,
    kHandleScopeDataNextAddress,
    kHandleScopeDataLimitAddress,
    kHandleScopeDataLevelAddress,
    // skip handle scope data sealed_level canonical_scope
#ifndef __LP64__
    kLoadStubCacheAddress = kHandleScopeDataLevelAddress + 3,
#else
    kLoadStubCacheAddress = kHandleScopeDataLevelAddress + 2,
#endif
    kStoreStubCacheAddress,
    kIsProfilingAddress,
    kPromiseHookAddress,
    kIsTailCallEliminationEnabledAddress,
    kDateCacheStampAddress,
    kPromiseHookOrDebugIsActiveAddress,
    kDebugIsActiveAddress,
    kDebugHookOnnFunctionCallAddress,
    kDebugLastStepActionAddress,
    kDebugSuspendedGeneratorAddress,
    kDebugRestartFpAddress,
  };
  ...
  void Free();
  static bool AddressIsReference(AddressId index);
...
class Isolate {
 ...
 public:
  ~Isolate();
  void* GetEnv() { return NULL; }
  // <YUNOS> changed:
  class ArrayBufferAllocatorListNode {
   public:
    v8::ArrayBuffer::Allocator* allocator_;
    ArrayBufferAllocatorListNode* next_;
    int id_;

    ArrayBufferAllocatorListNode() :
        allocator_(NULL), next_(NULL), id_(-1) {}
   private:
    DISALLOW_COPY_AND_ASSIGN(ArrayBufferAllocatorListNode);
  };
  ...
  static const int kThreadLocalTopOffset =
                   Internals::kNumIsolateDataSlots << kPointerSizeLog2;
  static const int kHeapRootsDelta = Internals::kIsolateRootsOffset -
...
#undef NATIVE_CONTEXT_FIELD_ACCESSOR
  ...
  Address thread_local_top_address() {
    return reinterpret_cast<Address>(&thread_local_top_);
  }
  static inline ThreadLocalTop* current_thread_local_top();
  ...
  Builtins* builtins() { return worker_runtime_->builtins(); }
...
#if DEBUG
  static bool KeyCreated();
#endif

  void InitializeOptimizingCompileDispatcher();
  ...
  // <YUNOS> changed: begin
  // Set a new allocator when v8 initializing
  void set_array_buffer_allocator(v8::ArrayBuffer::Allocator* allocator) {
    // CHECK_EQ(nullptr, array_buffer_allocator_list_head_);
    AddArrayBufferAllocator(allocator);
  }

  // default get the top allocator in allocator-list
  v8::ArrayBuffer::Allocator* array_buffer_allocator(int id = -1);

  // Add one allocator to list instead of replacing the current one
  bool ForceSetArrayBufferAllocator(v8::ArrayBuffer::Allocator* allocator) {
    return AddArrayBufferAllocator(allocator);
  }

  // get current array buffer allocator's id
  int GetCurrentArrayBufferAllocatorID() {
    return array_buffer_allocator_list_head_ == NULL ?
        0 : array_buffer_allocator_list_head_->id_;
  }

  base::TimeTicks& delay_jit_ticks() {
    return delay_jit_ticks_;
  }

  void set_delay_jit_ticks(base::TimeTicks ticks) {
    delay_jit_ticks_ = ticks;
  }

  // TODO: Before isolate initialized, flags should cached in thread local. After isolate
  // initialized, flags directly set to isolate via SetFlagTOIsolate.
  class Flags {
   public:
#define FLAG_ACCESSOR(type, name, value)           \
  inline void set_##name(type v) { name##_ = v; }  \
  inline type name() const { return name##_; }

#define FLAG_LIST(V) \
        V(bool, optimize_for_size, FLAG_optimize_for_size)  \
        V(bool, during_delay_jit, false)
    FLAG_LIST(FLAG_ACCESSOR)
#undef FLAG_ACCESSOR

#define INIT_FLAG(type, name, value) \
     set_##name(value);
      Flags() {
        FLAG_LIST(INIT_FLAG)
      }
#undef INIT_FLAG
   ...
   private:
#define DEFINE_FLAG(type, name, value) \
     type name##_;
...
#undef FLAG_LIST
  ...
  interpreter::Interpreter* interpreter() const {
    return worker_runtime_->interpreter();
  }
  ...
  void set_is_profiling(bool is_profiling) { is_profiling_ = is_profiling; }
  inline Isolate* GetCurrentIsolate();
  inline VMThread* GetCurrentThread();
  inline Heap* GetCurrentHeap();
  bool is_main_isolate() const { return vm_thread_.is_main(); }
  bool is_worker_isolate() const { return !is_main_isolate(); }
  bool single_thread_mode() const { return vm_thread_.single_thread_mode(); }
  Isolate* main_isolate() { return worker_runtime_->main_isolate(); }
  VMThread* vmthread() { return &vm_thread_; }
  WorkerRuntime* worker_runtime() const { return worker_runtime_; }
  void WorkerStart() { vm_thread_.Start(); }
  bool IsProxyIsolate() const { return proxy_isolate() != this; }

 protected:
  explicit Isolate(bool enable_serializer, WorkerRuntime* runtime = nullptr);
 ...
 private:
  ...
  int32_t* jsregexp_static_offsets_vector_address_;
  ...
  Address date_cache_stamp_address_;
  bool promise_hook_or_debug_is_active_;
  Address debug_is_active_address_;
  Address debug_hook_on_function_call_address_;
  Address debug_last_step_action_address_;
  Address debug_suspended_generator_address_;
  Address debug_restart_fp_address_;
  Heap heap_;
  ...
  V8_EXPORT_PRIVATE static base::Thread::LocalStorageKey isolate_key_;
  ...
  Isolate* proxy_isolate() const { return proxy_isolate_; }
  void set_proxy_isolate(Isolate* isolate) { proxy_isolate_ = isolate; }
  ...
  // <YUNOS> changed:
  bool AddArrayBufferAllocator(v8::ArrayBuffer::Allocator *allocator);
  void DeleteArrayBufferAllocatorList();
  int GenerateArrayBufferAllocatorID();
...
#undef ISOLATE_FIELD_OFFSET
  ...
  ArrayBufferAllocatorListNode* array_buffer_allocator_list_head_;
  int array_buffer_allocator_id_generator_;
  ...
  base::TimeTicks delay_jit_ticks_;
  // <YUNOS> changed: end
  ...
  friend class IsolateProxyScope;
  friend class IsolateCurrentScope;
...
class IsolateProxyScope {
 public:
  IsolateProxyScope(Isolate* current, Isolate* proxy);
  ~IsolateProxyScope();
  static void Enter(Isolate* current, Isolate* proxy);
  static void Exit(Isolate* current, Isolate* proxy);

 private:
  Isolate* current_;
  DISALLOW_COPY_AND_ASSIGN(IsolateProxyScope);
};

class IsolateCurrentScope {
 public:
  explicit IsolateCurrentScope(Isolate* proxy);
  ~IsolateCurrentScope();

 private:
  Isolate* current_;
  DISALLOW_COPY_AND_ASSIGN(IsolateCurrentScope);
};

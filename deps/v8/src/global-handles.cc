...
Handle<Object> GlobalHandles::Create(Object* value) {
  ...
  if (FLAG_verify_thread_suspension)
    isolate_->vmthread()->AssertMutatorRunnable();
#endif
  if (first_free_ == NULL) {
...
Isolate* GlobalHandles::IsolateFromLocation(Object** location) {
  return Node::FromLocation(location)->GetGlobalHandles()->isolate();
}
...
DISABLE_CFI_PERF
void GlobalHandles::IterateWeakRootsForAllGc(RootVisitor* v) {
  for (NodeIterator it(this); !it.done(); it.Advance()) {
    Node* node = it.node();
    if (node->IsWeakRetainer()) {
      if (node->IsPendingPhantomResetHandle()) {
        // JSMT: no handles' callback can be called during gc
        // Pending weak phantom handles die immediately.
        // Everything else survives.
        node->ResetPhantomHandle();
        ++number_of_phantom_handle_resets_;
      } else {
        // JSMT:
        // Because normal object is not marked as pending,
        // So we should mark them here
        v->VisitRootPointer(Root::kGlobalHandles, node->location());
      }
    }
  }
...
#ifdef V8_5_DOT_7_PATCH
DISABLE_CFI_PERF
bool GlobalHandles::IterateObjectGroups(RootVisitor* v,
  ...
  for (int i = 0; i < object_groups_.length(); i++) {
    ...
    for (size_t j = 0; j < entry->length; ++j) {
      Object* object = *objects[j];
      if (object->IsHeapObject()) {
        v->VisitRootPointer(Root::kGlobalHandles, &object);
...
int GlobalHandles::DispatchPendingPhantomCallbacks(
  ...
  if (second_pass_callbacks.length() > 0) {
    if (FLAG_disable_pending_phantom_callbacks ||
        isolate()->flags().optimize_for_size() ||
        FLAG_predictable || synchronous_second_pass) {
      isolate()->heap()->CallGCPrologueCallbacks(
          GCType::kGCTypeProcessWeakCallbacks, kNoGCCallbackFlags);

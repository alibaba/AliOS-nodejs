...
class RootValueInfo {
 public:
  RootValueInfo(RootType root_type, uint32_t index) {
    DCHECK_LE(index, RootIndexBits::kMax);
    DCHECK_LE(static_cast<int>(root_type),  static_cast<int>(RootTypeBits::kMax));
    root_key_ = RootIndexBits::encode(index) | RootTypeBits::encode(root_type);
  }

  static MachineType machine_type(uint32_t root_key) {
    RootType root_type = RootTypeBits::decode(root_key);
    switch(root_type) {
#define DEF_CASE(name) case RootType::k##name##RootType: \
      return MachineType::name();
      ROOT_TYPE_LIST(DEF_CASE)
#undef DEF_CASE
      case RootType::kNoRootType:
      case RootType::kNumberOfRootTypes:
        UNREACHABLE();
        break;
    }
  ...
  static int32_t index(uint32_t root_key) {
    return RootIndexBits::decode(root_key);
  }
  uint32_t root_key() const { return root_key_; }

 private:
  static const int32_t kRootIndexSize = 16;
  static const int32_t kRootTypeSize = 8;

  class RootIndexBits: public BitField<int32_t, 0, kRootIndexSize> {};
  class RootTypeBits: public BitField<RootType, kRootIndexSize, kRootTypeSize> {};
...
MachineType LoadRootRepresentationOf(const Operator* const op) {
  DCHECK_EQ(IrOpcode::kLoadRoot, op->opcode());
  return RootValueInfo::machine_type(OpParameter<uint32_t>(op));
}

MachineType StoreRootRepresentationOf(const Operator* const op) {
  DCHECK_EQ(IrOpcode::kStoreRoot, op->opcode());
  return RootValueInfo::machine_type(OpParameter<uint32_t>(op));
}


int32_t LoadRootIndexOf(const Operator* const op) {
  DCHECK_EQ(IrOpcode::kLoadRoot, op->opcode());
  return RootValueInfo::index(OpParameter<uint32_t>(op));
}

int32_t StoreRootIndexOf(const Operator* const op) {
  DCHECK_EQ(IrOpcode::kStoreRoot, op->opcode());
  return RootValueInfo::index(OpParameter<uint32_t>(op));
}
...
const Operator* CommonOperatorBuilder::LoadRoot(RootType root_type, uint32_t index) {
  return new (zone()) Operator1<uint32_t>(
              IrOpcode::kLoadRoot,
              Operator::kNoDeopt | Operator::kNoThrow | Operator::kNoWrite,
              "LoadRoot",
              0, 1, 1, 1, 1, 0,
              RootValueInfo(root_type, index).root_key());
}


const Operator* CommonOperatorBuilder::StoreRoot(RootType root_type,
                                       uint32_t index) {
  return new (zone()) Operator1<uint32_t>(
              IrOpcode::kStoreRoot,

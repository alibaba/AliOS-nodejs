...
enum class RootType : uint8_t {
  kNoRootType = 0,
#define DEF_ENUM(name) k##name##RootType,
  ROOT_TYPE_LIST(DEF_ENUM)
#undef DEF_ENUM
  kNumberOfRootTypes
};
...
class V8_EXPORT_PRIVATE CommonOperatorBuilder final
 ...
 public:
  ...
  const Operator* LoadRoot(RootType root_type, uint32_t index);
  const Operator* StoreRoot(RootType root_type, uint32_t index);

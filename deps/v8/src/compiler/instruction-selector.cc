...
bool InstructionSelector::CanAddressRelativeToRootsRegister() const {
  return (!FLAG_js_mt_mode && (enable_serialization_ == kDisableSerialization) &&
         CanUseRootsRegister());
}
...
void InstructionSelector::VisitNode(Node* node) {
  ...
  switch (node->opcode()) {
    ...
    case IrOpcode::kLoadRoot: {
      MachineType type = LoadRootRepresentationOf(node->op());
      MarkAsRepresentation(type.representation(), node);
      return VisitLoadRoot(node);
    }
    case IrOpcode::kStoreRoot: {
      return VisitStoreRoot(node);
    }

      ...
      if (m.index().HasValue() && m.object().HasValue()) {
        Address const kRootsRegisterValue =
            ...
            reinterpret_cast<Address>(
                selector()->isolate()->thread_local_top_address());
...
void InstructionSelector::VisitLoadRoot(Node* node) {
  LoadRepresentation load_rep = LoadRootRepresentationOf(node->op());
  ...
  InstructionCode code = opcode | AddressingModeField::encode(kMode_Root);

  int32_t displacement = LoadRootIndexOf(node->op());
  inputs[0] = g.UseImmediate(displacement - kRootRegisterBias);

  Emit(code, 1, outputs, 1, inputs);
}

void InstructionSelector::VisitStoreRoot(Node* node) {
  MachineType store_type = StoreRootRepresentationOf(node->op());
  X64OperandGenerator g(this);
  Node* value = node->InputAt(0);

  InstructionOperand inputs[2];
  int32_t displacement = StoreRootIndexOf(node->op());
  inputs[0] = g.UseImmediate(displacement - kRootRegisterBias);
  inputs[1] = g.UseRegister(value);

  StoreRepresentation store_rep(store_type.representation(), kNoWriteBarrier);
  ArchOpcode opcode = GetStoreOpcode(store_rep);
  InstructionCode code = opcode | AddressingModeField::encode(kMode_Root);

  Emit(code, 0, static_cast<InstructionOperand*>(nullptr), 2,
       inputs);
}
...
void VisitWord64Compare(InstructionSelector* selector, Node* node,
    ...
    if (m.right().HasValue() &&
      ...
      return VisitCompare(
          ...
          g.TempImmediate((root_index * kPointerSize) -
                           kRootRegisterBias  + Isolate::kHeapRootsDelta),
  ...
  Int64BinopMatcher m(node);
  if (m.left().IsLoadRoot() && m.right().IsLoadStackPointer()) {
    Heap::RootListIndex root_index = Heap::kStackLimitRootIndex;
    int32_t offset = root_index * kPointerSize + Isolate::kHeapRootsDelta;

    int32_t index = LoadRootIndexOf(m.left().node()->op());
    if (index == offset) {

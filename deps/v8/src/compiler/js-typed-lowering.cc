...
Reduction JSTypedLowering::ReduceJSLoadMessage(Node* node) {
  ...
  ThreadLocalTop::AddressId index = ThreadLocalTop::kPendingMessageObjAddress;

  node->RemoveInput(0);
  NodeProperties::ChangeOp(node, common()->LoadRoot(RootType::kAnyTaggedRootType,
                                           index * kPointerSize));
  return Changed(node);
}
...
Reduction JSTypedLowering::ReduceJSStoreMessage(Node* node) {
  ...
  node->ReplaceInput(1, value);
  node->RemoveInput(0);
  NodeProperties::ChangeOp(node, common()->StoreRoot(RootType::kAnyTaggedRootType,
                                           index * kPointerSize));
  return Changed(node);
}

...
Node* GraphAssembler::LoadRoot(Heap::RootListIndex root_index) {
  RootType root_type = Heap::RootIsReference(root_index) ?
                    RootType::kAnyTaggedRootType : RootType::kPointerRootType;
  return current_effect_ =
    graph()->NewNode(common()->LoadRoot(root_type,
              root_index * kPointerSize + Isolate::kHeapRootsDelta),
              current_effect_,
              current_control_);
}

Node* GraphAssembler::StoreRoot(Heap::RootListIndex root_index, Node* value) {
  RootType root_type = Heap::RootIsReference(root_index) ?
                    RootType::kAnyTaggedRootType : RootType::kPointerRootType;
  return current_effect_ =
    graph()->NewNode(common()->StoreRoot(root_type,
        root_index * kPointerSize + Isolate::kHeapRootsDelta),
        value,
        current_effect_,
        current_control_);

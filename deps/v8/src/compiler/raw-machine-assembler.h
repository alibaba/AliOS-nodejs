...
class V8_EXPORT_PRIVATE RawMachineAssembler {
 public:
  ...
  Node* LoadRoot(RootType root_type, uint32_t index) {
    return AddNode(common()->LoadRoot(root_type, index));
  }
  Node* StoreRoot(RootType root_type, uint32_t index, Node* value) {
    return AddNode(common()->StoreRoot(root_type, index), value);
  }

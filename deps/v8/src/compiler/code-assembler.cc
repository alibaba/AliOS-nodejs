...
Node* CodeAssembler::LoadRoot(Heap::RootListIndex root_index) {
  if (isolate()->heap()->RootCanBeTreatedAsConstant(root_index)) {
    ...
    } else if (!isolate()->vmthread()->in_shared_codegen() ||
        Handle<HeapObject>::cast(root)->is_shared_object()) {
      return HeapConstant(Handle<HeapObject>::cast(root));
    }
  }

  RootType root_type = Heap::RootIsReference(root_index) ?
                    RootType::kAnyTaggedRootType : RootType::kPointerRootType;
  return raw_assembler()->LoadRoot(root_type,
                          root_index * kPointerSize + Isolate::kHeapRootsDelta);
}

Node* CodeAssembler::LoadRoot(RootType root_type, int index) {
  return raw_assembler()->LoadRoot(root_type, index);
}

Node* CodeAssembler::StoreRoot(RootType root_type, Heap::RootListIndex root_index,
                               Node* value) {
  DCHECK(Heap::RootCanBeWrittenAfterInitialization(root_index));
  return raw_assembler()->StoreRoot(root_type, root_index * kPointerSize
                                   + Isolate::kHeapRootsDelta, value);
}

Node* CodeAssembler::LoadTL(ThreadLocalTop::AddressId index) {
  RootType root_type = ThreadLocalTop::AddressIsReference(index) ?
                    RootType::kAnyTaggedRootType : RootType::kPointerRootType;
  return raw_assembler()->LoadRoot(root_type, index * kPointerSize);
}

Node* CodeAssembler::LoadTL(RootType root_type, ThreadLocalTop::AddressId index) {
  return raw_assembler()->LoadRoot(root_type, index * kPointerSize);
}

Node* CodeAssembler::StoreNoWriteBarrierTL(ThreadLocalTop::AddressId index,
                                           Node* value) {
  RootType root_type = ThreadLocalTop::AddressIsReference(index) ?
                    RootType::kAnyTaggedRootType : RootType::kPointerRootType;
  return raw_assembler()->StoreRoot(root_type, index * kPointerSize, value);
}

Node* CodeAssembler::StoreNoWriteBarrierTL(RootType root_type,

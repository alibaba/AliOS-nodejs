 ...
 public:
  ...
  MemOperand InputOffset(size_t* first_index) {
    ...
    switch (AddressingModeField::decode(instr_->opcode())) {
      ...
      case kMode_Root:
        *first_index += 1;
        return MemOperand(kRootRegister, InputInt32(index + 0));
    }
    UNREACHABLE();
  }
...
#define ASSEMBLE_ATOMIC_EXCHANGE_INTEGER(load_instr, store_instr)             \
  do {                                                                        \
    ...
    __ store_instr(i.TempRegister(1), i.InputRegister(2), i.TempRegister(0));\
...
#define ASSEMBLE_ATOMIC_COMPARE_EXCHANGE_INTEGER(load_instr, store_instr)     \
  do {                                                                        \
    ...
    __ b(ne, &exit);                                                          \
    __ store_instr(i.TempRegister(2), i.InputRegister(3), i.TempRegister(0)); \
...
#define ASSEMBLE_ATOMIC_BINOP(load_instr, store_instr, bin_instr)            \
  do {                                                                       \
    ...
    __ store_instr(i.TempRegister(1), i.TempRegister(2), i.TempRegister(0));\
    __ teq(i.TempRegister(1), Operand(0));                                   \
    __ b(ne, &binop);                                                        \
    __ dmb(ISH);                                                             \
  } while (0)

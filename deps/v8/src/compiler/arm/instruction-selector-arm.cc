...
void InstructionSelector::VisitLoadRoot(Node* node) {
  ...
  inputs[0] = g.UseImmediate(displacement);

  opcode |= AddressingModeField::encode(kMode_Root);
  Emit(opcode, 1, outputs, 1, inputs);
}

void InstructionSelector::VisitStoreRoot(Node* node) {
  MachineRepresentation rep = StoreRootRepresentationOf(node->op()).representation();
  InstructionCode opcode = kArchNop;
  ArmOperandGenerator g(this);
  switch (rep) {
    case MachineRepresentation::kFloat32:
  ...
  inputs[1] = g.UseImmediate(displacement);

  opcode |= AddressingModeField::encode(kMode_Root);
  Emit(opcode, 0, static_cast<InstructionOperand*>(nullptr), 2,
       inputs);
}
...
void InstructionSelector::VisitAtomicBinaryOperation(
  ...
  InstructionCode code = opcode | AddressingModeField::encode(addressing_mode);
  Emit(code, 1, outputs, input_count, inputs, 3, temps);
}

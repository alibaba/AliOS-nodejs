  ...
  if (!handlers_.empty()) {
    Handle<HandlerTable> table =
        Handle<HandlerTable>::cast(isolate()->factory()->NewFixedArray(
            ...
            result->is_shared_object() ? SHARED_TENURED : TENURED));
...
Handle<PodArray<InliningPosition>> CreateInliningPositions(
    CompilationInfo* info, PretenureFlag pretenure) {
  ...
  Handle<PodArray<InliningPosition>> inl_positions =
      PodArray<InliningPosition>::New(
          info->isolate(), static_cast<int>(inlined_functions.size()), pretenure);
...
void CodeGenerator::PopulateDeoptimizationData(Handle<Code> code_object) {
  ...
  PretenureFlag pretenure = code_object->is_shared_object() ?
                            SHARED_TENURED : TENURED;
  Handle<DeoptimizationInputData> data =
      DeoptimizationInputData::New(isolate(), deopt_count, pretenure);

  Handle<ByteArray> translation_array =
      translations_.CreateByteArray(isolate()->factory(), pretenure);
  ...
  Handle<FixedArray> literals = isolate()->factory()->NewFixedArray(
      static_cast<int>(deoptimization_literals_.size()), pretenure);

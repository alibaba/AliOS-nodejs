...
class SerializerReference {
 public:
  ...
  static SerializerReference BackReference(SerializerSpace space,
                                           uint32_t chunk_index,
                                           uint32_t chunk_offset) {
    DCHECK(IsAligned(chunk_offset, kObjectAlignment));
    DCHECK_NE(kSerializerLoSpace, space);
  ...
  static SerializerReference MapReference(uint32_t index) {
    return SerializerReference(SpaceBits::encode(kSerializerMapSpace) |
                               ValueIndexBits::encode(index));
  }

  static SerializerReference SharedMapReference(uint32_t index) {
    return SerializerReference(SpaceBits::encode(kSerializerMapSpace) |
                               ValueIndexBits::encode(index));
  }

  static SerializerReference LargeObjectReference(uint32_t index) {
    return SerializerReference(SpaceBits::encode(kSerializerLoSpace) |
                               ValueIndexBits::encode(index));
  }
  ...
  bool is_back_reference() const {
    return SpaceBits::decode(bitfield_) <= NUMBER_OF_SERIALIZER_SPACE;
  }

  SerializerSpace space() const {
    DCHECK(is_back_reference());
    return static_cast<SerializerSpace>(SpaceBits::decode(bitfield_));
  }
 ...
 private:
  ...
  static const int kSerializerSpacetSize = 4;
  static const int kChunkOffsetSize = kPageSizeBits - kObjectAlignmentBits;
  static const int kChunkIndexSize = 32 - kChunkOffsetSize - kSerializerSpacetSize;
  static const int kValueIndexSize = kChunkOffsetSize + kChunkIndexSize;

  static const int kSpecialValueSpace = NUMBER_OF_SERIALIZER_SPACE;
  static const int kAttachedReferenceSpace = kSpecialValueSpace + 1;
  STATIC_ASSERT(kAttachedReferenceSpace < (1 << kSerializerSpacetSize));
  ...
  class SpaceBits : public BitField<int, kValueIndexSize, kSerializerSpacetSize> {};

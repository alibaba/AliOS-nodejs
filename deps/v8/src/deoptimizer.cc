...
static MemoryChunk* AllocateCodeChunk(MemoryAllocator* allocator) {
  return allocator->AllocateChunk(Deoptimizer::GetMaxDeoptTableSize(),
                                  ...
                                  EXECUTABLE, NULL, "deopt_entry_code");
}
// <YUNOS> changed: end
...
DeoptimizerData::DeoptimizerData(MemoryAllocator* allocator)
  ...
  for (int i = 0; i <= Deoptimizer::kLastBailoutType; ++i) {
    ...
    //JSMT: AllocateCodeChunk may fail when too many threads
    // DCHECK(deopt_entry_code_[i] != NULL);
  }
}
...
DeoptimizerData::~DeoptimizerData() {
  for (int i = 0; i <= Deoptimizer::kLastBailoutType; ++i) {
    //JSMT: workaround for code range oom
    if (deopt_entry_code_[i] != NULL) {
      allocator_->Free<MemoryAllocator::kFull>(deopt_entry_code_[i], NULL);
...
Handle<ByteArray> TranslationBuffer::CreateByteArray(Factory* factory,
  ...
  Handle<ByteArray> result = factory->NewByteArray(CurrentIndex(), pretenure);

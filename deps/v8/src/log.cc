...
void Logger::EnterExternal(Isolate* isolate) {
  ...
  CHECK(isolate->current_vm_state() == JS);
...
void Logger::LeaveExternal(Isolate* isolate) {
  ...
  CHECK(isolate->current_vm_state() == EXTERNAL);
...
// <YUNOS> changed: begin
void Logger::SharedFunctionInfoNewMapEvent(Address addr, int map_count) {
  // TODO (zhefeng.wu) append log to trace map count
  // if (!is_logging_code_events()) return;
...
bool Logger::SetUp(Isolate* isolate) {
  ...
  if (FLAG_expose_logger) return false;
...
#ifdef V8_YUNOS_LOGGER
bool Logger::SetUp(Isolate* isolate, const char* log_file_name) {
  if (!FLAG_expose_logger) return false;
  if (is_initialized_) return true;
  is_initialized_ = true;

  // FLAG_log_snapshot_positions should be set before snapshot deserialization.
  // It's too late here.
  // --ll-prof implies --log-snapshot-positions.
  // if (FLAG_ll_prof) {

...
Handle<SharedFunctionInfo> FunctionTemplateInfo::GetOrCreateSharedFunctionInfo(
  ...
  // FunctionTemplateInfo is not shared and SFI should not shared too
  UnSharedCodegenScope ucs(isolate->vmthread(), !info->is_shared_object());
...
MaybeHandle<Object> Object::GetPropertyWithAccessor(LookupIterator* it) {
  ...
  if (structure->IsAccessorInfo()) {
    ...
    FastApiCallScope fas(isolate->vmthread(), info->is_fast_accessor());
    ...
    if (info->replace_on_access() && receiver->IsJSReceiver()) {
      FastApiCallScope fas(isolate->vmthread(), true);
...
bool String::MakeExternal(v8::String::ExternalStringResource* resource) {
  ...
  if (is_shared_object() && !heap->isolate()->single_thread_mode())
    return false;
...
Handle<WeakCell> Map::WeakCellForMap(Handle<Map> map) {
  ...
  PretenureFlag pretenure = map->is_shared_object() ?
                            SHARED_TENURED : TENURED;
  Handle<WeakCell> weak_cell = isolate->factory()->NewWeakCell(map, pretenure);
...
Handle<Map> Map::RawCopy(Handle<Map> map, int instance_size) {
  ...
  // TODO: Profile map count which is associated with aone<11721279>.
  // <YUNOS> changed: begin
  // if (map->GetIsolate()->is_profiling()) {
  // }
  // <YUNOS> changed: end
  return result;
}
...
void WeakFixedArray::Set(Handle<WeakFixedArray> array, int index,
  ...
  PretenureFlag pretenure = array->is_shared_object() ?
                            SHARED_TENURED : TENURED;
  Handle<WeakCell> cell =
      value->IsMap() ? Map::WeakCellForMap(Handle<Map>::cast(value))
      : array->GetIsolate()->factory()->NewWeakCell(value, pretenure);
...
Handle<WeakFixedArray> WeakFixedArray::Add(Handle<Object> maybe_array,
                                           Handle<HeapObject> value,
                                           int* assigned_index, bool is_shared) {
  Handle<WeakFixedArray> array =
      (maybe_array.is_null() || !maybe_array->IsWeakFixedArray())
          ? Allocate(value->GetIsolate(), 1, Handle<WeakFixedArray>::null(), is_shared)
  ...
  Handle<WeakFixedArray> new_array =
      Allocate(array->GetIsolate(), new_length, array, is_shared);
  if (FLAG_trace_weak_arrays) {
...
Handle<WeakFixedArray> WeakFixedArray::Allocate(
    ...
    Handle<WeakFixedArray> initialize_from, bool is_shared) {
  DCHECK(0 <= size);
  PretenureFlag pretenure = is_shared ? SHARED_TENURED : NOT_TENURED;
  Handle<FixedArray> result =
      isolate->factory()->NewFixedArray(size + kFirstIndex, pretenure);
  int index = 0;
...
Handle<FixedArray> String::CalculateLineEnds(Handle<String> src,
  ...
  PretenureFlag pretenure = isolate->vmthread()->in_shared_codegen() ?
                            SHARED_TENURED : TENURED;
...
void JSFunction::MarkForOptimization(ConcurrencyMode mode) {
  ...
  DCHECK(shared()->allows_lazy_compilation() ||
         (!shared()->optimization_disabled() &&
          !feedback_vector()->optimization_disabled()));
...
void Script::InitLineEnds(Handle<Script> script) {
  ...
  if (!src_obj->IsString()) {
    ...
    SharedCodegenScope scs(isolate->vmthread(), script->is_shared_object());
...
void Script::SetSource(Isolate* isolate, Handle<String> source) {
  if (is_shared_object() && !source->is_shared_object()) {
    source = isolate->factory()->InternalizeString(source);
  }
  set_source(*source);
}
...
Handle<JSObject> Script::GetWrapper(Handle<Script> script) {
  ...
  // JSMT: TODO support script set_wrapper?
  if (!script->is_shared_object()) {
...
Script::Iterator::Iterator(Isolate* isolate)
    : iterator_(isolate->heap()->script_list()),
      shared_iterator_(isolate->heap()->shared_script_list()),
      next_(nullptr), shared_next_(nullptr) {}


Script* Script::Iterator::Next() {
  // JSMT: Sort?
  next_ = (next_ == nullptr) ? iterator_.Next<Script>() : next_;
  shared_next_ = (shared_next_ == nullptr) ?
                 shared_iterator_.Next<Script>() : shared_next_;
  Script* result;
  if (next_ != nullptr && (shared_next_ == nullptr ||
      next_->id() < shared_next_->id())) {
    result = next_;
    next_ = nullptr;
  } else {
    result = shared_next_;
    shared_next_ = nullptr;
  }
  return result;
}
...
SharedFunctionInfo::GlobalIterator::GlobalIterator(Isolate* isolate)
    : script_iterator_(isolate),
      ...
      shared_noscript_sfi_iterator_(
        isolate->heap()->shared_noscript_shared_function_infos()),
...
SharedFunctionInfo* SharedFunctionInfo::GlobalIterator::Next() {
  ...
  next = shared_noscript_sfi_iterator_.Next<SharedFunctionInfo>();
  if (next != nullptr) return next;
  for (;;) {
...
void SharedFunctionInfo::SetScript(Handle<SharedFunctionInfo> shared,
  ...
  // duplicates.
  bool is_shared = shared->is_shared_object();
  if (script_object->IsScript()) {
    Handle<Script> script = Handle<Script>::cast(script_object);
    CHECK_EQ(is_shared, script->is_shared_object());
...
#endif
    PretenureFlag pretenure = is_shared ? SHARED_TENURED : NOT_TENURED;
    Handle<WeakCell> cell = isolate->factory()->NewWeakCell(shared, pretenure);
    list->set(shared->function_literal_id(), *cell);
  } else if (is_shared) {
    WorkerRuntime* runtime = isolate->worker_runtime();
    sync::SafepointMutexLock mu(isolate->vmthread(),
        WorkerRuntime::GetLock(runtime, WorkerRuntime::kNoScriptSharedFunctionInfos));
    Handle<Object> list = isolate->factory()->shared_noscript_shared_function_infos();
#ifdef DEBUG
    if (FLAG_enable_slow_asserts) {
...
#endif  // DEBUG
    list = WeakFixedArray::Add(list, shared, NULL, is_shared);
    isolate->heap()->SetRootSharedNoScriptSharedFunctionInfos(*list);
  } else {
...
FixedArray* SharedFunctionInfo::GetAsmWasmData(Isolate* isolate) {
  DCHECK(HasAsmWasmData());
  if (!is_shared_object()) {
    return FixedArray::cast(function_data());
  }
  if (isolate->heap()->weak_wasm_data_table() == Smi::kZero) {
    return isolate->heap()->empty_fixed_array();
  }
  Handle<WeakHashTable> table =
      Handle<WeakHashTable>::cast(isolate->factory()->weak_wasm_data_table());
  Handle<SharedFunctionInfo> shared(this, isolate);
  Object* result = table->Lookup(shared);
  // Do not found ?
...
void SharedFunctionInfo::SetAsmWasmData(Isolate* isolate, Handle<FixedArray> data) {
  DCHECK(function_data()->IsUndefined(isolate) || HasAsmWasmData());
  DCHECK(!data->is_shared_object());
  if (!is_shared_object()) {
  ...
  if (heap->weak_wasm_data_table() == Smi::kZero) {
    heap->SetRootWeakWasmDataTable(*WeakHashTable::New(isolate, 16, TENURED));
  }
  Handle<WeakHashTable> table(
      WeakHashTable::cast(heap->weak_wasm_data_table()), isolate);
  Handle<SharedFunctionInfo> shared(this, isolate);
  table = WeakHashTable::Put(table, shared, data);
  heap->SetRootWeakWasmDataTable(*table);
  // set HasAsmWasmData
  set_function_data(heap->empty_fixed_array());
  sync::ThreadFenceForConstructor();
  DCHECK_EQ(*data, GetAsmWasmData(isolate));
}

void SharedFunctionInfo::ClearAsmWasmData(Isolate* isolate) {
  DCHECK(function_data()->IsUndefined(isolate) || HasAsmWasmData());
  if (!is_shared_object()) {
    set_function_data(isolate->heap()->undefined_value());
    return;
  }
  DCHECK(isolate->heap()->weak_wasm_data_table() != Smi::kZero);
  Handle<WeakHashTable> table =
    Handle<WeakHashTable>::cast(isolate->factory()->weak_wasm_data_table());
  Handle<SharedFunctionInfo> shared(this, isolate);
  WeakHashTable::Remove(table, shared);
  set_function_data(isolate->heap()->undefined_value());
  sync::ThreadFenceForConstructor();
}
...
void SharedFunctionInfo::DisableOptimization(BailoutReason reason) {
  ...
  DCHECK(reason != kDeoptimizedTooManyTimes);
...
void SharedFunctionInfo::InitFromFunctionLiteral(
  ...
  Handle<String> inferred_name = lit->inferred_name();
  if (shared_info->is_shared_object()) {
    inferred_name = isolate->factory()->InternalizeString(inferred_name);
  }
  shared_info->set_inferred_name(*inferred_name);
...
void SharedFunctionInfo::ResetForNewContext(int new_ic_age) {
  ...
  // NOTE: We move the flag of disabling opt for to many deopt times to feedback_vecotr.
  // All of other reasons for optimization disabled will not be changed between different
  // contexes.
  //
...
void Code::MakeOlder() {
  // <YUNOS> changed: begin
  // Make all codes don't grow older during seed intializetion
  // Make all codes loaded in seed keep young
  if (FLAG_freeze_code_for_seed &&
      (FLAG_enable_seed_heap ||
       Heap::InSeedHeap(reinterpret_cast<Address>(this)))) {
      return;
  }
  // <YUNOS> changed: end
  byte* sequence = FindCodeAgeSequence();
...
void BytecodeArray::MakeOlder() {
  // <YUNOS> changed: begin
  // Make all bytecodes don't grow older during seed intializetion
  // Make all bytecodes loaded in seed keep young
  if (FLAG_freeze_code_for_seed &&
      (FLAG_enable_seed_heap ||
       Heap::InSeedHeap(reinterpret_cast<Address>(this)))) {
      return;
...
class StringSharedKey : public HashTableKey {
 public:
  ...
  Handle<Object> AsHandle(Isolate* isolate, bool need_shared = false) {
    PretenureFlag pretenure = need_shared ? SHARED_TENURED : NOT_TENURED;
    Handle<FixedArray> array = isolate->factory()->NewFixedArray(4, pretenure);
    DCHECK(!need_shared || shared_->is_shared_object());
    DCHECK(!need_shared || source_->is_shared_object());
    array->set(0, *shared_);
    array->set(1, *source_);
...
Handle<Derived> HashTable<Derived, Shape>::EnsureCapacity(
  ...
  pretenure = table->is_shared_object() ?
              SHARED_TENURED : (should_pretenure ? TENURED : NOT_TENURED);
  Handle<Derived> new_table = HashTable::New(
      isolate, new_nof, pretenure);
...
Handle<Derived> HashTable<Derived, Shape>::Shrink(Handle<Derived> table) {
  ...
  bool should_pretenure =
      ...
      !isolate->heap()->InNewSpace(*table);
  PretenureFlag pretenure = table->is_shared_object() ?
              SHARED_TENURED : (should_pretenure ? TENURED : NOT_TENURED);
  Handle<Derived> new_table = HashTable::New(
      isolate,
      at_least_room_for,
  ...
  DCHECK(table->is_shared_object() == new_table->is_shared_object());
  table->Rehash(*new_table);
  return new_table;
}
...
MaybeHandle<String> StringTable::LookupTwoCharsStringIfExists(
  ...
  WorkerRuntime* worker_runtime = isolate->worker_runtime();
  DisallowHeapAllocation no_gc;
  sync::MutexLock mu(self, WorkerRuntime::GetLock(worker_runtime,
                              WorkerRuntime::kStringTable));
  TwoCharHashTableKey key(c1, c2, isolate->heap()->HashSeed());
  Handle<StringTable> string_table = isolate->worker_runtime()->string_table();
...
void StringTable::EnsureCapacityForDeserialization(Isolate* isolate,
  ...
  sync::MutexLock mu(self, WorkerRuntime::GetLock(worker_runtime,
                              WorkerRuntime::kStringTable));
  Handle<StringTable> table = worker_runtime->string_table();
  // We need a key instance for the virtual hash function.
  table = StringTable::EnsureCapacity(table, expected);
  isolate->worker_runtime()->SetStringTableRoot(*table);
}
...
Handle<String> StringTable::LookupKey(Isolate* isolate, StringTableKey* key) {
  ...
  Handle<String> string;
  {
    sync::MutexUnLock unlock(self, WorkerRuntime::GetLock(worker_runtime,
                                   WorkerRuntime::kStringTable));
  ...
  worker_runtime->SetStringTableRoot(*table);
  return Handle<String>::cast(string);
}

Handle<StringTable> StringTable::EnsureCapacity(
    Handle<StringTable> table, int n) {
  if (table->HasSufficientCapacityToAdd(n)) return table;

  DCHECK(table->is_shared_object());
  int nof = 0;
  ...
  while (true) {
    nof = table->NumberOfElements() + n;
    {
      // Free String Table lock, and allow cause gc
      sync::MutexUnLock unlock(isolate->vmthread(),
          WorkerRuntime::GetLock(worker_runtime,
            WorkerRuntime::kStringTable));
      AllowHeapAllocation gc;
      new_table = HashTable::New(isolate, nof, SHARED_TENURED);
    }
    table = worker_runtime->string_table();
    if (table->HasSufficientCapacityToAdd(n)) return table;
    if (table->NumberOfElements() + n <= nof ) break;
  }

  DCHECK(new_table->HasSufficientCapacityToAdd(n));
  table->Rehash(*new_table);
  return new_table;
}
...
Object* StringTable::LookupStringIfExists_NoAllocate(String* string) {
  ...
  sync::MutexLock mu(self, WorkerRuntime::GetLock(worker_runtime,
                              WorkerRuntime::kStringTable));
  StringTable* table = worker_runtime->StringTableRoot();
...
Handle<CompilationCacheTable> CompilationCacheTable::PutScript(
  ...
  StringSharedKey key(src, shared, language_mode, kNoSourcePosition);
  Handle<Object> k = key.AsHandle(isolate, cache->is_shared_object());
  cache = EnsureCapacity(cache, 1);
  int entry = cache->FindInsertionEntry(key.Hash());
  cache->set(EntryToIndex(entry), *k);
  DCHECK(cache->is_shared_object() == value->is_shared_object());
  cache->set(EntryToIndex(entry) + 1, *value);
  // JSMT: TODO script literals not use now.
...
Handle<WeakHashTable> WeakHashTable::Put(Handle<WeakHashTable> table,
  ...
  PretenureFlag pretenure = key->is_shared_object() ?
                            SHARED_TENURED : TENURED;
  Handle<WeakCell> key_cell = isolate->factory()->NewWeakCell(key, pretenure);
...
void WeakHashTable::Remove(Handle<WeakHashTable> table, Handle<HeapObject> key) {
  Isolate* isolate = key->GetIsolate();
  Heap* heap = isolate->heap();
  DCHECK(table->IsKey(isolate, *key));
  int entry = table->FindEntry(key);
  // Key is already in table, just overwrite value.
  if (entry != kNotFound) {
    table->set(EntryToIndex(entry), heap->the_hole_value());
    table->set(EntryToValueIndex(entry), heap->the_hole_value());
    table->ElementRemoved();
  }
}


void WeakHashTable::Compact(Isolate* isolate) {
  Heap* heap = isolate->heap();
  WeakHashTable* table = this;
  ...
  for (uint32_t i = 0; i < capacity; i++) {
    ...
    if (WeakCell::cast(key)->cleared()) {
      table->set(key_index, heap->the_hole_value());
      table->set(value_index, heap->the_hole_value());
      table->ElementRemoved();
    }
  }
}
...
void JSArrayBuffer::FreeBackingStore() {
  ...
  // <YUNOS> START:
  if (is_shared_process()) {
    ArrayBuffer::Allocator::FreeShared(allocation_base(), length);
  } else {
    // get the array-buffer-allocator according to the allocator-id
    v8::ArrayBuffer::Allocator* allocator =
        GetIsolate()->array_buffer_allocator(allocator_id());
    allocator->Free(allocation_base(), length, mode);
  }
  // <YUNOS> END
...
void JSArrayBuffer::Setup(Handle<JSArrayBuffer> array_buffer, Isolate* isolate,
                          ...
                          SharedFlag shared, bool is_shared_process) {
  return Setup(array_buffer, isolate, is_external, data, allocated_length, data,
               allocated_length, shared, is_shared_process);
}
...
void JSArrayBuffer::Setup(Handle<JSArrayBuffer> array_buffer, Isolate* isolate,
  ...
  array_buffer->set_is_shared_process(is_shared_process);
  // Encode allocator's id into the array-buffer object.
  array_buffer->set_allocator_id(isolate->GetCurrentArrayBufferAllocatorID());
  ...
  if (allocated_length != 0) {
    ...
    if (shared == SharedFlag::kShared)
      isolate->counters()->shared_array_allocations()->AddSample(
          ConvertToMb(allocated_length));
    if (initialize) {
      data = is_shared_process ?
          ArrayBuffer::Allocator::AllocateShared(allocated_length) :
          allocator->Allocate(allocated_length);
    } else {
      data = is_shared_process ?
          ArrayBuffer::Allocator::AllocateUninitializedShared(allocated_length) :
          allocator->AllocateUninitialized(allocated_length);
    }
    if (data == NULL) {
...
bool JSArrayBuffer::SetupAllocatingDataSharedByProcess(
    Handle<JSArrayBuffer> array_buffer, Isolate* isolate,
    key_t key, size_t allocated_length, bool initialize) {
  void* data;
  CHECK(isolate->array_buffer_allocator() != NULL);
  // Prevent creating array buffers when serializing.
  DCHECK(!isolate->serializer_enabled());
  if (allocated_length != 0) {
    if (initialize) {
      data = ArrayBuffer::Allocator::AllocateShared(allocated_length, key);
    } else {
      data = ArrayBuffer::Allocator::AllocateUninitializedShared(
          allocated_length, key);
    }
...
Handle<JSArrayBuffer> JSTypedArray::MaterializeArrayBuffer(
  ...
  // <YUNOS> : begin
  // Allocator_id should be update here, since the allocator_id may initialized
  // inexactly in JSArrayBuffer::Setup, as the data in Setup may be null, so we
  // should set the allocator_id to the alloctor which malloc buffer.
  buffer->set_allocator_id(isolate->GetCurrentArrayBufferAllocatorID());
  // <YUNOS> : end
  // Initialize backing store at last to avoid handling of |JSArrayBuffers| that
  // are currently being constructed in the |ArrayBufferTracker|. The
  // registration method below handles the case of registering a buffer that has

...
/* YUNOS BEGIN */
/* module(vm):wanglin@alibaba-inc.com */
#include "src/memfd.h"
/* YUNOS END */
...
size_t SnapshotCreator::AddTemplate(Local<Template> template_obj) {
  ...
  i::ApiObjectAccessScope aoa(isolate);
  DCHECK(!data->created_);
...
void ResourceConstraints::ConfigureDefaults(uint64_t physical_memory,
  ...
  set_max_old_space_capacity(i::Heap::kMaxOldSpaceCapacityLowMemoryDevice);
...
void SetResourceConstraints(i::Isolate* isolate,
  ...
  int max_old_space_capacity = constraints.max_old_space_capacity();
  size_t code_range_size = constraints.code_range_size();
  size_t max_pool_size = constraints.max_zone_pool_size();
  if (max_old_space_capacity < old_space_size) {
    max_old_space_capacity = old_space_size;
  }
  if (semi_space_size != 0 || old_space_size != 0 || code_range_size != 0 ||
      max_old_space_capacity != 0) {
    isolate->heap()->ConfigureHeap(semi_space_size, old_space_size,
                                   max_old_space_capacity, code_range_size);
  }
...
i::Object** V8::CopyPersistent(i::Object** obj) {
  i::ApiObjectAccessScope aoa(i::GlobalHandles::IsolateFromLocation(obj));
...
void V8::MakeWeak(i::Object** location, void* parameter,
  ...
  i::ApiObjectAccessScope aoa(i::GlobalHandles::IsolateFromLocation(location));
...
void HandleScope::Initialize(Isolate* isolate) {
  ...
  i::ApiObjectAccessScope aoa(internal_isolate);
...
HandleScope::~HandleScope() {
  i::Isolate* internal_isolate = reinterpret_cast<i::Isolate*>(isolate_);
  i::ApiObjectAccessScope aoa(internal_isolate);
...
i::Object** HandleScope::CreateHandle(i::HeapObject* heap_object,
  ...
  i::Isolate* isolate = heap_object->GetIsolate();
  i::ApiObjectAccessScope aoa(isolate);
...
i::Object** EscapableHandleScope::Escape(i::Object** escape_value) {
  ...
  i::ApiObjectAccessScope aoa(heap->isolate());
...
void Template::Set(v8::Local<Name> name, v8::Local<Data> value,
  ...
  // <YUNOS> changed: begin
  // TODO: revert it after node update
  // more detal at https://github.com/v8/v8/commit/4de391ef6dd65f8694d2d6d17a41535a4468ba5d
...
Local<FunctionTemplate> FunctionTemplate::New(
  ...
  i::ApiObjectAccessScope aoa(i_isolate);
...
Local<ObjectTemplate> ObjectTemplate::New(
  ...
  i::ApiObjectAccessScope aoa(reinterpret_cast<i::Isolate*>(isolate));
...
Local<ObjectTemplate> ObjectTemplate::New() {
  i::Isolate* isolate = i::Isolate::Current();
  i::ApiObjectAccessScope aoa(isolate);
  return New(isolate, Local<FunctionTemplate>());
}
...
MaybeLocal<UnboundScript> ScriptCompiler::CompileUnboundScript(
    Isolate* v8_isolate, Source* source, CompileOptions options) {
  i::ApiObjectAccessScope aoa(reinterpret_cast<i::Isolate*>(v8_isolate));
  Utils::ApiCheck(
...
MaybeLocal<Script> ScriptCompiler::Compile(Local<Context> context,
  ...
  if (!origin.SourceMapUrl().IsEmpty()) {
    i::String* str = i::String::cast(*Utils::OpenHandle(*(origin.SourceMapUrl())));
    script->set_source_mapping_url(
        *isolate->factory()->
          InternalizeString(i::Handle<i::String>(str)));
  }
...
v8::TryCatch::TryCatch()
  ...
  i::ApiObjectAccessScope aoa(isolate_);
  ResetInternal();
...
bool StackFrame::IsEval() const {
  i::ApiObjectAccessScope aoa;
  return Utils::OpenHandle(this)->is_eval();
}
...
bool StackFrame::IsWasm() const {
  i::ApiObjectAccessScope aoa;
  return Utils::OpenHandle(this)->is_wasm();
}
...
bool Value::IsFunction() const {
  i::ApiObjectAccessScope aoa;
  return Utils::OpenHandle(this)->IsCallable();
}
...
bool Value::IsObject() const {
  i::ApiObjectAccessScope aoa;
  return Utils::OpenHandle(this)->IsJSReceiver();
}
...
bool Value::IsProxy() const {
  i::ApiObjectAccessScope aoa;
  return Utils::OpenHandle(this)->IsJSProxy();
}
...
bool Value::IsPromise() const {
  i::ApiObjectAccessScope aoa;
  return Utils::OpenHandle(this)->IsJSPromise();
}
...
bool v8::Object::ForceSet(v8::Local<Value> key, v8::Local<Value> value,
  ...
  i::ApiObjectAccessScope aoa;
  i::Isolate* isolate = Utils::OpenHandle(this)->GetIsolate();
  ENTER_V8_NO_SCRIPT(isolate, v8::Local<v8::Context>(), Object, ForceSet,
                     false, i::HandleScope);
...
HeapStatistics::HeapStatistics()
    : total_heap_size_(0),
      ...
      does_zap_garbage_(0),
      // <YUNOS> changed: begin
      memory_allocator_used_(0),
      memory_allocator_available_(0),
      new_space_size_(0),
      new_space_used_(0),
      new_space_available_(0),
      new_space_committed_physical_(0),
      new_space_committed_(0),
      old_space_size_(0),
      old_space_used_(0),
      old_space_available_(0),
      old_space_committed_physical_(0),
      old_space_committed_(0),
      code_space_size_(0),
      code_space_used_(0),
      code_space_available_(0),
      code_space_committed_physical_(0),
      code_space_committed_(0),
      map_space_size_(0),
      map_space_used_(0),
      map_space_available_(0),
      map_space_committed_physical_(0),
      map_space_committed_(0),
      lo_space_size_(0),
      lo_space_used_(0),
      lo_space_available_(0),
      lo_space_committed_physical_(0),
      lo_space_committed_(0),
      mutable_space_size_(0),
      mutable_space_used_(0),
      mutable_space_available_(0),
      mutable_space_committed_physical_(0),
      mutable_space_committed_(0),
      shared_old_space_size_(0),
      shared_old_space_used_(0),
      shared_old_space_available_(0),
      shared_old_space_committed_physical_(0),
      shared_old_space_committed_(0),
      shared_code_space_size_(0),
      shared_code_space_used_(0),
      shared_code_space_available_(0),
      shared_code_space_committed_physical_(0),
      shared_code_space_committed_(0),
      shared_map_space_size_(0),
      shared_map_space_used_(0),
      shared_map_space_available_(0),
      shared_map_space_committed_physical_(0),
      shared_map_space_committed_(0),
      shared_lo_space_size_(0),
      shared_lo_space_used_(0),
      shared_lo_space_available_(0),
      shared_lo_space_committed_physical_(0),
      shared_lo_space_committed_(0),
      heap_available_(0),
      amount_of_external_allocated_memory_(0),
      scavenger_gc_count_(0),
      full_gc_count_(0) { }
      // <YUNOS> changed: end
...
// <YUNOS> changed: begin
void v8::Isolate::RequestGC(RequestGCType type) {
  ...
  switch(type) {
    ...
    case kMedium:
         ...
         break;
    case kHeavy:
         isolate->heap()->CollectAllAvailableGarbage(i::GarbageCollectionReason::kTesting);
         break;
    default:
         v8::internal::PrintF("Unsupported RequestGCType!\n");
         break;
  }
}


void v8::Isolate::SwitchWorkStatus(WorkStatus type) {
  i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
  switch (type) {
    case STATUS_BACKGROUND:
         isolate->heap()->SwitchWorkStatus(
           v8::internal::Heap::STATUS_BACKGROUND);
         break;
    case STATUS_FOREGROUND:
         isolate->heap()->SwitchWorkStatus(
           v8::internal::Heap::STATUS_FOREGROUND);
         break;
    default:
         v8::internal::PrintF("Unsupported type!\n");
    break;
  }
}


void Isolate::MarkSeedHeapPreFork() {
  if (i::FLAG_enable_seed_heap) {
    i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
    isolate->heap()->MarkSeedHeapPreFork();
    if (i::FLAG_flatten_source_code_for_seed) {
      i::FLAG_flatten_source_code_for_seed = false;
    }
  }
}


void Isolate::ConfigMultiThreadsContextPostFork() {
  i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
  if (i::FLAG_enable_concurrent_threads_after_fork) {
    i::FLAG_concurrent_sweeping = true;
    i::FLAG_parallel_compaction = true;
    if (!i::FLAG_concurrent_recompilation) {
      i::FLAG_concurrent_recompilation = true;
      isolate->InitializeOptimizingCompileDispatcher();
    }
  }
}


void v8::Isolate::ResetMaxOldGenerationSize() {
  i::Isolate* isolate = i::Isolate::Current();
  isolate->heap()->ResetMaxOldGenerationSize();
}


void v8::Isolate::SetInteractiveModeHeapStrategy() {
  i::Isolate* isolate = i::Isolate::Current();
  isolate->heap()->SetInteractiveModeHeapStrategy();
}


void v8::V8::ForceSetArrayBufferAllocator(
    ArrayBuffer::Allocator* allocator) {
  i::Isolate* isolate = i::Isolate::Current();
  isolate->ForceSetArrayBufferAllocator(allocator);
}


void v8::Isolate::NotifyVmEnterKeyScene() {
  i::Isolate* isolate = i::Isolate::Current();
  // Disable crankshaft
  if (i::FLAG_delay_optimized_compiler &&
      !isolate->flags().during_delay_jit()) {
    i::PrintF("Enter key scene!");
    isolate->flags().set_during_delay_jit(true);
    isolate->set_delay_jit_ticks(base::TimeTicks::Now());
  }
  return;
}


void v8::Isolate::NotfiyVmExitKeyScene() {
  i::Isolate* isolate = i::Isolate::Current();
  if (i::FLAG_delay_optimized_compiler &&
      !isolate->delay_jit_ticks().IsNull() &&
      isolate->flags().during_delay_jit()) {
    i::PrintF("Exit key scene!");
    isolate->flags().set_during_delay_jit(false);
    isolate->set_delay_jit_ticks(base::TimeTicks());
  }
  return;
}
// <YUNOS> changed: end
...
bool v8::String::CanMakeExternal() {
  ...
  // shared string can not be externalized
  if (obj->is_shared_object() && !isolate->single_thread_mode())
    return false;
...
/* YUNOS BEGIN */
/* module(vm):wanglin@alibaba-inc.com */
#define LENGTH_KEY_NAME 16
#define MAX_SABEX_NUM 16

std::map<void*, int> v8::ArrayBuffer::Allocator::shared_array_buffer_ex_fd_;

v8::ArrayBuffer::Allocator::~Allocator() {
  // clean the memfd map.
  for (auto& buffer : shared_array_buffer_ex_fd_) {
    int fd = buffer.second;
    if (fd <= 0) {
      continue;
    }
    off_t fsize = lseek(fd, 0, SEEK_END);
    unsigned int length = static_cast<unsigned int>(fsize -
                                      LENGTH_KEY_NAME + sizeof(unsigned int));
    FreeShared(buffer.first, length);
  }
  shared_array_buffer_ex_fd_.clear();
}


int v8::ArrayBuffer::Allocator::GetSharedHandle(void *data) {
  return shared_array_buffer_ex_fd_[data];
}


void* v8::ArrayBuffer::Allocator::AllocateShared(size_t length, int key) {
  return AllocateUninitializedShared(length, key);
}


void* v8::ArrayBuffer::Allocator::AllocateUninitializedShared(size_t length,
                                                              int key) {
  if (key == 0) {
    // if key(fd) is 0,then we should create a new memfd.
    pid_t pid = getpid();
    // the format of name if "/XXXXXXXYYYYYYYY",the former is pid and the later
    // is a random number get from /dev/urandom for security.
    char name[LENGTH_KEY_NAME + 1] = {0};
    char randstr[8] = {0};
    int fd = open("/dev/urandom", O_RDONLY, 0644);
    if (fd >= 0) {
      ssize_t ret = read(fd, randstr, sizeof(randstr));
      if (ret < static_cast<ssize_t>(sizeof(randstr))) {
        i::PrintF("Generate name for SAB failed!\n");
      }
      close(fd);
    } else {
      struct timespec stime;
      stime.tv_sec = 0;
      stime.tv_nsec = 10*1000;  // sleep 10us and retry
      ...
      if (fd >= 0) {
        ...
        i::PrintF("Open /dev/urandom failed!\n");
        return NULL;
      }
    }
    snprintf(name, LENGTH_KEY_NAME + 1, "/%07d%02x%02x%02x%02x", pid, randstr[0],
             randstr[1], randstr[2], randstr[3]);
    size_t infosize = LENGTH_KEY_NAME + sizeof(unsigned int);
    size_t size = infosize + length;
    fd = memfd_create(name, MFD_ALLOW_SEALING);
    if (fd < 0) {
      i::PrintF("Create memfd failed!\n");
      i::PrintF("errno = %d: %s\n", errno, strerror(errno));
      return NULL;
    }
    if (ftruncate(fd, size) == -1) {
      i::PrintF("Setting size of memfd failed!\n");
      return NULL;
    }
    // change size or add new seal is not allowed.
    int new_seals = F_SEAL_SHRINK|F_SEAL_GROW|F_SEAL_SEAL;
    int error = fcntl(fd, F_ADD_SEALS, new_seals);
    if (error != 0) {
      i::PrintF("Seal memfd failed!\n");
      return NULL;
    }
    void *shm_base = mmap(NULL, size, PROT_WRITE|PROT_READ, MAP_SHARED, fd, 0);
    if (shm_base == NULL) {
      i::PrintF("Mmap memfd failed!\n");
      close(fd);
      return NULL;
    }
    memcpy(shm_base, name, LENGTH_KEY_NAME);
    unsigned int *pinfosize = reinterpret_cast<unsigned int *>(
        static_cast<int8_t*>(shm_base) + LENGTH_KEY_NAME);
    *pinfosize = static_cast<unsigned int>(infosize);

    // create mutex for the buffer.
    sem_t *sem = sem_open(name, O_CREAT|O_RDWR, 0660, 0);
    if (sem == SEM_FAILED) {
      i::PrintF("Create semaphore failed! %d\n", errno);
      munmap(shm_base, size);
      close(fd);
      return NULL;
    }
    void *pdata = reinterpret_cast<void *>(static_cast<int8_t*>(shm_base)
        + infosize);
    shared_array_buffer_ex_fd_[pdata] = fd;
    return pdata;
  } else {
    // just use the key to get the  shm
    int fd = key;
    char name[LENGTH_KEY_NAME + 1] = {0};
    size_t infosize = LENGTH_KEY_NAME + sizeof(unsigned int);
    size_t size = infosize + length;
    off_t fsize = lseek(fd, 0, SEEK_END);
    if (size != static_cast<size_t>(fsize)) {
    }
    void *shm_base = mmap(NULL, size, PROT_WRITE|PROT_READ, MAP_SHARED, fd, 0);
    if (shm_base == NULL) {
      i::PrintF("Mmap memfd failed!\n");
    ...
    memcpy(name, shm_base, LENGTH_KEY_NAME);
    sem_t *sem = sem_open(name, 0);
    if (sem == SEM_FAILED) {
      i::PrintF("Open semaphore failed! %d\n", errno);
      munmap(shm_base, size);
      close(fd);
      return NULL;
    }
...
void v8::ArrayBuffer::Allocator::FreeShared(void* data, size_t length) {
  if (data == NULL || length == 0) {
    return;
  }
  char name[LENGTH_KEY_NAME + 1] = {0};
  size_t infosize = LENGTH_KEY_NAME + sizeof(unsigned int);
  void *pdata = reinterpret_cast<void *>(static_cast<int8_t*>(data)
                                         -infosize);
  int fd = shared_array_buffer_ex_fd_[pdata];
  int ret = 0;
  memcpy(name, pdata, 8);
  int pid = static_cast<int>(strtol(name + 1, NULL, 10));
  if (pid == getpid()) {
    //  only the creator could clean the semaphore.
    memcpy(name, pdata, sizeof(name) - 1);
    sem_t *sem = sem_open(name, 0);
    if (sem != SEM_FAILED) {
      ret = sem_close(sem);
      if (ret != 0) {
        i::PrintF("Close semaphore %s failed!\n", name);
      }
      ret = sem_unlink(name);
      if (ret != 0) {
        i::PrintF("Unlink semaphore %s failed!\n", name);
      }
    }
  }
  ret = munmap(pdata, length+infosize);
  if (ret != 0) {
    i::PrintF("FreeShared munmap failed!\n");
  }
  if (pid == getpid()) {
    close(fd);
  }
  shared_array_buffer_ex_fd_.erase(data);
}

#undef LENGTH_KEY_NAME
#undef MAX_SABEX_NUM
/* YUNOS END */
...
v8::SharedArrayBuffer::Contents v8::SharedArrayBuffer::GetContents() {
  ...
  if (self->is_shared_process()) {
    byte_length = ArrayBuffer::Allocator::GetSharedHandle(contents.data_);
  }
...
Local<SharedArrayBuffer> v8::SharedArrayBuffer::New(
    Isolate* isolate, int key, size_t byte_length) {
  CHECK(i::FLAG_harmony_sharedarraybufferex);
  i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
  LOG_API(i_isolate, SharedArrayBuffer, New);
  ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
  i::Handle<i::JSArrayBuffer> obj =
      i_isolate->factory()->NewJSArrayBufferex();
  // TODO(jbroman): It may be useful in the future to provide a MaybeLocal
  // version that throws an exception or otherwise does not crash.
  if (!i::JSArrayBuffer::SetupAllocatingDataSharedByProcess(
        obj, i_isolate, key, byte_length, true)) {
...
void Isolate::SetObjectGroupId(v8::internal::Object** object, v8::UniqueId id) {
...
void Isolate::SetReferenceFromGroup(v8::UniqueId id, v8::internal::Object** object) {
  i::Isolate* internal_isolate = reinterpret_cast<i::Isolate*>(this);
...
void Isolate::SetReference(v8::internal::Object** parent,
                           v8::internal::Object** child) {
  i::Isolate* internal_isolate = reinterpret_cast<i::Isolate*>(this);
...
Isolate* IsolateNewImpl(internal::Isolate* isolate,
  ...
  if (params.array_buffer_allocator != NULL) {
    ...
    isolate->set_array_buffer_allocator(v8::ArrayBuffer::Allocator::NewDefaultAllocator());
  }
  ...
  // JSMT: TODO when worker started, change state
  i::VMThread* self = isolate->vmthread();
  self->TransitionFromRunnableToSuspended(i::kNative);
  return v8_isolate;
}
...
void Isolate::GetHeapStatistics(HeapStatistics* heap_statistics) {
  // Just collect Heap's memory information to HeapStatistics.
  // This API do nothing but collect memory information.
  i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
  ...
  // <YUNOS> changed: begin
  heap_statistics->memory_allocator_used_ =
      isolate->heap()->memory_allocator()->Size();
  heap_statistics->memory_allocator_available_ =
      isolate->heap()->memory_allocator()->Available();

  heap_statistics->new_space_size_ = heap->new_space()->Size();
  heap_statistics->new_space_used_ = heap->new_space()->SizeOfObjects();
  heap_statistics->new_space_available_ = heap->new_space()->Available();
  heap_statistics->new_space_committed_ = heap->new_space()->CommittedMemory();
  heap_statistics->new_space_committed_physical_ =
      heap->new_space()->CommittedPhysicalMemory();

  heap_statistics->old_space_size_ = heap->old_space()->Size();
  heap_statistics->old_space_used_ = heap->old_space()->SizeOfObjects();
  heap_statistics->old_space_available_ = heap->old_space()->Available();
  heap_statistics->old_space_committed_ =
      heap->old_space()->CommittedMemory();
  heap_statistics->old_space_committed_physical_ =
      heap->old_space()->CommittedPhysicalMemory();

  heap_statistics->code_space_size_ = heap->code_space()->Size();
  heap_statistics->code_space_used_ = heap->code_space()->SizeOfObjects();
  heap_statistics->code_space_available_ = heap->code_space()->Available();
  heap_statistics->code_space_committed_ = heap->code_space()->CommittedMemory();
  heap_statistics->code_space_committed_physical_ =
      heap->code_space()->CommittedPhysicalMemory();

  heap_statistics->map_space_size_ = heap->map_space()->Size();
  heap_statistics->map_space_used_ = heap->map_space()->SizeOfObjects();
  heap_statistics->map_space_available_ = heap->map_space()->Available();
  heap_statistics->map_space_committed_ = heap->map_space()->CommittedMemory();
  heap_statistics->map_space_committed_physical_ =
      heap->map_space()->CommittedPhysicalMemory();

  heap_statistics->lo_space_size_ = heap->lo_space()->Size();
  heap_statistics->lo_space_used_ = heap->lo_space()->SizeOfObjects();
  heap_statistics->lo_space_available_ = heap->lo_space()->Available();
  heap_statistics->lo_space_committed_ = heap->lo_space()->CommittedMemory();
  heap_statistics->lo_space_committed_physical_ =
      heap->lo_space()->CommittedPhysicalMemory();

  heap_statistics->mutable_space_size_ = heap->mutable_space()->Size();
  heap_statistics->mutable_space_used_ = heap->mutable_space()->SizeOfObjects();
  heap_statistics->mutable_space_available_ = heap->mutable_space()->Available();
  heap_statistics->mutable_space_committed_ =
      heap->mutable_space()->CommittedMemory();
  heap_statistics->mutable_space_committed_physical_ =
      heap->mutable_space()->CommittedPhysicalMemory();

  heap_statistics->shared_old_space_size_ = heap->shared_old_space()->Size();
  heap_statistics->shared_old_space_used_ = heap->shared_old_space()->SizeOfObjects();
  heap_statistics->shared_old_space_available_ = heap->shared_old_space()->Available();
  heap_statistics->shared_old_space_committed_ =
      heap->shared_old_space()->CommittedMemory();
  heap_statistics->shared_old_space_committed_physical_ =
      heap->shared_old_space()->CommittedPhysicalMemory();

  heap_statistics->shared_code_space_size_ = heap->shared_code_space()->Size();
  heap_statistics->shared_code_space_used_ = heap->shared_code_space()->SizeOfObjects();
  heap_statistics->shared_code_space_available_ = heap->shared_code_space()->Available();
  heap_statistics->shared_code_space_committed_ = heap->shared_code_space()->CommittedMemory();
  heap_statistics->shared_code_space_committed_physical_ =
      heap->shared_code_space()->CommittedPhysicalMemory();

  heap_statistics->shared_map_space_size_ = heap->shared_map_space()->Size();
  heap_statistics->shared_map_space_used_ = heap->shared_map_space()->SizeOfObjects();
  heap_statistics->shared_map_space_available_ = heap->shared_map_space()->Available();
  heap_statistics->shared_map_space_committed_ = heap->shared_map_space()->CommittedMemory();
  heap_statistics->shared_map_space_committed_physical_ =
      heap->shared_map_space()->CommittedPhysicalMemory();

  heap_statistics->shared_lo_space_size_ = heap->shared_lo_space()->Size();
  heap_statistics->shared_lo_space_used_ = heap->shared_lo_space()->SizeOfObjects();
  heap_statistics->shared_lo_space_available_ = heap->shared_lo_space()->Available();
  heap_statistics->shared_lo_space_committed_ = heap->shared_lo_space()->CommittedMemory();
  heap_statistics->shared_lo_space_committed_physical_ =
      heap->shared_lo_space()->CommittedPhysicalMemory();

  heap_statistics->heap_available_ = heap->Available();
  heap_statistics->amount_of_external_allocated_memory_ =
      heap->external_memory();

  heap_statistics->scavenger_gc_count_ = heap->gc_count() - heap->ms_count();
  heap_statistics->full_gc_count_ = heap->ms_count();
  // <YUNOS> changed: end
}
...
void Isolate::LowMemoryNotification() {
  ...
  i::ApiObjectAccessScope aoa(isolate);
  {
    i::IsolateCurrentScope isc(isolate);
...
// <YUNOS> changed: begin
void Isolate::AdviceGC() {
  i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
  isolate->heap()->AdviceGC();
}


void Isolate::PrintChunkInfoInSpaces() {
  i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
  isolate->heap()->PrintChunkInfoInSpaces();
}


void Isolate::ForceScavenger() {
  i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
  isolate->heap()->CollectGarbage(i::NEW_SPACE,
      i::GarbageCollectionReason::kLowMemoryNotification);
}
...
// <YUNOS> changed: begin
void Isolate::DeleteAllSnapshots() {
  reinterpret_cast<i::Isolate*>(this)->heap_profiler()
      ->DeleteAllSnapshots();
}
// <YUNOS> changed: end
struct WorkerSerialization::PrivateData {
  explicit PrivateData(ArrayBuffer::Allocator* array_buffer_allocator)
    : serialization_(array_buffer_allocator) {}

  worker::WorkerSerialization serialization_;
};


WorkerSerialization::WorkerSerialization(ArrayBuffer::Allocator*
    array_buffer_allocator)
    : private_(new PrivateData(array_buffer_allocator)) {}

WorkerSerialization::~WorkerSerialization() { delete private_; }


WorkerSerializationData* WorkerSerialization::SerializeValue(Isolate* isolate,
    Local<Value> value) {
  worker::SerializationData* out_data =
      private_->serialization_.SerializeValue(isolate, value);
  return reinterpret_cast<WorkerSerializationData*>(out_data);
}


WorkerSerializationData* WorkerSerialization::SerializeValue(Isolate* isolate,
    Local<Value> value, Local<Array> transfer) {
  ApiObjectAccessScope aoa(isolate);
  worker::SerializationData* out_data =
      private_->serialization_.SerializeValue(isolate, value, transfer);
  return reinterpret_cast<WorkerSerializationData*>(out_data);
}


MaybeLocal<Value> WorkerSerialization::DeserializeValue(Isolate* isolate,
    WorkerSerializationData* data) {
  ApiObjectAccessScope aoa(isolate);
  worker::SerializationData* in_data = reinterpret_cast<worker::SerializationData*>(data);
  return private_->serialization_.DeserializeValue(isolate, in_data);
}


void WorkerSerialization::CleanupExternalizedSharedContents() {
  private_->serialization_.CleanupExternalizedSharedContents();
}


void WorkerSerialization::CleanupSerializationData(WorkerSerializationData* in_data) {
  worker::SerializationData* data = reinterpret_cast<worker::SerializationData*>(in_data);
  private_->serialization_.CleanupSerializationData(data);
}


ApiObjectAccessScope::ApiObjectAccessScope()
    : isolate_(reinterpret_cast<Isolate*>(i::Isolate::Current())) {
  i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate_);
  i::VMThreadState old_state = i::ApiObjectAccessScope::EnterSafe(i_isolate);
  old_state_ = static_cast<int>(old_state);
}


ApiObjectAccessScope::ApiObjectAccessScope(Isolate* isolate)
    : isolate_(isolate) {
  i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
  i::VMThreadState old_state = i::ApiObjectAccessScope::Enter(i_isolate);
  old_state_ = static_cast<int>(old_state);
}
...
ApiObjectAccessScope::~ApiObjectAccessScope() {
  i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate_);
  i::ApiObjectAccessScope::Exit(i_isolate, static_cast<i::VMThreadState>(old_state_));
}


Isolate* Isolate::CreateNewThreadWorker(const Isolate::CreateParams& params,
  WorkerStartCallback callback) {
  i::Isolate* parent_isolate = reinterpret_cast<i::Isolate*>(this);
  i::Isolate* isolate = parent_isolate->worker_runtime()->CreateNewWorker(callback);
  Isolate* v8_isolate = reinterpret_cast<Isolate*>(isolate);
  if (params.array_buffer_allocator == NULL) {
    isolate->set_array_buffer_allocator(parent_isolate->array_buffer_allocator());
  } else {
    isolate->set_array_buffer_allocator(params.array_buffer_allocator);
  }
if (params.entry_hook) {
  ...
  SetResourceConstraints(isolate, params.constraints);
  if (parent_isolate->is_main_isolate()) {
    i::VMThread* self = parent_isolate->vmthread();
    if (self->state() == i::kRunnable) {
      self->TransitionFromRunnableToSuspended(i::kNative);
    }
  }
  // JSMT: TODO suport more params
  isolate->WorkerStart();
  return v8_isolate;
}
...
void Isolate::JoinAll() {
  i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
  CHECK(isolate->is_main_isolate());
  /*Stop all not set skip_join_all  threads*/
  isolate->worker_runtime()->thread_list()->JoinAllJsThreads();
}


void Isolate::MarkCurrentThreadSkipJoinAll() {
  i::Isolate* isolate = reinterpret_cast<i::Isolate*>(this);
  isolate->vmthread()->set_skip_join_all(true);;
}
...
int debug::Script::Id() const {
  i::ApiObjectAccessScope aoa;
  return Utils::OpenHandle(this)->id();
}
...
void CpuProfile::Serialize(OutputStream* stream,
  ...
  Utils::ApiCheck(format == kJSON,
                  "v8::CpuProfile::Serialize",
                  "Unknown serialization format");
  Utils::ApiCheck(stream->GetChunkSize() > 0,
                  "v8::CpuProfile::Serialize",
                  "Invalid stream chunk size");
  i::CpuProfileJSONSerializer serializer(
...
void InvokeAccessorGetterCallback(
    ...
    v8::AccessorNameGetterCallback getter, AccessorInfo* callback) {
  // Leaving JavaScript.
  Isolate* isolate = reinterpret_cast<Isolate*>(info.GetIsolate());
  FastApiCallScope fas(isolate->vmthread(), callback->is_fast_accessor());

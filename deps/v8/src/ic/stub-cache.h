...
class TestReference {
 public:
  ...
  explicit TestReference(Address address) : address_(address) {}
 private:
  Address address_;
};
...
class StubCache {
 public:
  ...
  static int key_offset(StubCache::Table table) {
    return  first_entry(table) + offsetof(Entry, key);
  }
 
  static int map_offset(StubCache::Table table) {
    return  first_entry(table) + offsetof(Entry, map);
  }
 
  static int value_offset(StubCache::Table table) {
    return  first_entry(table) + offsetof(Entry, value);
  }

  static int first_entry(StubCache::Table table) {
    switch (table) {
      case StubCache::kPrimary:
        return offsetof(StubCache, primary_);
      case StubCache::kSecondary:
        return offsetof(StubCache, secondary_);
    }
    UNREACHABLE();
  }

...
void AccessorAssembler::GenericPropertyLoad(Node* receiver, Node* receiver_map,
  ...
  if (use_stub_cache == kUseStubCache) {
    ...
    Node* stub_cache = LoadTL(ThreadLocalTop::kLoadStubCacheAddress);
    TryProbeStubCache(stub_cache, receiver, p->name,
...
void AccessorAssembler::TryProbeStubCacheTable(Node* stub_cache,
  ...
  const int key_offset = StubCache::key_offset(table);
  // Check that the key in the entry matches the name.
  Node* entry_key = Load(MachineType::Pointer(), stub_cache,
                    IntPtrAdd(entry_offset, IntPtrConstant(key_offset)));
  GotoIf(WordNotEqual(name, entry_key), if_miss);

  // Get the map entry from the cache.
  Node* entry_map =
        Load(MachineType::Pointer(), stub_cache,
        IntPtrAdd(entry_offset, IntPtrConstant(kPointerSize * 2 + key_offset)));
  GotoIf(WordNotEqual(map, entry_map), if_miss);

  DCHECK_EQ(kPointerSize, StubCache::value_offset(table) - key_offset);
  Node* handler = Load(MachineType::TaggedPointer(), stub_cache,
...
void AccessorAssembler::TryProbeStubCache(Node* stub_cache, Node* receiver,
...
void AccessorAssembler::LoadIC_Noninlined(const LoadICParameters* p,
...

  {
    ...
    Node* stub_cache = LoadTL(ThreadLocalTop::kLoadStubCacheAddress);
    TryProbeStubCache(stub_cache, p->receiver, p->name,
...
void AccessorAssembler::StoreIC(const StoreICParameters* p,
  ...
  BIND(&try_megamorphic);
  {
    ...
    Node* store_stub_cache = LoadTL(ThreadLocalTop::kStoreStubCacheAddress);
    TryProbeStubCache(store_stub_cache, p->receiver, p->name,
                      &if_handler, &var_handler, &miss);
  }
  BIND(&try_uninitialized);
  {

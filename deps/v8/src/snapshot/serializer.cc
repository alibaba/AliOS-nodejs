...
Serializer::Serializer(Isolate* isolate)
  ...
  for (int i = 0; i < kNumberOfPreallocatedSpaces; i++) {
    pending_chunk_[i] = 0;
    SerializerSpace space = static_cast<SerializerSpace>(i);
    AllocationSpace allocation_space = SerializerSpaceToAllocationSpace(space);
    max_chunk_size_[i] = static_cast<uint32_t>(
        MemoryAllocator::PageAreaSize(allocation_space));
  }
...
void Serializer::OutputStatistics(const char* name) {
  ...
  for (int space = 0; space < kNumberOfSpaces; space++) {
    PrintF("%16s", SerializerSpaceName(static_cast<SerializerSpace>(space)));
  }
  PrintF("\n");
...
void Serializer::EncodeReservations(
  ...
  out->last().mark_as_last();
  out->Add(SerializedData::Reservation(num_shared_maps_ * Map::kSize));
  out->last().mark_as_last();
}

#ifdef DEBUG
bool Serializer::BackReferenceIsAlreadyAllocated(
    SerializerReference reference, bool is_shared) {
  DCHECK(reference.is_back_reference());
  SerializerSpace space = reference.space();
  if (space == kSerializerLoSpace) {
    ...
    if (is_shared) {
      return reference.map_index() < num_shared_maps_;
    } else {
...
void Serializer::PutBackReference(HeapObject* object,
  ...
  DCHECK(BackReferenceIsAlreadyAllocated(reference, object->is_shared_object()));
  sink_.PutInt(reference.back_reference(), "BackRefValue");
  if (reference.space() == kSerializerMapSpace) {
    sink_.Put(object->is_shared_object()?
        NOT_EXECUTABLE_SHARED : NOT_EXECUTABLE_NOT_SHARED, "if a shared map ?");
  }
  hot_objects_.Add(object);
}
...
SerializerReference Serializer::AllocateSharedMap() {
  // Maps are allocated one-by-one when deserializing.
  return SerializerReference::SharedMapReference(num_shared_maps_++);
}

SerializerReference Serializer::Allocate(SerializerSpace space, int size) {
...
void Serializer::ObjectSerializer::SerializePrologue(SerializerSpace space,
                                                     int size, Map* map) {
  ...
  if (space == kSerializerLoSpace) {
    ...
    if (object_->IsCode()) {
      if (object_->is_shared_object())
        sink_->Put(EXECUTABLE_SHARED, "executable shared large object");
      else
        sink_->Put(EXECUTABLE_NOT_SHARED, "executable large object");
    } else {
      if (object_->is_shared_object())
        sink_->Put(NOT_EXECUTABLE_SHARED, "not executable shared large object");
      else
        sink_->Put(NOT_EXECUTABLE_NOT_SHARED, "not executable large object");
    }
    ...
    sink_->PutInt(size >> kObjectAlignmentBits, "ObjectSizeInWords");
    sink_->Put(NOT_EXECUTABLE_NOT_SHARED, "normal map");
  } else if (space == kSerializerSharedMapSpace) {
    DCHECK_EQ(Map::kSize, size);
    back_reference = serializer_->AllocateSharedMap();
    // JSMT:because the encode just suppot 8 kinds of kSerializerMaps
    // so we did not use kSerializerSharedMapSpace in snapshot
    // but use a another tag to show if we is shared
    sink_->Put(kNewObject + reference_representation_ + kSerializerMapSpace, "NewMap");
    // This is redundant, but we include it anyways.
    sink_->PutInt(size >> kObjectAlignmentBits, "ObjectSizeInWords");
    sink_->Put(NOT_EXECUTABLE_SHARED, "shared map");
  } else {
...
void Serializer::ObjectSerializer::SerializeExternalStringAsSequentialString() {
  ...
  SerializerSpace space =
      (allocation_size > kMaxRegularHeapObjectSize) ?
      kSerializerLoSpace : kSerializerOldSpace;
  if (string->is_shared_object()) {
    space = kSerializerSharedOldSpace;
  }
...
void Serializer::ObjectSerializer::SerializeContent() {
  ...
  Map* map = object_->map();
  Space* heap_space = MemoryChunk::FromAddress(object_->address())->owner();
  AllocationSpace allocation_space = heap_space->identity();
  SerializerSpace space = SelectSerializerSpace(allocation_space);

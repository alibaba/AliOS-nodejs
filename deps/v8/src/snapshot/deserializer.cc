...
void Deserializer::DecodeReservation(
  ...
  DCHECK_EQ(0, reservations_[kSerializerNewSpace].size());
  STATIC_ASSERT(kSerializerNewSpace == 0);
  int current_space = kSerializerNewSpace;
  for (auto& r : res) {
    reservations_[current_space].push_back({r.chunk_size(), NULL, NULL});
    if (r.is_last()) current_space++;
  }
  // Now we should have 9 spaces, but shared_map_space is not include in kNumberOfSpaces
  DCHECK_EQ(kNumberOfReservations, current_space);
...
void Deserializer::Deserialize(Isolate* isolate) {
...

  {
    ...
    isolate_->heap()->IterateStrongRoots(this, VISIT_ONLY_STRONG_FOR_DESERIALIZATION);
...
Deserializer::~Deserializer() {
  ...
  CHECK_EQ(shared_allocated_maps_.length(), shared_next_map_index_);
#endif  // DEBUG
}
...
void Deserializer::VisitRootPointers(Root root, Object** start, Object** end) {
  ...
  ReadData(start, end, kSerializerNewSpace, NULL);
}
  ...
  for (int code = source_.Get(); code != kSynchronize; code = source_.Get()) {
    switch (code) {
      ...
      default: {
        ...
        DCHECK(space <= kNumberOfReservations);
...
HeapObject* Deserializer::PostProcessNewObject(HeapObject* obj, int space) {
  ...
  if (obj->IsAllocationSite()) {
    ...
    if (deserializing_user_code() || space == kSerializerLoSpace) {
...
void Deserializer::CommitPostProcessedObjects(Isolate* isolate) {
  ...
  for (Handle<Script> script : new_scripts_) {
    ...
    if (script->is_shared_object()) {
      ...
      heap->SetRootSharedScriptList(*list);
    } else {
...
HeapObject* Deserializer::GetBackReferencedObject(int space) {
  ...
  if (space == kSerializerLoSpace) {
    uint32_t index = back_reference.large_object_index();
    obj = deserialized_large_objects_[index];
  } else if (space == kSerializerMapSpace) {
    int index = back_reference.map_index();
    byte source_code = source_.Get();
    Sharability sharable =
      static_cast<Sharability>((source_code & kSharabilityMask) >> kSharabilityShift);
    if (sharable == SHARED) {
      DCHECK(index < shared_next_map_index_);
      obj = HeapObject::FromAddress(shared_allocated_maps_[index]);
    } else {
    ...
    CHECK(space != kSerializerSharedMapSpace);
...
#ifdef DEBUG
  if (obj->IsCode()) {
    DCHECK(space_number == kSerializerCodeSpace
           || space_number == kSerializerSharedCodeSpace
           || space_number == kSerializerLoSpace);
  } else {
    DCHECK(space_number != kSerializerCodeSpace
           || space_number == kSerializerSharedCodeSpace);
  }
...
Address Deserializer::Allocate(int space_index, int size) {
  if (space_index == kSerializerLoSpace) {
    AlwaysAllocateScope scope(isolate_);
    LargeObjectSpace* lo_space;
    byte source_code = source_.Get();
    Executability exec =
      static_cast<Executability>((source_code & kExecutabilityMask) >> kExecutabilityShift);
    Sharability sharable =
      static_cast<Sharability>((source_code & kSharabilityMask) >> kSharabilityShift);
    if (sharable == SHARED)
      lo_space = isolate_->heap()->shared_lo_space();
    else
      lo_space = isolate_->heap()->lo_space();
    AllocationResult result = lo_space->AllocateRaw(size, exec);
    HeapObject* obj = result.ToObjectChecked();
    deserialized_large_objects_.Add(obj);
    return obj->address();
  } else if (space_index == kSerializerMapSpace) {
    DCHECK_EQ(Map::kSize, size);
    byte source_code = source_.Get();
    Sharability sharable =
      static_cast<Sharability>((source_code & kSharabilityMask) >> kSharabilityShift);
    if (sharable == SHARED) {
      return shared_allocated_maps_[shared_next_map_index_++];
    } else {
...
#endif
    if (space_index == kSerializerCodeSpace ||
        space_index == kSerializerSharedCodeSpace) {
...
bool Deserializer::ReadData(Object** current, Object** limit, int source_space,
  ...
  bool write_barrier_needed =
      (current_object_address != NULL && source_space != kSerializerNewSpace &&
       source_space != kSerializerCodeSpace && source_space != kSerializerSharedCodeSpace);
  while (current < limit) {
...
#define CASE_BODY(where, how, within, space_number_if_any)                     \
  {                                                                            \
    ...
    if (where == kNewObject && how == kPlain && within == kStartOfObject) {    \
      ...
      emit_write_barrier = (space_number == kSerializerNewSpace);                        \
    } else {                                                                   \
...
#define ALL_SPACES(where, how, within)                   \
  CASE_STATEMENT(where, how, within, kSerializerNewSpace)          \
  CASE_BODY(where, how, within, kSerializerNewSpace)               \
  CASE_STATEMENT(where, how, within, kSerializerOldSpace)          \
  CASE_STATEMENT(where, how, within, kSerializerMutableSpace)      \
  CASE_STATEMENT(where, how, within, kSerializerCodeSpace)         \
  CASE_STATEMENT(where, how, within, kSerializerMapSpace)          \
  CASE_STATEMENT(where, how, within, kSerializerSharedOldSpace)    \
  CASE_STATEMENT(where, how, within, kSerializerSharedCodeSpace)   \
  CASE_STATEMENT(where, how, within, kSerializerLoSpace)           \
...
#define SINGLE_CASE(where, how, within, space) \
  ...
  CASE_BODY(where, how, within, space)
      ...
      // code object.
      SINGLE_CASE(kNewObject, kPlain, kInnerPointer, kSerializerCodeSpace)
      SINGLE_CASE(kNewObject, kPlain, kInnerPointer, kSerializerSharedCodeSpace)
      ...
      SINGLE_CASE(kNewObject, kPlain, kInnerPointer, kSerializerOldSpace)
      SINGLE_CASE(kNewObject, kPlain, kInnerPointer, kSerializerSharedOldSpace)
      ...
      SINGLE_CASE(kBackref, kPlain, kInnerPointer, kSerializerCodeSpace)
      SINGLE_CASE(kBackref, kPlain, kInnerPointer, kSerializerSharedCodeSpace)
      SINGLE_CASE(kBackrefWithSkip, kPlain, kInnerPointer, kSerializerCodeSpace)
      SINGLE_CASE(kBackrefWithSkip, kPlain, kInnerPointer, kSerializerSharedCodeSpace)
      // Support for pointers into a cell.
      SINGLE_CASE(kBackref, kPlain, kInnerPointer, kSerializerOldSpace)
      SINGLE_CASE(kBackref, kPlain, kInnerPointer, kSerializerSharedOldSpace)
      SINGLE_CASE(kBackrefWithSkip, kPlain, kInnerPointer, kSerializerOldSpace)
      SINGLE_CASE(kBackrefWithSkip, kPlain, kInnerPointer, kSerializerSharedOldSpace)

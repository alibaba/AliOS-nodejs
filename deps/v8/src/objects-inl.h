...
bool HeapObject::IsDictionary() const {
  return IsHashTable() && this != GetHeapFromAddress()->string_table();
}
...
bool HeapObject::is_shared_object() const {
  return MemoryChunk::FromAddress(
                   reinterpret_cast<Address>(const_cast<HeapObject*>(this)))
                   ->InSharedSpace();
}

Heap* HeapObject::GetHeapFromAddress() const {
...
Heap* HeapObject::GetHeap() const {
  Space* space = MemoryChunk::FromAddress(
                   reinterpret_cast<Address>(const_cast<HeapObject*>(this)))
                   ->owner();
  // Normal space is not shared between heap, so just get the space's heap
  if (!space->sharable()) return space->heap_safe();
  Isolate* isolate = space->heap_safe()->isolate();
  return isolate->GetCurrentHeap();
}

Isolate* HeapObject::GetIsolateFromAddress() const {
  return GetHeapFromAddress()->isolate();
}
...
Isolate* HeapObject::GetIsolate() const {
  Space* space = MemoryChunk::FromAddress(
                   ...
                   ->owner();
  Isolate* isolate = space->heap_safe()->isolate();
  if (!space->sharable()) return space->heap_safe()->isolate();
  return isolate->GetCurrentIsolate();
}
...
#ifdef OPT_HASHTABLE
  // <YUNOS> changed: begin
  // 'at_least_space_for' have already multipled by nof
  if (at_least_space_for > kLargeCapacity) {
    at_least_space_for /= 2;
  }
  // <YUNOS> changed: end
#endif // OPT_HASHTABLE
...
void AbstractCode::set_source_position_table(ByteArray* source_position_table) {
  if (IsCode()) {
    ...
    DCHECK_EQ(source_position_table->is_shared_object(),
              GetBytecodeArray()->is_shared_object());
...
bool JSArrayBuffer::is_shared_process() {
  return IsProcessShared::decode(bit_field());
}


void JSArrayBuffer::set_is_shared_process(bool value) {
  set_bit_field(IsProcessShared::update(bit_field(), value));
}
// <YUNOS> END
...
// <YUNOS> changed:
// getter and setter for encoding external allocator's id
void JSArrayBuffer::set_allocator_id(int id) {
  set_bit_field(OwnerAllocatorIDOffsetField::update(bit_field(), id));
}


// <YUNOS> changed:
// getter and setter for encoding external allocator's id
int JSArrayBuffer::allocator_id() {
  return OwnerAllocatorIDOffsetField::decode(bit_field());
}
...
bool AccessorInfo::is_fast_accessor() {
  return BooleanBit::get(flag(), kFastAccessor);
}

void AccessorInfo::set_fast_accessor(bool value) {
  set_flag(BooleanBit::set(flag(), kFastAccessor, value));
}

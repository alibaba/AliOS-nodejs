...
#define HARMONY_INPROGRESS_BASE(V)                                    \
  ...
  V(harmony_sharedarraybufferex, "harmony sharedarraybufferex")       \
...
// <YUNOS> changed: begin
DEFINE_BOOL(enable_seed_heap, false, "whether enable seed heap")
DEFINE_BOOL(use_mutable_space, true, "allocate mutable object in mutable_space")
// platform-posix.cc
DEFINE_BOOL(output_to_logcat, true, "output log to logcat")
// TODO: This flag is a workaround solution only used to fix the HP-pad memory
// leak bug, it need to be fixed in the furture by root cause.
// ref: https://aone.alibaba-inc.com/issue/9597774
DEFINE_BOOL(disable_pending_phantom_callbacks, false,
            "disable pending phantom callback")
DEFINE_INT(rss_limit, -1,
            "Perform a full gc when RSS is larger than it (in Mbytes).")
DEFINE_INT(rss_step, 2,
            "How much we decrease/increase for rss_limit "
            "when rss_limit is too big/small (in Mbytes).")
DEFINE_BOOL(delay_optimized_compiler, true, "delay optimized compiler")
DEFINE_INT(max_delay_compiler_times, 2000, "max delay optimized compiler time(ms)")
DEFINE_INT(max_old_space_capacity, 0,
           "max size of the old space if large heap enable(in Mbytes)")
DEFINE_BOOL(freeze_code_for_seed, true, "codes in seed are always young")
DEFINE_BOOL(flatten_source_code_for_seed, true,
            "flatten source code in seed in order to share "
            "the flattened source code")
DEFINE_BOOL(expose_logger, false,
            "enable YunOSLogger")
DEFINE_NEG_IMPLICATION(expose_logger, prof)
DEFINE_NEG_IMPLICATION(expose_logger, prof_cpp)
DEFINE_BOOL(enable_concurrent_threads_after_fork, false,
            "whether enable start concurrent threads after fork")
// <YUNOS> changed: end
...
DEFINE_INT(max_shared_space_size, 0, "max size of the shared spaces (in Mbytes)")
...
DEFINE_BOOL(stress_shared_gc, false,
            "replace all allocation fail gc to shared gc")
...
DEFINE_BOOL(verify_shared_object, false, "verify shared object during gc")
DEFINE_BOOL(verify_thread_suspension, true, "verify shared object during gc")
DEFINE_BOOL(heap_for_worker, true, "privat heap for worker with shared space")
...
DEFINE_BOOL(for_cctest, false, "just for cctest")
...
DEFINE_BOOL(test_d8, false, "test d8")
...
DEFINE_INT(rwmonitor_timeout, 30, "rwmonitor timeout in seconds")
DEFINE_BOOL(js_mt_mode, true, "js multithreads mode")
DEFINE_BOOL(deflate_monitors, false, "deflate monitor.")

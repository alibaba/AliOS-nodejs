...
int64_t TurboAssembler::RootRegisterDelta(ExternalReference other) {
  if (FLAG_js_mt_mode) return kInvalidRootRegisterDelta;
  ...
  Address roots_register_value =
      kRootRegisterBias +
      reinterpret_cast<Address>(isolate()->thread_local_top_address());
...
void TurboAssembler::LoadRoot(Register destination, Heap::RootListIndex index) {
  ...
  movp(destination, Operand(kRootRegister,
                            ...
                            Isolate::kHeapRootsDelta));
}

void TurboAssembler::LoadRootAddress(Register destination,
                                     Heap::RootListIndex index) {
  DCHECK(root_array_available_);
  leap(destination, Operand(kRootRegister,
                            (index << kPointerSizeLog2) - kRootRegisterBias +
                            Isolate::kHeapRootsDelta));
}


void MacroAssembler::XORRoot(Register destination, Heap::RootListIndex index) {
  DCHECK(root_array_available_);
  xorp(destination, Operand(kRootRegister,
                            (index << kPointerSizeLog2) - kRootRegisterBias +
                            Isolate::kHeapRootsDelta));
}
...
void MacroAssembler::StoreRoot(Register source, Heap::RootListIndex index) {
  ...
  movp(Operand(kRootRegister, (index << kPointerSizeLog2) - kRootRegisterBias +
          Isolate::kHeapRootsDelta), source);
}


void MacroAssembler::PushRoot(Heap::RootListIndex index) {
  DCHECK(root_array_available_);
  Push(Operand(kRootRegister,
        (index << kPointerSizeLog2) - kRootRegisterBias + Isolate::kHeapRootsDelta));
}
...
// for thread local top
void TurboAssembler::LoadAddressTL(Register destination,
                                   ThreadLocalTop::AddressId index) {
  DCHECK(root_array_available_);
  int32_t offset = ((index) << kPointerSizeLog2);
  leap(destination, Operand(kRootRegister, offset - kRootRegisterBias));
}


void  TurboAssembler::LoadTL(Register destination,
                            ThreadLocalTop::AddressId index) {
  DCHECK(root_array_available_);
  int32_t offset = ((index) << kPointerSizeLog2);
  movp(destination, Operand(kRootRegister, offset - kRootRegisterBias));
}


void TurboAssembler::StoreTL(Register source,
                             ThreadLocalTop::AddressId index) {
  DCHECK(root_array_available_);
  int32_t offset = ((index) << kPointerSizeLog2);
  movp(Operand(kRootRegister, offset - kRootRegisterBias), source);
}

void TurboAssembler::StoreTL(ThreadLocalTop::AddressId index,
                             Immediate value) {
  DCHECK(root_array_available_);
  int32_t offset = ((index) << kPointerSizeLog2);
  movp(Operand(kRootRegister, offset - kRootRegisterBias), value);
}


void TurboAssembler::CompareTL(Register with,
                               ThreadLocalTop::AddressId index) {
  DCHECK(root_array_available_);
  int32_t offset = ((index) << kPointerSizeLog2);
  cmpp(with, Operand(kRootRegister, offset - kRootRegisterBias));
}


void TurboAssembler::CompareTL(const Operand& with,
                                 ThreadLocalTop::AddressId index) {
  DCHECK(root_array_available_);
  DCHECK(!with.AddressUsesRegister(kScratchRegister));
  LoadTL(kScratchRegister, index);
  cmpp(with, kScratchRegister);
}

void TurboAssembler::PopTL(ThreadLocalTop::AddressId index) {
  DCHECK(root_array_available_);
  int32_t offset = ((index) << kPointerSizeLog2);
  popq(Operand(kRootRegister, offset - kRootRegisterBias));
}

void TurboAssembler::PushTL(ThreadLocalTop::AddressId index) {
  DCHECK(root_array_available_);
  int32_t offset = ((index) << kPointerSizeLog2);
  Push(Operand(kRootRegister, offset - kRootRegisterBias));
}

void TurboAssembler::PushIsolateAddress() {
  DCHECK(root_array_available_);
  LoadIsolateAddress(kScratchRegister);
  Push(kScratchRegister);
}

void TurboAssembler::LoadIsolateAddress(Register destination) {
  leap(destination, Operand(kRootRegister,
       -Isolate::kThreadLocalTopOffset - kRootRegisterBias));
}
...
void TurboAssembler::InitializeRootRegister() {
  // call VMThread::current_thread_local_top() to get ThreadLocalTop*
  // of current js-thread.
  if (FLAG_js_mt_mode) {
    FrameScope scope(this, StackFrame::MANUAL);
    pushq(kReturnRegister0);
    for (int i = 0; i < kNumberOfSavedRegs; i++) {
      Register reg = saved_regs[i];
      if (!reg.is(kReturnRegister0)) {
        pushq(reg);
      }
    }
    PrepareCallCFunction(0);
    CallCFunction(ExternalReference::current_thread_local_top(isolate()), 0);
    ...
    leap(kRootRegister, Operand(kReturnRegister0, kRootRegisterBias));
    popq(kReturnRegister0);
  } else {
    ExternalReference main_thread_local_top =
        ExternalReference::main_thread_local_top(isolate());
    Move(kRootRegister, main_thread_local_top);
...
void MacroAssembler::PushStackHandler() {
  ...
  PushTL(ThreadLocalTop::kHandlerAddress);

  // Set this new handler as the current one.
  StoreTL(rsp, ThreadLocalTop::kHandlerAddress);
}


void MacroAssembler::PopStackHandler() {
  STATIC_ASSERT(StackHandlerConstants::kNextOffset == 0);
  PopTL(ThreadLocalTop::kHandlerAddress);
...
void MacroAssembler::MaybeDropFrames() {
  ...
  LoadTL(kScratchRegister, ThreadLocalTop::kDebugRestartFpAddress);
  movp(rbx, Operand(kScratchRegister, 0));
  testp(rbx, rbx);
...
void MacroAssembler::CheckDebugHook(Register fun, Register new_target,
                                    ...
                                    const ParameterCount& actual) {
  Label skip_hook;
  LoadTL(kScratchRegister, ThreadLocalTop::kDebugHookOnnFunctionCallAddress);
  cmpb(Operand(kScratchRegister, 0), Immediate(0));
  j(equal, &skip_hook);
  {
...
void MacroAssembler::EnterExitFramePrologue(bool save_rax,
  ...
  StoreTL(rbp, ThreadLocalTop::kCEntryFPAddress);
  StoreTL(rsi, ThreadLocalTop::kContextAddress);
  StoreTL(rbx, ThreadLocalTop::kCFunctionAddress);
}
...
void MacroAssembler::LeaveExitFrameEpilogue(bool restore_context) {
  ...
  if (restore_context) {
    LoadTL(rsi, ThreadLocalTop::kContextAddress);
  }
#ifdef DEBUG
  StoreTL(ThreadLocalTop::kContextAddress, Immediate(0));
#endif

  // Clear the top frame.
  StoreTL(ThreadLocalTop::kCEntryFPAddress, Immediate(0));
}
...
void MacroAssembler::LoadAllocationTopHelper(Register result,
  ...
  Heap::RootListIndex allocation_top =
      AllocationUtils::GetAllocationTopAddressId(flags);
  ...
  if ((flags & RESULT_CONTAINS_TOP) != 0) {
    ...
    CompareRoot(result, allocation_top);
  ...
  if (scratch.is_valid()) {
    LoadRootAddress(scratch, allocation_top);
    movp(result, Operand(scratch, 0));
  } else {
    LoadRoot(result, allocation_top);
  }
}
...
void MacroAssembler::MakeSureDoubleAlignedHelper(Register result,
  ...
  if (kPointerSize == kDoubleSize) {
    ...
    if ((flags & PRETENURE) != 0) {
      Heap::RootListIndex allocation_limit =
          AllocationUtils::GetAllocationLimitAddressId(flags);
      CompareRoot(result, allocation_limit);
...
void MacroAssembler::UpdateAllocationTopHelper(Register result_end,
  ...
  if (scratch.is_valid()) {
    ...
    StoreRoot(result_end, allocation_top);
  }
}
...
void MacroAssembler::Allocate(int object_size,
  ...
  CompareRoot(top_reg, allocation_limit);
  j(above, gc_required);
...
void MacroAssembler::Allocate(Register object_size,
  ...
  CompareRoot(result_end, allocation_limit);
  j(above, gc_required);
...
void MacroAssembler::TestJSArrayForAllocationMemento(
  ...
  Label map_check;
  Label top_check;
  Heap::RootListIndex new_space_allocation_top =
        Heap::kNewSpaceAllocationTopRootIndex;
  ...
  XORRoot(scratch_reg, new_space_allocation_top);
  ...
  bind(&top_check);
  leap(scratch_reg, Operand(receiver_reg, kMementoLastWordOffset));
  CompareRoot(scratch_reg, new_space_allocation_top);

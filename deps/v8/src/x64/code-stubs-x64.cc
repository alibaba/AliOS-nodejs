...
void StoreBufferOverflowStub::Generate(MacroAssembler* masm) {
  ...
  __ LoadIsolateAddress(arg_reg_1);
...
void CEntryStub::Generate(MacroAssembler* masm) {
  ...
  if (result_size() <= kMaxRegisterResultSize) {
    ...
    __ LoadIsolateAddress(kCCallArg2);
  } else {
    ...
    __ LoadIsolateAddress(kCCallArg3);
  }
  __ call(rbx);
  ...
  if (FLAG_debug_code) {
    ...
    __ CompareTL(r14, ThreadLocalTop::kPendingExceptionAddress);
                                 ...
                                 isolate());
  {
    ...
    __ LoadIsolateAddress(arg_reg_3);
    __ PrepareCallCFunction(3);
    __ CallCFunction(find_handler, 3);
  }

  // Retrieve the handler context, SP and FP.
  __ LoadTL(rsi, ThreadLocalTop::kPendingHandlerContextAddress);
  __ LoadTL(rsp, ThreadLocalTop::kPendingHandlerSPAddress);
  __ LoadTL(rbp, ThreadLocalTop::kPendingHandlerFPAddress);
  ...
  __ LoadTL(rdi, ThreadLocalTop::kPendingHandlerCodeAddress);
  __ LoadTL(rdx, ThreadLocalTop::kPendingHandlerOffsetAddress);
...
void JSEntryStub::Generate(MacroAssembler* masm) {
  ...
  __ PushTL(ThreadLocalTop::kCEntryFPAddress);

  // If this is the outermost JS call, set js_entry_sp value.
  __ LoadTL(rax, ThreadLocalTop::kJSEntrySPAddress);
  __ testp(rax, rax);
  __ j(not_zero, &not_outermost_js);
  __ Push(Immediate(StackFrame::OUTERMOST_JSENTRY_FRAME));
  __ movp(rax, rbp);
  __ StoreTL(rax, ThreadLocalTop::kJSEntrySPAddress);
  Label cont;
  __ jmp(&cont);
  ...
  __ StoreTL(rax, ThreadLocalTop::kPendingExceptionAddress);
  ...
  // TODO(JSMT) Clear any pending exceptions
  ...
  __ StoreTL(ThreadLocalTop::kJSEntrySPAddress, Immediate(0));
  __ bind(&not_outermost_js_2);

  // Restore the top frame descriptor from the stack.
  __ PopTL(ThreadLocalTop::kCEntryFPAddress);
...
void RecordWriteStub::Generate(MacroAssembler* masm) {
  ...
  __ LoadRoot(kScratchRegister, Heap::kRecordWriteStubModeRootIndex);
  __ cmpb(kScratchRegister, Immediate(INCREMENTAL));
  __ j(equal, &skip_to_incremental_noncompacting, Label::kNear);
  __ j(greater, &skip_to_incremental_compacting, Label::kFar);
...
static void CallApiFunctionAndReturn(MacroAssembler* masm,
  ...
  Isolate* isolate = masm->isolate();
  Factory* factory = isolate->factory();
  const int kNextOffset = offsetof(HandleScopeData, next);
  const int kLimitOffset = offsetof(HandleScopeData, limit);
  const int kLevelOffset = offsetof(HandleScopeData, level);
  ...
  DCHECK_EQ(kNextOffset, 0);
  __ LoadAddressTL(base_reg, ThreadLocalTop::kHandleScopeDataNextAddress);
  ...
  // JSMT: support change states, we use thunk_ref
  {
  ...
  __ LoadTL(rdi, ThreadLocalTop::kScheduledExceptionAddress);
  __ Cmp(rdi, factory->the_hole_value());

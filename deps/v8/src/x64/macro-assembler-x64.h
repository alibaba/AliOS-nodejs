...
#undef AVX_OP2_WITH_TYPE
  ...
  void LoadRootAddress(Register destination, Heap::RootListIndex index);
  ...
  // Operations on thread local top.
  void LoadAddressTL(Register destination, ThreadLocalTop::AddressId index);
  void LoadTL(Register destination, ThreadLocalTop::AddressId index);
  void LoadTL(const Operand& destination, ThreadLocalTop::AddressId index) {
    LoadTL(kScratchRegister, index);
    movp(destination, kScratchRegister);
  }
  void StoreTL(Register source, ThreadLocalTop::AddressId index);
  void StoreTL(ThreadLocalTop::AddressId index, Immediate value);
  void CompareTL(Register with, ThreadLocalTop::AddressId index);
  void CompareTL(const Operand& with, ThreadLocalTop::AddressId index);
  void PopTL(ThreadLocalTop::AddressId index);
  void PushTL(ThreadLocalTop::AddressId index);
  void LoadIsolateAddress(Register destination);
  void PushIsolateAddress();
...
class MacroAssembler : public TurboAssembler {
 public:
  ...
  void XORRoot(Register destination, Heap::RootListIndex index);

...
void MacroAssembler::LoadRootAddress(Register destination,
                              Heap::RootListIndex index) {
  add(destination, kRootRegister,
        Operand((index << kPointerSizeLog2) + Isolate::kHeapRootsDelta));
}

void TurboAssembler::LoadRoot(Register destination, Heap::RootListIndex index,
                              Condition cond) {
  ldr(destination, MemOperand(kRootRegister, (index << kPointerSizeLog2) + Isolate::kHeapRootsDelta), cond);
}

// for thread local top
void MacroAssembler::LoadAddressTL(Register destination,
...
void MacroAssembler::LoadTL(Register destination,
                            ThreadLocalTop::AddressId index) {
  int32_t offset = ((index) << kPointerSizeLog2);
...
void MacroAssembler::StoreTL(Register source,
                             ThreadLocalTop::AddressId index) {
  int32_t offset = ((index) << kPointerSizeLog2);
  str(source, MemOperand(kRootRegister, offset));
}


void MacroAssembler::StoreTL(ThreadLocalTop::AddressId index, const Operand& source) {
  int32_t offset = ((index) << kPointerSizeLog2);
  mov(ip, source);
  str(ip, MemOperand(kRootRegister, offset));
}


void MacroAssembler::CompareTL(Register with,
                               ThreadLocalTop::AddressId index) {
  DCHECK(!with.is(ip));
  LoadTL(ip, index);
  cmp(with, ip);
...
void MacroAssembler::PopTL(ThreadLocalTop::AddressId index) {
  int32_t offset = ((index) << kPointerSizeLog2);
  Pop(ip);
  str(ip, MemOperand(kRootRegister, offset));
}
...
void MacroAssembler::PushTL(ThreadLocalTop::AddressId index) {
  int32_t offset = ((index) << kPointerSizeLog2);
...
void MacroAssembler::LoadIsolateAddress(Register destination) {
  sub(destination, kRootRegister, Operand(Isolate::kThreadLocalTopOffset));
}
...
void MacroAssembler::StoreRoot(Register source,
  ...
  str(source, MemOperand(kRootRegister, (index << kPointerSizeLog2) + Isolate::kHeapRootsDelta), cond);
}
...
void MacroAssembler::EnterExitFrame(bool save_doubles, int stack_space,
  ...
  StoreTL(fp, ThreadLocalTop::kCEntryFPAddress);
  StoreTL(cp, ThreadLocalTop::kContextAddress);
...
void MacroAssembler::LeaveExitFrame(bool save_doubles, Register argument_count,
  ...
  // Clear top frame.
  mov(r3, Operand::Zero());
  StoreTL(r3, ThreadLocalTop::kCEntryFPAddress);

  // Restore current context from top and clear it in debug mode.
  if (restore_context) {
    LoadTL(cp, ThreadLocalTop::kContextAddress);
  }
#ifdef DEBUG
  StoreTL(r3, ThreadLocalTop::kContextAddress);
#endif
...
void MacroAssembler::CheckDebugHook(Register fun, Register new_target,
  ...
  Label skip_hook;
  LoadTL(r4, ThreadLocalTop::kDebugHookOnnFunctionCallAddress);
...
void MacroAssembler::MaybeDropFrames() {
  ...
  LoadTL(r1, ThreadLocalTop::kDebugRestartFpAddress);
...
void MacroAssembler::PushStackHandler() {
  ...
  StoreTL(sp, ThreadLocalTop::kHandlerAddress);
}
...
void MacroAssembler::Allocate(int object_size,
  ...
  DCHECK((allocation_limit - allocation_top) == 1);
  ...
  LoadRootAddress(top_address, allocation_top);
  ...
  if ((flags & RESULT_CONTAINS_TOP) == 0) {
    ...
    ldr(alloc_limit, MemOperand(top_address, kPointerSize));
  }

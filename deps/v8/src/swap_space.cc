...
template <typename FreeByStartSet, typename FreeBySizeSet, typename FreeBySizeEntry>
static void InsertChunk(FreeByStartSet* free_by_start,
                        FreeBySizeSet* free_by_size,
                        FreeBySizeEntry& free_by_size_entry,
                        const SpaceChunk& chunk) {
  DCHECK_NE(chunk.size, 0u);
  auto insert_result = free_by_start->insert(chunk);
  CHECK(insert_result.second);
  free_by_size_entry = std::make_pair(chunk.size, insert_result.first);
  free_by_size->insert(free_by_size_entry);
}
...
SwapSpace::SwapSpace(int fd, size_t initial_size)
  ...
  InsertChunk(&free_by_start_, &free_by_size_, free_by_size_entry_, NewFileChunk(initial_size));
}
...
static size_t CollectFree(const FreeByStartSet& free_by_start, const FreeBySizeSet& free_by_size) {
  if (free_by_start.size() != free_by_size.size()) {
    PrintF("Error: Size: = \n, free_by_start.size() = %zu\n, free_by_size.size() = %zu\n",
        free_by_start.size(), free_by_size.size());
  }
...
void* SwapSpace::Alloc(size_t size) {
  ...
  if (old_chunk.size != size) {
    ...
    InsertChunk(&free_by_start_, &free_by_size_, free_by_size_entry_, new_chunk);
  }
...
SpaceChunk SwapSpace::NewFileChunk(size_t min_size) {
  ...
  size_t result = TEMP_FAILURE_RETRY(ftruncate64(fd_, size_ + next_part));
  if (result != 0) {
    PrintF("Error: Unable to increase swap file.\n");
  }
  uint8_t* ptr = reinterpret_cast<uint8_t*>(
      mmap(nullptr, next_part, PROT_READ | PROT_WRITE, MAP_SHARED, fd_, size_));
  if (ptr == MAP_FAILED) {
    PrintF("Unable to mmap new swap file chunk. \n");
    PrintF("Current size: %zu\n, requested: %zu\n", next_part, min_size);
    PrintF("Free list:");
    DumpFreeMap(free_by_size_);
    PrintF("In free list:");
    CollectFree(free_by_start_, free_by_size_);
    PrintF("Aborting...\n");
  }
  size_ += next_part;
  ...
  PrintF("Error: No swap file support on the Mac.\n");
...
void SwapSpace::Free(void* ptrV, size_t size) {
  ...
  InsertChunk(&free_by_start_, &free_by_size_, free_by_size_entry_, chunk);

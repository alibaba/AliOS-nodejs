...
void ObjectLiteral::BuildConstantProperties(Isolate* isolate) {
  ...
  for (int i = 0; i < properties()->length(); i++) {
    ...
    if (key->IsString() && String::cast(*key)->AsArrayIndex(&element_index)) {
      key = isolate->factory()->NewNumberFromUint(element_index, pretenure);
    } else if (key->IsNumber() && !key->ToArrayIndex(&element_index)) {
      // JSMT: TODO shared number to string cache
      key = isolate->factory()->NumberToString(key, true, pretenure == SHARED_TENURED);
    }
...
void ArrayLiteral::BuildConstantElements(Isolate* isolate) {
  ...
  Handle<FixedArray> fixed_array =
      isolate->factory()->NewFixedArrayWithHoles(constants_length, pretenure);
  ...
  if (IsDoubleElementsKind(kind)) {
    ...
    elements = isolate->factory()->NewFixedDoubleArray(constants_length, pretenure);

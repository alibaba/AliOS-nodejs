...
class RunnableConfig(GraphConfig):
  ...
  def GetCommandFlags(self, extra_flags=None):
    ...
    if extra_flags != None:
      if '--generate-jso' in extra_flags:
        for source in self.resources:
          suffix.append(source)
        jso_file = self.main + '.jso'
        jso_out_flag = '--aot_out=' + jso_file
        if os.path.exists(jso_file):
          os.path.remove(jso_file)
        suffix += [jso_out_flag]
        extra_flags.remove('--generate-jso')


      if '--load-jso' in extra_flags:
        jso_in_flag = '--aot_in=' + self.main + '.jso'
        suffix += [jso_in_flag]
        extra_flags.remove('--load-jso')
  ...
  def GetCommand(self, shell_dir, extra_flags=None):
    ...
    # print "len(extra_flags)=" + str(len(extra_flags))
    if '--generate-jso' in extra_flags:
      cmd = [os.path.join(shell_dir, 'mkjso')]
    else:
...
class Platform(object):
  def __init__(self, options):
    ...
    self.test_jso = options.test_jso
...
class DesktopPlatform(Platform):
  ...
  def _Run(self, runnable, count, no_patch=False):
    ...
    # If test_jso is enabled, generate jso file
    if self.test_jso:
      if self.extra_flags != None:
        self.extra_flags.append('--generate-jso')
      else:
        self.extra_flags == ['--generate-jso']

      command_jso = self.command_prefix + runnable.GetCommand(shell_dir,
                                                              self.extra_flags)
      try:
    ...
    if self.test_jso:
      self.extra_flags = [ '--load-jso' if x == '--generate-jso' else x  for x in self.extra_flags ]
...
def Main(args):
  ...
  parser.add_option("--test-jso",
                    help="Test with jso, firstly generate js aot file, then load it and execute it",
                    default=False, action="store_true")

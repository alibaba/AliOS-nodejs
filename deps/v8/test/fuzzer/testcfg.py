...
class FuzzerTestSuite(testsuite.TestSuite):
  ...
  def __init__(self, name, root, target_root=None, device=None):
    super(FuzzerTestSuite, self).__init__(name, root, target_root, device)
  ...
  def GetFlagsForTestCase(self, testcase, context):
    ...
    if self.target_root is None:
        ...
        return [os.path.join(self.target_root, "test", "fuzzer", suite, name)]
...
def GetSuite(name, root, target_root=None, device=None):
  return FuzzerTestSuite(name, root, target_root, device)

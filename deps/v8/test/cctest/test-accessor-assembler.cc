...
TEST(TryProbeStubCache) {
...

  {
    ...
    Node* stub_cache_node = m.ExternalConstant(
         ExternalReference(TestReference(reinterpret_cast<Address>(&stub_cache))));
    m.TryProbeStubCache(stub_cache_node, receiver, name, &if_handler, &var_handler,
                        &if_miss);
    m.BIND(&if_handler);

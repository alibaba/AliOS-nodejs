...
class InspectorProtocolTestSuite(testsuite.TestSuite):
  ...
  def __init__(self, name, root, target_root=None, device=None):
    super(InspectorProtocolTestSuite, self).__init__(name, root, target_root, device)
    self.PATH = ""
    tmp_root = root
    while True:
  ...
  def GetFlagsForTestCase(self, testcase, context):
    ...
    if self.target_root is None:
      ...
      testfilename = os.path.join(self.target_root, self.PATH, testcase.path + self.suffix())
      protocoltestfilename = os.path.join(self.target_root, self.PATH, PROTOCOL_TEST_JS)
...
def GetSuite(name, root, target_root=None, device=None):
  return InspectorProtocolTestSuite(name, root, target_root, device)

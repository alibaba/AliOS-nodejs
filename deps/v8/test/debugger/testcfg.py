...
class DebuggerTestSuite(testsuite.TestSuite):
  ...
  def __init__(self, name, root, target_root=None, device=None):
    super(DebuggerTestSuite, self).__init__(name, root, target_root, device)
    self.PATH = ""
    tmp_root = root
    while True:
  ...
  def GetFlagsForTestCase(self, testcase, context):
    ...
    if self.target_root is None:
      ...
      files.append(os.path.normpath(os.path.join(self.target_root, self.PATH, "..", "mjsunit", "mjsunit.js")))
      files.append(os.path.join(self.target_root, self.PATH, "test-api.js"))
      files.extend([ os.path.normpath(os.path.join(self.target_root, self.PATH,'..', '..', f))
                    for f in files_list ])
      if MODULE_PATTERN.search(source):
        files.append("--module")
      files.append(os.path.join(self.target_root, self.PATH, testcase.path + self.suffix()))
...
def GetSuite(name, root, target_root=None, device=None):
  return DebuggerTestSuite(name, root, target_root, device=None)

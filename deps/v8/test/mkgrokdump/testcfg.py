...
class MkGrokdump(testsuite.TestSuite):
  ...
  def __init__(self, name, root, target_root=None, device=None):
    super(MkGrokdump, self).__init__(name, root, target_root, device)
  ...
  def IsFailureOutput(self, testcase):
    ...
    self_root = self.root
    if self.target_root:
      self_root = os.path.join(self.target_root, "test", "mkgrokdump")
    v8_path = os.path.dirname(os.path.dirname(os.path.abspath(self_root)))
...
def GetSuite(name, root, target_root=None, device=None):
  return MkGrokdump(name, root, target_root, device)
